...
if ( ! function_exists( 'aureon_spacing_customize_register' ) ) {
	add_action( 'customize_register', 'aureon_spacing_customize_register', 99 );
	/**
	 ...
	 */
	function aureon_spacing_customize_register( $wp_customize ) {
...
if ( ! function_exists( 'aureon_right_sidebar_width' ) ) {
	add_filter( 'aureon_right_sidebar_width', 'aureon_right_sidebar_width' );
	/**
	 * Set our right sidebar width.
	 *
	 * @param int $width The sidebar width.
	 */
	function aureon_right_sidebar_width( $width ) {
		// Bail if we don't have our defaults.
		if ( ! function_exists( 'aureon_spacing_get_defaults' ) ) {
			return $width;
		}
...
if ( ! function_exists( 'aureon_left_sidebar_width' ) ) {
	add_filter( 'aureon_left_sidebar_width', 'aureon_left_sidebar_width' );
	/**
	 * Set our left sidebar width.
	 *
	 * @param int $width The sidebar width.
	 */
	function aureon_left_sidebar_width( $width ) {
		// Bail if we don't have our defaults.
		if ( ! function_exists( 'aureon_spacing_get_defaults' ) ) {
			return $width;
		}
...
if ( ! function_exists( 'aureon_spacing_customizer_live_preview' ) ) {
	add_action( 'customize_preview_init', 'aureon_spacing_customizer_live_preview' );
	/**
	 * Add our live preview JS
	 */
	function aureon_spacing_customizer_live_preview() {
		wp_enqueue_script(
			'aureon-spacing-customizer',
			trailingslashit( plugin_dir_url( __FILE__ ) ) . 'customizer/js/customizer.js',
			array( 'jquery', 'customize-preview' ),
			AUREON_SPACING_VERSION,
			true
		);
...
if ( ! function_exists( 'aureon_include_spacing_defaults' ) ) {
	/**
	 * Check if we should include our default.css file.
	 *
	 * @since 1.3.42
	 */
	function aureon_include_spacing_defaults() {
		return true;
	}
}

if ( ! function_exists( 'aureon_spacing_premium_defaults' ) ) {
	add_filter( 'aureon_spacing_option_defaults', 'aureon_spacing_premium_defaults' );
	/**
	 ...
	 */
	function aureon_spacing_premium_defaults( $defaults ) {
		...
		// These defaults were added to Aureon (free) in 3.0.0.
 ...
 */
function aureon_spacing_do_premium_css() {
	// Bail if we don't have our defaults.
	if ( ! function_exists( 'aureon_spacing_get_defaults' ) ) {
		return;
	}
	...
	// This CSS was added to Aureon (free) in 3.0.0.
	if ( defined( 'AUREON_VERSION' ) && version_compare( AUREON_VERSION, '3.0.0-alpha.1', '<' ) ) {
	...
	if ( function_exists( 'aureon_menu_plus_get_defaults' ) ) {
		...
		if ( 'false' !== $menu_plus['sticky_menu'] && '' !== $spacing_settings['sticky_menu_item_height'] ) {
			$premium_css->start_media_query( aureon_premium_get_media_query( 'tablet' ) . ',' . aureon_premium_get_media_query( 'desktop' ) );
...
if ( ! function_exists( 'aureon_spacing_premium_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'aureon_spacing_premium_css', 105 );
	/**
	 * Add premium spacing CSS
	 *
	 * @since 1.3
	 */
	function aureon_spacing_premium_css() {
		$css = aureon_spacing_do_premium_css();

		if ( 'inline' === aureon_get_css_print_method() && $css ) {
			wp_add_inline_style( 'aureon-style', $css );
		}
	}
}

add_filter( 'aureon_external_dynamic_css_output', 'aureon_spacing_add_to_external_stylesheet' );
/**
 ...
 */
function aureon_spacing_add_to_external_stylesheet( $css ) {
