...
add_action( 'customize_controls_enqueue_scripts', 'aureon_premium_control_inline_scripts', 100 );
/**
 ...
 * @since 1.4
 */
function aureon_premium_control_inline_scripts() {
	$is_using_dynamic_typography = function_exists( 'aureon_is_using_dynamic_typography' ) && aureon_is_using_dynamic_typography();

	if ( function_exists( 'aureon_typography_default_fonts' ) && ! $is_using_dynamic_typography ) {
		$number_of_fonts = apply_filters( 'aureon_number_of_fonts', 200 );

		wp_localize_script( 'aureon-pro-typography-customizer', 'aureon_customize', array( 'nonce' => wp_create_nonce( 'aureon_customize_nonce' ) ) );
	...
	wp_enqueue_script( 'aureon-pro-customizer-controls', plugin_dir_url( __FILE__ )  . 'customizer/controls/js/aureon-controls.js', array( 'customize-controls', 'jquery' ), AUREON_STUDIO_VERSION, true );

	$overlay_defaults = apply_filters( 'aureon_off_canvas_overlay_style_defaults', array(
	...
	wp_localize_script(
		'aureon-button-actions',
		'aureonButtonActions',
		array(
			'warning' => esc_html__( 'This will design your overlay by changing options in the Customizer for you. Once saved, this can not be undone.', 'aureon-studio' ),
			'styling' => $overlay_defaults,
		)
	);

	$controls_a11y = array(
		'fontSizeLabel' => esc_html__( 'Font size', 'aureon-studio' ),
		'mobileHeaderFontSizeLabel' => esc_html__( 'Mobile header font size', 'aureon-studio' ),
	);

	if ( function_exists( 'aureon_get_default_fonts' ) ) {
		$font_defaults = aureon_get_default_fonts();
	...
	wp_localize_script(
		...
		array(
			...
			'hasWooCommerce' => class_exists( 'WooCommerce' ) && aureon_is_module_active( 'aureon_package_woocommerce', 'AUREON_WOOCOMMERCE' ),
		)
	);
}

add_action( 'customize_register', 'aureon_premium_customizer_shortcut_controls', 100 );
/**
 * Add shortcuts to sections we don't control in this plugin.
 *
 * @since 1.8
 */
function aureon_premium_customizer_shortcut_controls( $wp_customize ) {
	...
	$wp_customize->add_control(
		new Aureon_Section_Shortcut_Control(
			$wp_customize,
			'aureon_header_layout_shortcuts',
			array(
	...
	$wp_customize->add_control(
		new Aureon_Section_Shortcut_Control(
			$wp_customize,
			'aureon_primary_navigation_layout_shortcuts',
			array(
	...
	if ( $wp_customize->get_control( 'blogname' ) ) {
		$wp_customize->get_control( 'aureon_settings[container_width]' )->priority = 1;
	}

	$wp_customize->add_control(
		new Aureon_Section_Shortcut_Control(
			$wp_customize,
			'aureon_content_layout_shortcuts',
			array(
	...
	$wp_customize->add_control(
		new Aureon_Section_Shortcut_Control(
			$wp_customize,
			'aureon_sidebar_layout_shortcuts',
			array(
	...
	$wp_customize->add_control(
		new Aureon_Section_Shortcut_Control(
			$wp_customize,
			'aureon_footer_layout_shortcuts',
			array(
	...
	$wp_customize->add_control(
		new Aureon_Section_Shortcut_Control(
			$wp_customize,
			'aureon_colors_shortcuts',
			array(
				'section' => 'aureon_colors_section',
				'element' => __( 'Colors', 'aureon-studio' ),
				'shortcuts' => array(),
				'settings' => ( isset( $wp_customize->selective_refresh ) ) ? array() : 'blogname',
				'priority' => 1,
			)
	...
	$wp_customize->add_control(
		new Aureon_Section_Shortcut_Control(
			$wp_customize,
			'aureon_typography_shortcuts',
			array(
				'section' => 'aureon_typography_section',
				'element' => __( 'Typography', 'aureon-studio' ),
				'shortcuts' => array(),
				'settings' => ( isset( $wp_customize->selective_refresh ) ) ? array() : 'blogname',
				'priority' => 1,
			)
...
add_action( 'customize_register', 'aureon_premium_layout_block_element_messages', 1000 );
/**
 * Add shortcuts to sections we don't control in this plugin.
 *
 * @since 1.8
 */
function aureon_premium_layout_block_element_messages( $wp_customize ) {
	if ( ! class_exists( 'WP_Customize_Panel' ) ) {
		return;
	}
	...
	if ( version_compare( PHP_VERSION, '5.6', '>=' ) ) {
			...
			if ( $wp_customize->get_section( $section ) ) {
				$wp_customize->add_control(
					new Aureon_Information_Customize_Control(
						$wp_customize,
						'aureon_using_site_footer_element_' . $section,
						array(
							'section'     => $section,
							'description' => sprintf(
								/* translators: URL to the Elements dashboard. */
								__( 'This page is using a <a href="%s">Site Footer Element</a>. Some of the options below may not apply.', 'aureon-studio' ),
								admin_url( 'edit.php?post_type=aureon_elements' )
							),
							'notice' => true,
							'settings' => ( isset( $wp_customize->selective_refresh ) ) ? array() : 'blogname',
							'active_callback' => function() {
								$has_block_element = aureon_has_active_element( 'site-footer', true );
		...
		foreach ( $header_sections as $section ) {
			if ( $wp_customize->get_section( $section ) ) {
				$wp_customize->add_control(
					new Aureon_Information_Customize_Control(
						$wp_customize,
						'aureon_using_site_header_element_' . $section,
						array(
							'section'     => $section,
							'description' => sprintf(
								/* translators: URL to the Elements dashboard. */
								__( 'This page is using a <a href="%s">Site Header Element</a>. Some of the options below may not apply.', 'aureon-studio' ),
								admin_url( 'edit.php?post_type=aureon_elements' )
							),
							'notice' => true,
							'settings' => ( isset( $wp_customize->selective_refresh ) ) ? array() : 'blogname',
							'active_callback' => function() {
								$has_block_element = aureon_has_active_element( 'site-header', true );
		...
		foreach ( $sidebar_sections as $section ) {
			if ( $wp_customize->get_section( $section ) ) {
				$wp_customize->add_control(
					new Aureon_Information_Customize_Control(
						$wp_customize,
						'aureon_using_sidebar_element_' . $section,
						array(
							'section'     => $section,
							'description' => sprintf(
								/* translators: URL to the Elements dashboard. */
								__( 'This page is using a <a href="%s">Sidebar Element</a>. Some of the options below may not apply.', 'aureon-studio' ),
								admin_url( 'edit.php?post_type=aureon_elements' )
							),
							'notice' => true,
							'settings' => ( isset( $wp_customize->selective_refresh ) ) ? array() : 'blogname',
							'active_callback' => function() {
								$has_right_sidebar_block_element = aureon_has_active_element( 'right-sidebar', true );

								if ( $has_right_sidebar_block_element ) {
									return true;
								}

								$has_left_sidebar_block_element = aureon_has_active_element( 'left-sidebar', true );
		...
		if ( $wp_customize->get_section( 'aureon_blog_section' ) ) {
			$wp_customize->add_control(
				new Aureon_Information_Customize_Control(
					$wp_customize,
					'aureon_using_post_loop_item_element',
					array(
						'section'     => 'aureon_blog_section',
						'description' => sprintf(
							/* translators: URL to the Elements dashboard. */
							__( 'This page is using a <a href="%s">Content Template Element</a>. Some of the options below may not apply.', 'aureon-studio' ),
							admin_url( 'edit.php?post_type=aureon_elements' )
						),
						'notice' => true,
						'settings' => ( isset( $wp_customize->selective_refresh ) ) ? array() : 'blogname',
						'active_callback' => function() {
							$has_block_element = aureon_has_active_element( 'content-template', true );
			...
			$wp_customize->add_control(
				new Aureon_Information_Customize_Control(
					$wp_customize,
					'aureon_using_page_hero_element',
					array(
						'section'     => 'aureon_blog_section',
						'description' => sprintf(
							/* translators: URL to the Elements dashboard. */
							__( 'This page is using a <a href="%s">Page Hero Element</a>. Some of the options below may not apply.', 'aureon-studio' ),
							admin_url( 'edit.php?post_type=aureon_elements' )
						),
						'notice' => true,
						'settings' => ( isset( $wp_customize->selective_refresh ) ) ? array() : 'blogname',
						'active_callback' => function() {
							$has_block_element = aureon_has_active_element( 'page-hero', true );
			...
			$wp_customize->add_control(
				new Aureon_Information_Customize_Control(
					$wp_customize,
					'aureon_using_post_meta_area_element',
					array(
						'section'     => 'aureon_blog_section',
						'description' => sprintf(
							/* translators: URL to the Elements dashboard. */
							__( 'This page is using a <a href="%s">Post Meta Template Element</a>. Some of the options below may not apply.', 'aureon-studio' ),
							admin_url( 'edit.php?post_type=aureon_elements' )
						),
						'notice' => true,
						'settings' => ( isset( $wp_customize->selective_refresh ) ) ? array() : 'blogname',
						'active_callback' => function() {
							$has_block_element = aureon_has_active_element( 'post-meta-template', true );
...
add_action( 'customize_controls_print_styles', 'aureon_premium_customize_print_styles' );
/**
 * Print control styles for the Customizer.
 *
 * @since 1.9
 */
function aureon_premium_customize_print_styles() {
	$sizes = apply_filters( 'aureon_customizer_device_preview_sizes', array(
		'tablet' => 800,
		'mobile' => 411,
		'mobile_height' => 731,
	) );
