	...
	function aureon_enqueue_google_fonts() {
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				$variants = aureon_get_google_font_variants( $aureon_settings[ $key ], $key );
		...
		if ( $google_fonts ) {
			wp_enqueue_style( 'aureon-fonts', $fonts_url, array(), null, 'all' ); // phpcs:ignore
		}
	}
}

if ( ! function_exists( 'aureon_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'aureon_default_fonts_customize_register' );
	/**
	 ...
	 */
	function aureon_default_fonts_customize_register( $wp_customize ) {
		if ( aureon_is_using_dynamic_typography() ) {
			return;
		}

		if ( function_exists( 'aureon_fonts_customize_register' ) ) {
			// Bail if Aureon Studio is active.
			return;
		}
		...
		if ( method_exists( $wp_customize, 'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Aureon_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'aureon_sanitize_variants',
			)
		);
		...
		$wp_customize->add_control(
			new Aureon_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'label' => __( 'Body', 'aureon' ),
		...
		$wp_customize->add_control(
			new Aureon_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'aureon' ),
					'section' => 'font_section',
					'settings' => array(
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'type' => 'option',
				'sanitize_callback' => 'aureon_sanitize_decimal_integer',
				'transport' => 'postMessage',
			)
		);
		...
		$wp_customize->add_control(
			new Aureon_Range_Slider_Control(
				...
				array(
					'type' => 'aureon-range-slider',
					'description' => __( 'Line height', 'aureon' ),
					'section' => 'font_section',
					'settings' => array(
		...
		$wp_customize->add_control(
			new Aureon_Range_Slider_Control(
				...
				array(
					'type' => 'aureon-range-slider',
					'description' => __( 'Paragraph margin', 'aureon' ),
					'section' => 'font_section',
					'settings' => array(
		...
		if ( ! function_exists( 'aureon_fonts_customize_register' ) && ! defined( 'AUREON_STUDIO_VERSION' ) ) {
			$wp_customize->add_control(
...
if ( ! function_exists( 'aureon_get_all_google_fonts_ajax' ) ) {
	...
	function aureon_get_all_google_fonts_ajax() {
		...
		if ( ! isset( $_POST['aureon_customize_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['aureon_customize_nonce'] ), 'aureon_customize_nonce' ) ) {
			wp_die();
		}
...
if ( ! function_exists( 'aureon_get_google_font_variants' ) ) {
	 ...
	 */
	function aureon_get_google_font_variants( $font, $key = '' ) {
...
if ( ! function_exists( 'aureon_get_google_font_category' ) ) {
	 ...
	 */
	function aureon_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'aureon_get_font_family_css' ) ) {
	...
	function aureon_get_font_family_css( $font, $settings, $default ) {
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . aureon_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'aureon_add_to_font_customizer_list' ) ) {
	add_filter( 'aureon_typography_customize_list', 'aureon_add_to_font_customizer_list' );
	/**
	 ...
	 */
	function aureon_add_to_font_customizer_list( $fonts ) {
		...
		$all_fonts = aureon_get_all_google_fonts();
		$select_fonts = aureon_get_all_google_fonts( apply_filters( 'aureon_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going.
			if ( ! isset( $aureon_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $aureon_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $aureon_settings[ $setting ], aureon_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $aureon_settings[ $setting ] ) ) ] = array(
				'name' => $aureon_settings[ $setting ],
		...
		if ( function_exists( 'aureon_secondary_nav_get_defaults' ) ) {
			...
			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings['font_secondary_navigation'], aureon_typography_default_fonts() ) ) {
...
if ( ! function_exists( 'aureon_typography_set_font_data' ) ) {
	add_action( 'admin_init', 'aureon_typography_set_font_data' );
	/**
	 ...
	 * @since 1.3.40
	 */
	function aureon_typography_set_font_data() {
		if ( aureon_is_using_dynamic_typography() ) {
			return;
		}
		...
		if ( $defaults['font_body'] == $aureon_settings['font_body'] ) { // phpcs:ignore
			return;
		}

		// Don't need to continue if we're using a system font or our default font.
		if ( in_array( $aureon_settings['font_body'], aureon_typography_default_fonts() ) ) {
			return;
		}
		...
		// Get the ID from our font.
		$id = strtolower( str_replace( ' ', '_', $aureon_settings['font_body'] ) );
