 ...
 */
class Aureon_Typography_Migration {
	/**
	 * Class instance.
	 *
	 * @access private
	...
	public static function get_option_prefixes() {
		...
		if ( function_exists( 'aureon_secondary_nav_typography_selectors' ) ) {
			$data[] = array(
		...
		if ( function_exists( 'aureon_menu_plus_typography_selectors' ) ) {
			$data[] = array(
		...
		if ( function_exists( 'aureon_woocommerce_typography_selectors' ) ) {
			$data[] = array(
	...
	public static function get_mapped_typography_data() {
		...
		foreach ( self::get_option_prefixes() as $key => $data ) {
			...
			foreach ( $legacy_setting_ids as $name => $id ) {
				...
				if ( 'secondary_navigation' === $data['legacy_prefix'] && function_exists( 'aureon_secondary_nav_get_defaults' ) ) {
					$secondary_nav_settings = get_option( 'aureon_secondary_nav_settings', array() );
					$secondary_nav_defaults = aureon_secondary_nav_get_defaults();
	...
	public static function get_mapped_font_data() {
		...
		foreach ( self::get_option_prefixes() as $key => $data ) {
			...
			if ( 'secondary_navigation' === $data['legacy_prefix'] && function_exists( 'aureon_secondary_nav_get_defaults' ) ) {
				$settings = get_option( 'aureon_secondary_nav_settings', array() );
			...
			if ( self::has_saved_value( 'font_' . $data['legacy_prefix'], $settings, $defaults ) ) {
				...
				$local_fonts = aureon_typography_default_fonts();
...
Aureon_Typography_Migration::get_instance();
