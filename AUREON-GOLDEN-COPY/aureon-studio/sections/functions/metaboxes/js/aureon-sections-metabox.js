...
(function($, Aureon_Sections) {


    // Model
    Aureon_Sections.Section = Backbone.Model.extend({
        defaults: {
            "title": "",
            "box_type": "",
            "inner_box_type": "",
    ...
    // Collection
    Aureon_Sections.SectionsCollection = Backbone.Collection.extend({
        model: Aureon_Sections.Section,
        el: "#aureon_sections_container",
     ...
     */
    Aureon_Sections.backbone_modal.Application = Backbone.View.extend({

        attributes: {
            id: "aureon-sections-modal-dialog",
            class: "aureon-sections-modal",
            role: "dialog"
        },

        template: wp.template("aureon-sections-modal-window"),
        ...
        events: {
            ...
            "keydown": "keydown",
            "click .aureon-sections-upload-button": "openMediaUploader",
            "click .aureon-sections-remove-image": "removeImage",
        ...
        changeInsertText: function(restore) {
            ...
            if (restore == false && typeof(wp.media.view.l10n.insertIntoPost) !== "undefined") {
                ...
                wp.media.view.l10n.insertIntoPost = aureon_sections_metabox_i18n.insert_into_section;
        ...
        render: function() {
            ...
            jQuery("body").addClass("aureon-modal-open").prepend(this.$el);
		...
		launchEditor: function() {
			var id = this.ui.panels.find( ".aureon-sections-editor-wrap" ).find( 'textarea' ).attr( 'id' ),
				customButtonsContainer = this.ui.panels.find( '.aureon-sections-editor-wrap' ).find( '#custom-media-buttons' );
			...
			var custom_settings = {
				...
				setup: function( editor ) {
					editor.on( 'init', function( e ) {
						if ( 'html-active' === aureon_sections_metabox_i18n.default_editor && aureon_sections_metabox_i18n.user_can_richedit ) {
							thisPanel.find( '#wp-aureon-sections-editor-wrap' ).removeClass( 'tmce-active' ).addClass( 'html-active' );
							editor.hidden = true;
						}
					} );
				}
			...
			if ( ! aureon_sections_metabox_i18n.user_can_richedit ) {
				init_settings = false;
			}
			...
			var buttonsElement = this.ui.panels.find( '#wp-aureon-sections-editor-wrap' ).find( '.wp-media-buttons' );
			buttonsElement.attr( 'id', 'custom-media-buttons' );
			$( customButtons ).appendTo( buttonsElement );

			if ( ! aureon_sections_metabox_i18n.user_can_richedit ) {
				this.ui.panels.find( '#aureon-sections-editor' ).addClass( 'no-rich-edit' );
			}
		},
        ...
        renderContent: function() {
            ...
            var menu_item = wp.template("aureon-sections-modal-menu-item");
            ...
            if (aureon_sections_metabox_i18n.tabs.length) {

                // for...of is nicer, but not supported by minify, so stay with for...in for now
                for (var tab in aureon_sections_metabox_i18n.tabs) {

                    if (aureon_sections_metabox_i18n.tabs.hasOwnProperty(tab)) {

                        tab = aureon_sections_metabox_i18n.tabs[tab];
						...
						var panel = wp.template("aureon-sections-edit-" + tab.target);
						...
						jQuery( 'body' ).on( 'aureon_section_show_settings', function() {
							jQuery( '.aureon-sections-modal .media-menu-item' ).removeClass('active');
							jQuery( '.aureon-sections-modal .panel' ).removeClass('active');
							jQuery( '.aureon-section-item-style' ).addClass('active');
							jQuery( '.aureon-section-settings' ).addClass('active');
						});
        ...
        renderPreview: function(image_id) {
            ...
            var $preview = $("#aureon-section-image-preview");
                ...
                this.background.fetch({
                    success: function(att) {
                        if (_.contains(['png', 'jpg', 'gif', 'jpeg'], att.get('subtype'))) {
                            ...
                            $preview.next().find(".aureon-sections-remove-image").show();
                        }
                    }
                });
            }
        ...
        colorPicker: function() {
            this.$el.find(".aureon-sections-color").wpColorPicker();
        },
        ...
        openMediaUploader: function(e) {
            ...
            $input = jQuery(e.currentTarget).prev("#aureon-sections-background-image");
            ...
            this.mediaUploader = wp.media.frames.file_frame = wp.media({
                title: aureon_sections_metabox_i18n.media_library_title,
                button: {
                    text: aureon_sections_metabox_i18n.media_library_button
                },
                multiple: false
            });
        ...
        removeImage: function(e) {
            e.preventDefault();
            $("#aureon-section-image-preview").children().remove();
            $("#aureon-section-image-preview").next().find(".aureon-sections-remove-image").hide();
            $("#aureon-sections-background-image").val("");
        },
        ...
        closeModal: function(e) {
            ...
            jQuery("body").removeClass("aureon-modal-open");
            jQuery("body").removeClass("aureon-section-content");
			...
			if (typeof tinyMCE != "undefined") {
				tinymce.EditorManager.execCommand("mceRemoveEditor", true, "aureon-sections-editor");
			}

            // remove modal and unset instances
            this.remove();
            Aureon_Sections.backbone_modal.__instance = undefined;
            this.mediaUploader = null;
            Aureon_Sections.modalOpen = null;

            // switch the insert button text back
            this.changeInsertText(true);

            // send focus back to where it was prior to modal open
            Aureon_Sections.lastFocus.focus();
        ...
        switchTab: function(e) {
            ...
            if (typeof tinyMCE != "undefined" && 'style' == jQuery( e.currentTarget ).data( 'target' ) && this.ui.panels.find( '#wp-aureon-sections-editor-wrap' ).hasClass( 'tmce-active' )) {
    ...
    // Singular View
    Aureon_Sections.sectionView = Backbone.View.extend({
        ...
        template: wp.template("aureon-sections-section"),
        ...
        render: function() {
            ...
            if (!this.model.get('title')) {
                this.$el.find('h3.section-title > span').text(aureon_sections_metabox_i18n.default_title);
            }
        ...
        editSection: function(e) {
            ...
            Aureon_Sections.lastFocus = document.activeElement;
            Aureon_Sections.modalOpen = true;

            e.preventDefault();
            if (Aureon_Sections.backbone_modal.__instance === undefined) {
                Aureon_Sections.backbone_modal.__instance = new Aureon_Sections.backbone_modal.Application({
                    model: this.model
                });
            }
        ...
        removeSection: function(e) {
            e.preventDefault();
            if (confirm(aureon_sections_metabox_i18n.confirm)) {
                this.model.destroy();
                Aureon_Sections.sectionList.render(); // manually calling instead of listening since listening interferes with sorting
            }
        },
    });
    ...
    // List View
    Aureon_Sections.sectionListView = Backbone.View.extend({
        ...
        addOne: function(model) {
            var view = new Aureon_Sections.sectionView({
                model: model
            });
    ...
    // The Buttons & Nonce
    Aureon_Sections.ButtonControls = Backbone.View.extend({

        attributes: {
            class: "aureon_sections_buttons"
        },

        tagName: 'p',

        el: "#_aureon_sections_metabox",

        template: wp.template("aureon-sections-buttons"),

        // Attach events
        events: {
            "click .button-primary": "newSection",
            "click #aureon-delete-sections": "clearAll",
            'click .edit-section': 'editSection',
        },
        ...
        newSection: function(e) {
            e.preventDefault();
            var newSection = new Aureon_Sections.Section();
            Aureon_Sections.sectionList.addSection(newSection);
        },
        ...
        clearAll: function(e) {
            e.preventDefault();
            if (confirm(aureon_sections_metabox_i18n.confirm)) {
                Aureon_Sections.sectionCollection.reset();
                Aureon_Sections.sectionList.render();
            }
        },

        render: function() {
            this.$el.find(".aureon_sections_control").append(this.template);
            return this;
        },
    ...
    // init
    Aureon_Sections.initApplication = function() {

        // Create Collection From Existing Meta
        Aureon_Sections.sectionCollection = new Aureon_Sections.SectionsCollection(aureon_sections_metabox_i18n.sections);

        // Create the List View
        Aureon_Sections.sectionList = new Aureon_Sections.sectionListView({
            collection: Aureon_Sections.sectionCollection
        });
        Aureon_Sections.sectionList.render();

        // Buttons
        Aureon_Sections.Buttons = new Aureon_Sections.ButtonControls({
            collection: Aureon_Sections.sectionCollection
        });
        Aureon_Sections.Buttons.render();

    };


    /*-----------------------------------------------------------------------------------*/
    /* Execute the above methods in the Aureon_Sections object.
    /*-----------------------------------------------------------------------------------*/

	jQuery( function( $ ) {

        Aureon_Sections.initApplication();

        $( '#aureon_sections_container' ).sortable({
            axis: "y",
            opacity: 0.5,
            grid: [20, 10],
            tolerance: "pointer",
        ...
        function aureonShowSections() {
			...
			// Set a trigger
            $('body').trigger('aureon_show_sections');

        }

        function aureonHideSections() {
            ...
            $('body').trigger('aureon_hide_sections');

        }

		$( document ).on( 'click', '.edit-section.edit-settings', function() {
			$( 'body' ).trigger( 'aureon_section_show_settings' );
		});

    });


})(jQuery, Aureon_Sections);
