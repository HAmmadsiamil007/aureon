...
add_filter( 'aether_adapter_wc_filter_data', 'ferm_living_wc_filter_data' );
...
function ferm_living_shop_hero_data( $data ) {
	...
	// Reference: text-only h1, no label or subtitle.
	$data['label']    = '';
	$data['subtitle'] = '';
...
/**
 * Override shop filter bar — Ferm Living categories instead of AETHER shoe categories.
 *
 * Reference: fermliving.com shop page shows Furniture, Lighting, Accessories,
 * Kids, Textiles, Kitchen, Outdoor Living — not Men's Boots, Women's Bags etc.
 */
function ferm_living_wc_filter_data( $data ) {
	if ( ! function_exists( 'aether_active_design' ) || 'fermliving' !== aether_active_design() ) {
		return $data;
	}
	...
	// Ferm Living categories — matching frozen reference.
	$categories = array(
		'Furniture'     => home_url( '/collections/furniture' ),
		'Lighting'      => home_url( '/collections/lighting' ),
		'Accessories'   => home_url( '/collections/accessories' ),
		'Kids'          => home_url( '/collections/kids' ),
		'Textiles'      => home_url( '/collections/textiles' ),
		'Kitchen'       => home_url( '/collections/kitchen' ),
		'Outdoor Living'=> home_url( '/collections/outdoor-living' ),
	);
...
/**
 * Replace AETHER blog post content with Ferm Living content in reference mode.
 */
add_filter( 'the_content', 'ferm_living_post_content_filter', 999 );

function ferm_living_post_content_filter( $content ) {
	if ( ! function_exists( 'aether_active_design' ) || 'fermliving' !== aether_active_design() ) {
		return $content;
	}
	...
	if ( str_contains( $title, 'AETHER Sample Post' ) || str_contains( $title, 'Step Into the Void' ) ) {
		return '<p>Welcome to the Ferm Living journal. We believe in the beauty of simplicity, the warmth of natural materials, and the quiet confidence of Scandinavian design. Our collections bring together form and function — pieces that feel timeless in your home, not trendy.</p>'
			. '<p>From handwoven textiles to sculptural furniture, every Ferm Living piece is crafted with intention. We partner with artisans across Europe who share our commitment to sustainable production and honest materials.</p>'
			. '<p>The philosophy we strip away the excess. No unnecessary ornamentation, no trend-chasing — just thoughtful design that serves your daily life. Explore our latest collections and find pieces that speak to the way you live.</p>';
	}

	if ( str_contains( $title, 'Hello world!' ) || str_contains( $title, 'Hello World' ) ) {
		return '<p>Welcome to Ferm Living — where Scandinavian simplicity meets contemporary living. We design furniture, lighting, and accessories that bring warmth and character to your home.</p>'
			. '<p>Founded in Copenhagen, our collections are inspired by the Nordic tradition of honest design. Every piece starts with a question: how can this make your daily life a little more beautiful?</p>'
			. '<p>Explore our collections and discover how Ferm Living can transform your space.</p>';
	}
...
/**
 * Replace AETHER blog post excerpts with Ferm Living content in reference mode.
 */
add_filter( 'wp_trim_excerpt', 'ferm_living_excerpt_filter', 999, 2 );

function ferm_living_excerpt_filter( $excerpt, $post = null ) {
	if ( ! function_exists( 'aether_active_design' ) || 'fermliving' !== aether_active_design() ) {
		return $excerpt;
	}
	if ( is_admin() ) {
	...
	if ( str_contains( $title, 'AETHER Sample Post' ) || str_contains( $title, 'Step Into the Void' ) ) {
		return 'We believe in the beauty of simplicity, the warmth of natural materials, and the quiet confidence of Scandinavian design. Our collections bring together form and function — pieces that feel timeless in your home… <a class="more-link" href="' . esc_url( get_permalink( $post ) ) . '">Read more</a>';
	}
	if ( 'Hello world!' === $title || str_contains( $title, 'Hello world' ) ) {
		return 'Welcome to Ferm Living — where Scandinavian simplicity meets contemporary living. We design furniture, lighting, and accessories that bring warmth and character to your home… <a class="more-link" href="' . esc_url( get_permalink( $post ) ) . '">Read more</a>';
	}
...
/**
 * Replace AETHER blog adapter output with Ferm Living content in reference mode.
 */
add_filter( 'aether_adapter_blog_data', 'ferm_living_blog_adapter_filter' );

function ferm_living_blog_adapter_filter( $data ) {
	if ( ! function_exists( 'aether_active_design' ) || 'fermliving' !== aether_active_design() ) {
		return $data;
	}
	...
	$replacements = array(
		'The Art of Simple Living' => array(
			'excerpt' => 'We believe in the beauty of simplicity, the warmth of natural materials, and the quiet confidence of Scandinavian design. Our collections bring together form and function — pieces that feel timeless in your home…',
		),
		'Introducing Ferm Living' => array(
			'excerpt' => 'Welcome to Ferm Living — where Scandinavian simplicity meets contemporary living. We design furniture, lighting, and accessories that bring warmth and character to your home…',
		),
	);

	foreach ( $data['items'] as &$item ) {
		if ( isset( $replacements[ $item['title'] ] ) ) {
			$item['excerpt'] = $replacements[ $item['title'] ]['excerpt'];
			$item['alt']     = $item['title'];
		}
	}
...
/**
 * Replace AETHER blog post titles with Ferm Living content in reference mode.
 */
add_filter( 'the_title', 'ferm_living_post_title_filter', 999, 2 );

function ferm_living_post_title_filter( $title, $id = 0 ) {
	if ( ! function_exists( 'aether_active_design' ) || 'fermliving' !== aether_active_design() ) {
		return $title;
	}
	if ( is_admin() ) {
	...
	if ( str_contains( $title, 'AETHER Sample Post' ) || str_contains( $title, 'Step Into the Void' ) ) {
		return 'The Art of Simple Living';
	}
	if ( 'Hello world!' === $title || str_contains( $title, 'Hello world' ) ) {
		return 'Introducing Ferm Living';
	}
 ...
 * Replace AETHER blog post excerpts with Ferm Living content in reference mode.
 */
add_filter( 'the_excerpt', 'ferm_living_post_excerpt_filter', 999 );

function ferm_living_post_excerpt_filter( $excerpt ) {
	if ( ! function_exists( 'aether_active_design' ) || 'fermliving' !== aether_active_design() ) {
		return $excerpt;
	}
	if ( is_admin() ) {
	...
	$title = get_the_title( $post );
	if ( str_contains( $title, 'AETHER Sample Post' ) || str_contains( $title, 'Step Into the Void' ) ) {
		return 'We believe in the beauty of simplicity, the warmth of natural materials, and the quiet confidence of Scandinavian design. Our collections bring together form and function — pieces that feel timeless in your home…';
	}
	if ( 'Hello world!' === $title || str_contains( $title, 'Hello world' ) ) {
		return 'Welcome to Ferm Living — where Scandinavian simplicity meets contemporary living. We design furniture, lighting, and accessories that bring warmth and character to your home…';
	}
