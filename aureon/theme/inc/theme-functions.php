 ...
 */
function aureon_get_option( $option ) {
...
if ( ! function_exists( 'aureon_get_layout' ) ) {
	/**
	 ...
	 */
	function aureon_get_layout() {
		$layout = aureon_get_option( 'layout_setting' );

		if ( is_single() ) {
			$layout = aureon_get_option( 'single_layout_setting' );
		}

		if ( is_singular() ) {
			$layout_meta = get_post_meta( get_the_ID(), '_aureon-sidebar-layout-meta', true );
		...
		if ( is_home() || is_archive() || is_search() || is_tax() ) {
			$layout = aureon_get_option( 'blog_layout_setting' );
		}

		return apply_filters( 'aureon_sidebar_layout', $layout );
	}
}

if ( ! function_exists( 'aureon_get_footer_widgets' ) ) {
	/**
	 ...
	 */
	function aureon_get_footer_widgets() {
		$widgets = aureon_get_option( 'footer_widget_setting' );

		if ( is_singular() ) {
			$widgets_meta = get_post_meta( get_the_ID(), '_aureon-footer-widget-meta', true );
		...
		return apply_filters( 'aureon_footer_widgets', $widgets );
	}
}

if ( ! function_exists( 'aureon_show_excerpt' ) ) {
	/**
	 * Figure out if we should show the blog excerpts or full posts
	 *
	 * @since 1.3.15
	 */
	function aureon_show_excerpt() {
		global $post;

		// Check to see if the more tag is being used.
		$more_tag = apply_filters( 'aureon_more_tag', strpos( $post->post_content, '<!--more-->' ) );

		$format = ( false !== get_post_format() ) ? get_post_format() : 'standard';

		$show_excerpt = ( 'excerpt' === aureon_get_option( 'post_content' ) ) ? true : false;
		...
		return apply_filters( 'aureon_show_excerpt', $show_excerpt );
	}
}

if ( ! function_exists( 'aureon_show_title' ) ) {
	/**
	 ...
	 */
	function aureon_show_title() {
		return apply_filters( 'aureon_show_title', true );
	}
}
 ...
 * @since 3.0.0
 */
function aureon_show_entry_header() {
	$show_entry_header      = true;
	$show_title             = aureon_show_title();
	$has_before_entry_title = has_action( 'aureon_before_entry_title' );
	$has_after_entry_title  = has_action( 'aureon_after_entry_title' );

	if ( is_page() ) {
		$has_before_entry_title = has_action( 'aureon_before_page_title' );
		$has_after_entry_title  = has_action( 'aureon_after_page_title' );
	}
	...
	return apply_filters( 'aureon_show_entry_header', $show_entry_header );
}

if ( ! function_exists( 'aureon_get_premium_url' ) ) {
	/**
	 ...
	 * @return string The URL to aureonstudio.com.
	 */
	function aureon_get_premium_url( $url = 'https://aureonstudio.com/premium', $trailing_slash = true ) {
		if ( $trailing_slash ) {
			$url = trailingslashit( $url );
		}

		$args = apply_filters(
			'aureon_premium_url_args',
			array(
				'ref' => null,
				'campaign' => null,
			)
...
if ( ! function_exists( 'aureon_padding_css' ) ) {
	/**
	 ...
	 */
	function aureon_padding_css( $top, $right, $bottom, $left ) {
...
if ( ! function_exists( 'aureon_get_link_url' ) ) {
	/**
	 * Return the post URL.
	 *
...
if ( ! function_exists( 'aureon_get_navigation_location' ) ) {
	/**
	 ...
	 */
	function aureon_get_navigation_location() {
		return apply_filters( 'aureon_navigation_location', aureon_get_option( 'nav_position_setting' ) );
	}
}
 ...
 * @since 2.3
 */
function aureon_has_logo_site_branding() {
	$has_site_title = ! aureon_get_option( 'hide_title' ) && get_bloginfo( 'title' );
	$has_site_tagline = ! aureon_get_option( 'hide_tagline' ) && get_bloginfo( 'description' );
 ...
 */
function aureon_get_svg_icon( $icon, $replace = false ) {
	...
	$output = apply_filters( 'aureon_svg_icon_element', $output, $icon );
	...
	return apply_filters( 'aureon_svg_icon', $output, $icon );
}
 ...
 */
function aureon_do_svg_icon( $icon, $replace = false ) {
	echo aureon_get_svg_icon( $icon, $replace ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in function.
}
 ...
 */
function aureon_get_media_query( $name ) {
	$desktop = apply_filters( 'aureon_desktop_media_query', '(min-width:1025px)' );
	$tablet_only = apply_filters( 'aureon_tablet_media_query', '(min-width: 769px) and (max-width: 1024px)' );
	$mobile = apply_filters( 'aureon_mobile_media_query', '(max-width:768px)' );
	$mobile_menu = apply_filters( 'aureon_mobile_menu_media_query', $mobile );

	$queries = apply_filters(
		'aureon_media_queries',
		array(
			'desktop' => $desktop,
 ...
 */
function aureon_do_element_classes( $context, $class = '' ) {
	$after = apply_filters( 'aureon_after_element_class_attribute', '', $context );

	if ( $after ) {
		$after = ' ' . $after;
	}

	echo 'class="' . join( ' ', aureon_get_element_classes( $context, $class ) ) . '"' . $after; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in function.
}
 ...
 */
function aureon_get_element_classes( $context, $class = '' ) {
	$classes = array();
	...
	return apply_filters( "aureon_{$context}_class", $classes, $class );
}
 ...
 * @since 3.0.0
 */
function aureon_get_schema_type() {
	return apply_filters( 'aureon_schema_type', 'microdata' );
}
 ...
 */
function aureon_get_microdata( $context ) {
	$data = false;
	...
	if ( 'body' === $context ) {
		...
		$type = apply_filters( 'aureon_body_itemtype', $type );
	...
	if ( 'article' === $context ) {
		$type = apply_filters( 'aureon_article_itemtype', 'CreativeWork' );
	...
	if ( $data ) {
		return apply_filters( "aureon_{$context}_microdata", $data );
	}
}
 ...
 * @param string $context The element to target.
 */
function aureon_do_microdata( $context ) {
	echo aureon_get_microdata( $context ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in function.
}
 ...
 * @since 3.0.0
 */
function aureon_is_using_hatom() {
	return apply_filters( 'aureon_is_using_hatom', true );
}
 ...
 * @since 3.0.0
 */
function aureon_has_menu_bar_items() {
	return has_action( 'aureon_menu_bar_items' );
}
 ...
 */
function aureon_do_template_part( $template ) {
	/**
	 * aureon_before_do_template_part hook.
	 *
	 * @since 3.0.0
	 * @param string $template The template.
	 */
	do_action( 'aureon_before_do_template_part', $template );

	if ( apply_filters( 'aureon_do_template_part', true, $template ) ) {
	...
	/**
	 * aureon_after_do_template_parts hook.
	 *
	 * @since 3.0.0
	 * @param string $template The template.
	 */
	do_action( 'aureon_after_do_template_part', $template );
}
 ...
 * @since 3.0.0
 */
function aureon_has_inline_mobile_toggle() {
	$has_inline_mobile_toggle = aureon_is_using_flexbox() && ( 'nav-float-right' === aureon_get_navigation_location() || 'nav-float-left' === aureon_get_navigation_location() );

	return apply_filters( 'aureon_has_inline_mobile_toggle', $has_inline_mobile_toggle );
}
 ...
 * @since 3.0.0
 */
function aureon_get_the_title_parameters() {
	$params = array(
		'before' => sprintf(
	...
	return apply_filters( 'aureon_get_the_title_parameters', $params );
}
 ...
 * @since 3.0.0
 */
function aureon_has_default_loop() {
	return apply_filters( 'aureon_has_default_loop', true );
}
 ...
 * @since 3.0.0
 */
function aureon_needs_site_branding_container() {
	$container = false;

	if ( aureon_has_logo_site_branding() ) {
		if ( aureon_is_using_flexbox() || aureon_get_option( 'inline_logo_site_branding' ) ) {
			$container = true;
		}
	}
 ...
 *
 * The contextual filter is of the form `aureon_attr_{context}`.
 *
 * @since 3.1.0
 *
 ...
 */
function aureon_parse_attr( $context, $attributes = array(), $settings = array() ) {
	...
	$classes = aureon_get_element_classes( $context );
	...
	// Contextual filter.
	return apply_filters( 'aureon_parse_attr', $attributes, $context, $settings );
}

/**
 * Build list of attributes into a string and apply contextual filter on string.
 *
 * The contextual filter is of the form `aureon_attr_{context}_output`.
 *
 * @since 3.1.0
 *
 * @param string $context    The context, to build filter name.
 ...
 */
function aureon_get_attr( $context, $attributes = array(), $settings = array() ) {
	$attributes = aureon_parse_attr( $context, $attributes, $settings );
	...
	$output = apply_filters( 'aureon_get_attr_output', $output, $attributes, $context, $settings );
 ...
 * @param array  $settings   Optional. Custom data to pass to filter.
 */
function aureon_do_attr( $context, $attributes = array(), $settings = array() ) {
	echo aureon_get_attr( $context, $attributes, $settings ); // phpcs:ignore -- Escaping done in function.
}
 ...
 * @since 3.1.0
 */
function aureon_get_editor_color_palette() {
	$global_colors = aureon_get_option( 'global_colors' );
	$editor_palette = array();
	$static_colors = false;

	if ( apply_filters( 'aureon_color_palette_use_static_colors', false ) ) {
		$static_colors = true;
	}
 ...
 * @since 3.1.0
 */
function aureon_get_global_colors() {
	$global_colors = aureon_get_option( 'global_colors' );
	$colors = array();
 ...
 * @since 3.1.0
 */
function aureon_get_system_default_font() {
	return apply_filters( 'aureon_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
}
 ...
 * @since 3.1.0
 */
function aureon_has_active_menu() {
	$has_active_menu = true;

	if ( ! aureon_get_navigation_location() ) {
		$has_active_menu = false;
	}

	return apply_filters( 'aureon_has_active_menu', $has_active_menu );
}
 ...
 * @since 3.1.0
 */
function aureon_is_using_dynamic_typography() {
	return aureon_get_option( 'use_dynamic_typography' );
}
 ...
 */
function aureon_add_inline_script( $handle, $data, $var, $position = 'before' ) {
	if ( ! empty( $data ) ) {
