...
add_action( 'after_setup_theme', 'aureon_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 * @since 1.3.47
 */
function aureon_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'aureon_construct_sidebars' );
}
 ...
 * @since 3.2.0
 */
function aureon_get_woocommerce_wrapper_tagname() {
	echo is_singular()
		? 'article'
		: 'div';
}

if ( ! function_exists( 'aureon_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'aureon_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 * @since 1.3.22
	 */
	function aureon_woocommerce_start() {
		?>
		<div <?php aureon_do_attr( 'content' ); ?>>
			<main <?php aureon_do_attr( 'main' ); ?>>
				...
				?>
				<<?php aureon_get_woocommerce_wrapper_tagname(); ?> <?php aureon_do_attr( 'woocommerce-content' ); ?>>
...
if ( ! function_exists( 'aureon_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'aureon_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 * @since 1.3.22
	 */
	function aureon_woocommerce_end() {
		?>
						</div>
						<?php
						/**
				...
				</<?php aureon_get_woocommerce_wrapper_tagname(); ?>>
				<?php
				/**
...
if ( ! function_exists( 'aureon_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'aureon_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 * @since 1.3.45
	 */
	function aureon_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}
...
if ( ! function_exists( 'aureon_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'aureon_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 * @since 1.3.45
	 */
	function aureon_bbpress_css() {
...
if ( ! function_exists( 'aureon_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'aureon_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 * @since 1.3.45
	 */
	function aureon_buddypress_css() {
...
if ( ! function_exists( 'aureon_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'aureon_beaver_builder_css', 100 );
	/**
	 ...
	 * @since 1.3.45
	 */
	function aureon_beaver_builder_css() {
		...
		if ( in_array( 'fl-builder', $body_classes ) && ! in_array( 'full-width-content', $body_classes ) && ! in_array( 'contained-content', $body_classes ) ) {
			...
			if ( $post_date < $compare_date ) {
				...
				$css = str_replace( array( "\r", "\n", "\t" ), '', $css );
				wp_add_inline_style( 'aureon-style', $css );
			}
		}
	}
}

add_action( 'wp_enqueue_scripts', 'aureon_do_third_party_plugin_css', 50 );
/**
 * Add CSS for third-party plugins.
 *
 * @since 3.0.1
 */
function aureon_do_third_party_plugin_css() {
	$css = new Aureon_CSS();

	if ( aureon_is_using_flexbox() && class_exists( 'Elementor\Plugin' ) ) {
		$css->set_selector( '.elementor-template-full-width .site-content' );
		$css->add_property( 'display', 'block' );
	}

	if ( $css->css_output() ) {
		wp_add_inline_style( 'aureon-style', $css->css_output() );
	}
}

add_action( 'wp_enqueue_scripts', 'aureon_do_pro_compatibility', 50 );
/**
 * Add CSS to ensure compatibility with Aureon Studio.
 *
 * @since 3.0.0
 */
function aureon_do_pro_compatibility() {
	...
	if ( version_compare( AUREON_STUDIO_VERSION, '1.11.0-alpha.1', '<' ) ) {
		if ( aureon_is_using_flexbox() && defined( 'AUREON_SECONDARY_NAV_VERSION' ) ) {
	...
	if ( version_compare( AUREON_STUDIO_VERSION, '1.12.0-alpha.1', '<' ) ) {
		if ( defined( 'AUREON_MENU_PLUS_VERSION' ) && function_exists( 'aureon_menu_plus_get_defaults' ) ) {
			$menu_plus_settings = wp_parse_args(
				get_option( 'aureon_menu_plus_settings', array() ),
				aureon_menu_plus_get_defaults()
			);

			if ( aureon_is_using_flexbox() && ( 'true' === $menu_plus_settings['sticky_menu'] || 'desktop' === $menu_plus_settings['sticky_menu'] || 'mobile' === $menu_plus_settings['sticky_menu'] || 'enable' === $menu_plus_settings['mobile_header_sticky'] ) ) {
			...
			if ( aureon_has_inline_mobile_toggle() && 'false' !== $menu_plus_settings['slideout_menu'] ) {
			...
			if ( 'font' === aureon_get_option( 'icons' ) ) {
				...
				$css->add_property( 'font-family', 'Aureon' );
			...
			if ( aureon_is_using_flexbox() && $menu_plus_settings['navigation_as_header'] ) {
				$content_left = 40;
				$content_right = 40;
		...
		if ( defined( 'AUREON_SECONDARY_NAV_VERSION' ) ) {
			if ( aureon_is_using_flexbox() && has_nav_menu( 'secondary' ) ) {
				if ( 'text' === aureon_get_option( 'container_alignment' ) && function_exists( 'aureon_secondary_nav_get_defaults' ) ) {
		...
		if ( aureon_is_using_flexbox() && defined( 'AUREON_FONT_VERSION' ) && function_exists( 'aureon_get_default_fonts' ) ) {
			...
			if ( isset( $font_settings['tablet_navigation_font_size'] ) && '' !== $font_settings['tablet_navigation_font_size'] ) {
				$css->start_media_query( aureon_get_media_query( 'tablet' ) );
		...
		if ( ! aureon_show_title() ) {
			...
			if ( is_single() ) {
				if ( ! apply_filters( 'aureon_post_author', true ) && ! apply_filters( 'aureon_post_date', true ) ) {
...
add_filter( 'aureon_menu_item_dropdown_arrow_direction', 'aureon_set_pro_menu_item_arrow_directions', 10, 3 );
/**
 ...
 */
function aureon_set_pro_menu_item_arrow_directions( $arrow_direction, $args, $depth ) {
	if ( function_exists( 'aureon_secondary_nav_get_defaults' ) && 'secondary' === $args->theme_location ) {
...
add_filter( 'aureon_menu_plus_option_defaults', 'aureon_set_menu_plus_compat_defaults' );
/**
 ...
 */
function aureon_set_menu_plus_compat_defaults( $defaults ) {
	if ( aureon_has_inline_mobile_toggle() ) {
...
add_filter( 'aureon_spacing_option_defaults', 'aureon_set_spacing_compat_defaults', 20 );
/**
 ...
 * @param array $defaults The existing defaults.
 */
function aureon_set_spacing_compat_defaults( $defaults ) {
...
add_filter( 'aureon_page_hero_css_output', 'aureon_do_pro_page_hero_css', 10, 2 );
/**
 ...
 */
function aureon_do_pro_page_hero_css( $css_output, $options ) {
	if ( ! defined( 'AUREON_STUDIO_VERSION' ) ) {
		return $css_output;
	}
	...
	if ( version_compare( AUREON_STUDIO_VERSION, '1.12.0-alpha.1', '<' ) ) {
		...
		if ( aureon_is_using_flexbox() && '' !== $options['site_header_merge'] ) {
...
add_action( 'customize_register', 'aureon_pro_compat_customize_register', 100 );
/**
 ...
 */
function aureon_pro_compat_customize_register( $wp_customize ) {
	if ( ! defined( 'AUREON_STUDIO_VERSION' ) ) {
		return;
	}

	if ( version_compare( AUREON_STUDIO_VERSION, '1.12.0-alpha.1', '<' ) ) {
		if ( $wp_customize->get_setting( 'aureon_spacing_settings[separator]' ) ) {
			$wp_customize->get_setting( 'aureon_spacing_settings[separator]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'aureon_spacing_settings[right_sidebar_width]' ) ) {
			$wp_customize->get_setting( 'aureon_spacing_settings[right_sidebar_width]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'aureon_spacing_settings[left_sidebar_width]' ) ) {
			$wp_customize->get_setting( 'aureon_spacing_settings[left_sidebar_width]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'aureon_spacing_settings[footer_widget_container_top]' ) ) {
			$wp_customize->get_setting( 'aureon_spacing_settings[footer_widget_container_top]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'aureon_spacing_settings[footer_widget_container_right]' ) ) {
			$wp_customize->get_setting( 'aureon_spacing_settings[footer_widget_container_right]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'aureon_spacing_settings[footer_widget_container_bottom]' ) ) {
			$wp_customize->get_setting( 'aureon_spacing_settings[footer_widget_container_bottom]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'aureon_spacing_settings[footer_widget_container_left]' ) ) {
			$wp_customize->get_setting( 'aureon_spacing_settings[footer_widget_container_left]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'aureon_spacing_settings[mobile_footer_widget_container_top]' ) ) {
			$wp_customize->get_setting( 'aureon_spacing_settings[mobile_footer_widget_container_top]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'aureon_spacing_settings[mobile_footer_widget_container_right]' ) ) {
			$wp_customize->get_setting( 'aureon_spacing_settings[mobile_footer_widget_container_right]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'aureon_spacing_settings[mobile_footer_widget_container_bottom]' ) ) {
			$wp_customize->get_setting( 'aureon_spacing_settings[mobile_footer_widget_container_bottom]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'aureon_spacing_settings[mobile_footer_widget_container_left]' ) ) {
			$wp_customize->get_setting( 'aureon_spacing_settings[mobile_footer_widget_container_left]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'aureon_spacing_settings[footer_top]' ) ) {
			$wp_customize->get_setting( 'aureon_spacing_settings[footer_top]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'aureon_spacing_settings[footer_right]' ) ) {
			$wp_customize->get_setting( 'aureon_spacing_settings[footer_right]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'aureon_spacing_settings[footer_bottom]' ) ) {
			$wp_customize->get_setting( 'aureon_spacing_settings[footer_bottom]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'aureon_spacing_settings[footer_left]' ) ) {
			$wp_customize->get_setting( 'aureon_spacing_settings[footer_left]' )->transport = 'refresh';
		}
	}

	if ( $wp_customize->get_panel( 'aureon_typography_panel' ) ) {
		$wp_customize->get_panel( 'aureon_typography_panel' )->active_callback = function() {
...
add_action( 'wp', 'aureon_do_pro_compatibility_setup' );
/**
 * Do basic compatibility with Aureon Studio versions.
 *
 * @since 3.0.0
 */
function aureon_do_pro_compatibility_setup() {
	if ( ! defined( 'AUREON_STUDIO_VERSION' ) ) {
		return;
	}
	...
	if ( version_compare( AUREON_STUDIO_VERSION, '1.12.0-alpha.1', '<' ) ) {
		...
		if ( function_exists( 'aureon_premium_do_elements' ) && ! is_singular() ) {
			add_filter( 'aureon_show_title', '__return_true', 20 );
		}
	}

	if ( aureon_is_using_dynamic_typography() ) {
		remove_action( 'wp_enqueue_scripts', 'aureon_enqueue_google_fonts', 0 );
		remove_action( 'wp_enqueue_scripts', 'aureon_typography_premium_css', 100 );
		remove_filter( 'aureon_external_dynamic_css_output', 'aureon_typography_add_to_external_stylesheet' );
	}
}
 ...
 */
function aureon_do_pro_active_menus( $has_active_menu ) {
	if ( ! defined( 'AUREON_STUDIO_VERSION' ) ) {
		return $has_active_menu;
	}

	if ( version_compare( AUREON_STUDIO_VERSION, '2.1.0-alpha.1', '<' ) ) {
		...
		if ( function_exists( 'aureon_secondary_nav_get_defaults' ) && has_nav_menu( 'secondary' ) ) {
			$has_active_menu = true;
		}
	}
...
add_action( 'init', 'aureon_do_customizer_compatibility_setup' );
/**
 * Make changes to the Customizer in the Pro version.
 */
function aureon_do_customizer_compatibility_setup() {
	if ( ! defined( 'AUREON_STUDIO_VERSION' ) ) {
		return;
	}
	...
	if ( version_compare( AUREON_STUDIO_VERSION, '2.1.0-alpha.1', '<' ) ) {
		if ( aureon_is_using_dynamic_typography() ) {
			remove_action( 'customize_register', 'aureon_fonts_customize_register' );
			remove_action( 'customize_preview_init', 'aureon_typography_customizer_live_preview' );
		}

		remove_action( 'customize_register', 'aureon_colors_customize_register' );
		remove_action( 'customize_preview_init', 'aureon_colors_customizer_live_preview' );
		remove_action( 'customize_controls_enqueue_scripts', 'aureon_enqueue_color_palettes', 1001 );
		remove_action( 'customize_register', 'aureon_colors_secondary_nav_customizer', 1000 );
		remove_action( 'customize_register', 'aureon_slideout_navigation_color_controls', 150 );
		remove_action( 'customize_register', 'aureon_colors_wc_customizer', 100 );
	}
}
