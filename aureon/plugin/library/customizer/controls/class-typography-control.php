...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Aureon_Pro_Typography_Customize_Control' ) ) :
class Aureon_Pro_Typography_Customize_Control extends WP_Customize_Control
{
    public $type = 'aureon-pro-customizer-typography';

	public function enqueue() {
		wp_enqueue_script( 'aureon-pro-typography-selectWoo', trailingslashit( plugin_dir_url( __FILE__ ) )  . 'js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), AUREON_STUDIO_VERSION, true );
		wp_enqueue_style( 'aureon-pro-typography-selectWoo', trailingslashit( plugin_dir_url( __FILE__ ) ) . 'css/selectWoo.min.css', array(), AUREON_STUDIO_VERSION );

		wp_enqueue_script( 'aureon-pro-typography-customizer', trailingslashit( plugin_dir_url( __FILE__ ) )  . 'js/typography-customizer.js', array( 'customize-controls', 'aureon-pro-typography-selectWoo' ), AUREON_STUDIO_VERSION, true );

		wp_enqueue_style( 'aureon-pro-typography-customizer', trailingslashit( plugin_dir_url( __FILE__ ) ) . 'css/typography-customizer.css', array(), AUREON_STUDIO_VERSION );
	}

	public function to_json() {
		parent::to_json();

		$this->json[ 'default_fonts_title'] = __( 'System Fonts', 'aureon-studio' );
		$this->json[ 'google_fonts_title'] = __( 'Google Fonts', 'aureon-studio' );
		$this->json[ 'default_fonts' ] = aureon_typography_default_fonts();
		$this->json[ 'family_title' ] = esc_html__( 'Font family', 'aureon-studio' );
		$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'aureon-studio' );
		$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'aureon-studio' );
		$this->json[ 'category_title' ] = '';
		$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'aureon-studio' );
	...
	public function content_template() {
		...
		<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
			...
			#>
			<div id={{{ data.variant.id }}}" class="aureon-font-variant" data-saved-value="{{ data.variant.value }}">
				<label>
		...
		<# if ( 'undefined' !== typeof ( data.category ) ) { #>
			<div class="aureon-font-category">
				<label>
						<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="aureon-hidden-input" />
		...
		<div class="aureon-weight-transform-wrapper">
			<# if ( 'undefined' !== typeof ( data.weight ) ) { #>
				<div class="aureon-font-weight">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="aureon-font-transform">
					<label>
	...
	public function get_font_weight_choices() {
		return array(
			'' => esc_html__( 'inherit', 'aureon-studio' ),
			'normal' => esc_html__( 'normal', 'aureon-studio' ),
			'bold' => esc_html__( 'bold', 'aureon-studio' ),
			'100' => esc_html( '100' ),
			'200' => esc_html( '200' ),
			'300' => esc_html( '300' ),
			'400' => esc_html( '400' ),
	...
	public function get_font_transform_choices() {
		return array(
			...
			'capitalize' => esc_html__( 'capitalize', 'aureon-studio' ),
			'uppercase' => esc_html__( 'uppercase', 'aureon-studio' ),
			'lowercase' => esc_html__( 'lowercase', 'aureon-studio' ),
		);
	}
}
endif;
