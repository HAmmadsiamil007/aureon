...
class Aureon_Pro_Rest extends WP_REST_Controller {
	...
	public function update_licensing( WP_REST_Request $request ) {
		...
		$old_license = get_option( 'aureon_studio_license_key', '' );
		$old_status = get_option( 'aureon_studio_license_key_status', 'deactivated' );
		...
		if ( $new_license ) {
			$api_params = array(
				...
				'item_name'  => rawurlencode( 'Aureon Studio' ),
				'url'        => home_url(),
			);
		...
		if ( isset( $api_params ) ) {
				...
				if ( false === $license_data->success ) {
					switch ( $license_data->error ) {
						...
						case 'item_name_mismatch':
							/* translators: Aureon Studio */
							$message = sprintf( __( 'This appears to be an invalid license key for %s.', 'aureon-studio' ), __( 'Aureon Studio', 'aureon-studio' ) );
							break;
			...
			update_option( 'aureon_studio_license_key_status', esc_attr( $license_data->license ) );
		}

		update_option( 'aureon_studio_license_key', sanitize_key( $new_license ) );

		if ( ! isset( $api_params ) ) {
			return $this->success( __( 'Settings saved.', 'aureon-studio' ) );
		}
