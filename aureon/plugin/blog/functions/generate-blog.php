...
if ( ! function_exists( 'aureon_blog_scripts' ) ) {
	add_action( 'wp_enqueue_scripts', 'aureon_blog_scripts', 50 );
	/**
	 * Enqueue scripts and styles
	 */
	function aureon_blog_scripts() {
		...
		wp_add_inline_style( 'aureon-style', aureon_blog_css() );
		wp_add_inline_style( 'aureon-style', aureon_blog_columns_css() );

		$deps = array();

		if ( 'true' == aureon_blog_get_masonry() && aureon_blog_get_columns() ) {
			$deps[] = 'masonry';
			$deps[] = 'imagesloaded';
		}
		...
		if ( ( 'true' == aureon_blog_get_masonry() && aureon_blog_get_columns() ) || ( $settings[ 'infinite_scroll' ] && ! is_singular() && ! is_404() && ! is_post_type_archive( 'product' ) ) ) {
			wp_enqueue_script( 'aureon-blog', plugin_dir_url( __FILE__ ) . "js/scripts{$suffix}.js", $deps, AUREON_BLOG_VERSION, true );
			...
			wp_localize_script(
				...
				array(
					...
					'icon' => function_exists( 'aureon_get_svg_icon' ) ? aureon_get_svg_icon( 'spinner' ) : '',
					'masonryInit' => apply_filters(
						'aureon_blog_masonry_init',
						array(
					...
					'infiniteScrollInit' => apply_filters(
						'aureon_blog_infinite_scroll_init',
						array(
		...
		if ( aureon_blog_get_columns() || $settings['infinite_scroll'] ) {
		...
		if ( $needs_columns_css && $needs_featured_image_css ) {
			wp_enqueue_style( 'aureon-blog', plugin_dir_url( __FILE__ ) . "css/style{$suffix}.css", array(), AUREON_BLOG_VERSION );
		} elseif ( $needs_columns_css ) {
			wp_enqueue_style( 'aureon-blog-columns', plugin_dir_url( __FILE__ ) . "css/columns{$suffix}.css", array(), AUREON_BLOG_VERSION );
		} elseif ( $needs_featured_image_css ) {
			wp_enqueue_style( 'aureon-blog-images', plugin_dir_url( __FILE__ ) . "css/featured-images{$suffix}.css", array(), AUREON_BLOG_VERSION );
		}
	}
}

if ( ! function_exists( 'aureon_blog_post_classes' ) ) {
	add_filter( 'post_class', 'aureon_blog_post_classes' );
	/**
	 * Adds custom classes to the content container
	 *
	 * @since 0.1
	 */
	function aureon_blog_post_classes( $classes ) {
		...
		if ( aureon_blog_get_columns() && ! is_singular() ) {
			$classes[] = 'aureon-columns';
			...
			if ( $wp_query->current_post == 0 && $paged == 1 && $settings['featured_column'] ) {
				if ( 50 == aureon_blog_get_column_count() ) {
					$classes[] = 'grid-100';
				}

				if ( 33 == aureon_blog_get_column_count() ) {
					$classes[] = 'grid-66';
				}

				if ( 25 == aureon_blog_get_column_count() ) {
					$classes[] = 'grid-50';
				}

				if ( 20 == aureon_blog_get_column_count() ) {
					$classes[] = 'grid-60';
				}
				$classes[] = 'featured-column';
			} else {
				$classes[] = 'grid-' . aureon_blog_get_column_count();
			}
		}
...
if ( ! function_exists( 'aureon_blog_body_classes' ) ) {
	add_filter( 'body_class', 'aureon_blog_body_classes' );
	/**
	 * Adds custom classes to the body
	 *
	 * @since 0.1
	 */
	function aureon_blog_body_classes( $classes ) {
		// Get theme options
		$settings = wp_parse_args(
			get_option( 'aureon_blog_settings', array() ),
			aureon_blog_get_defaults()
		...
		if ( aureon_blog_get_columns() && ! is_singular() ) {
			$classes[] = 'aureon-columns-activated';
		}
...
if ( ! function_exists( 'aureon_excerpt_length' ) ) {
	add_filter( 'excerpt_length', 'aureon_excerpt_length', 15 );
	/**
	 * Set our excerpt length
	 *
	 * @since 0.1
	 */
	function aureon_excerpt_length( $length ) {
		$aureon_settings = wp_parse_args(
			get_option( 'aureon_blog_settings', array() ),
			aureon_blog_get_defaults()
		);
		return absint( apply_filters( 'aureon_excerpt_length', $aureon_settings['excerpt_length'] ) );
	}
}

if ( ! function_exists( 'aureon_blog_css' ) ) {
	/**
	 * Build our inline CSS
	 *
	 * @since 0.1
	 */
	function aureon_blog_css() {
		global $post;
		$return = '';
		...
		if ( version_compare( AUREON_VERSION, '3.0.0-alpha.1', '<' ) ) {
			// Get disable headline meta.
			$disable_headline = ( isset( $post ) ) ? get_post_meta( $post->ID, '_aureon-disable-headline', true ) : '';
		...
		if ( ! $settings['page_post_image_padding'] || ! $settings['single_post_image_padding'] || ! $settings['post_image_padding'] ) {
			$return .= '@media ' . aureon_premium_get_media_query( 'mobile' ) . '{';
				if ( ! $settings['post_image_padding'] && 'post-image-aligned-center' == $settings['post_image_alignment'] && ! is_singular() ) {
...
add_filter( 'aureon_excerpt_more_text', 'aureon_blog_set_read_more_text' );
/**
 * Set the read more text with our Customizer setting.
 *
 * @param string $text The read more text.
 */
function aureon_blog_set_read_more_text( $text ) {
	$settings = wp_parse_args(
		get_option( 'aureon_blog_settings', array() ),
		aureon_blog_get_defaults()
	);
...
if ( ! function_exists( 'aureon_blog_excerpt_more' ) ) {
	add_filter( 'excerpt_more', 'aureon_blog_excerpt_more', 15 );
	/**
	 * Prints the read more HTML
	 */
	function aureon_blog_excerpt_more( $more ) {
		$aureon_settings = wp_parse_args(
			get_option( 'aureon_blog_settings', array() ),
			aureon_blog_get_defaults()
		);

		if ( '' == $aureon_settings['read_more'] ) {
			return '';
		}

		// We don't need to overwrite the entire element just to change its text.
		// If we can filter the text, stop here.
		if ( function_exists( 'aureon_get_read_more_text' ) ) {
			return $more;
		}
		...
		return apply_filters(
			...
			sprintf(
				...
				wp_kses_post( $aureon_settings['read_more'] ),
				sprintf(
					/* translators: Aria-label describing the read more button */
					_x( 'More on %s', 'more on post title', 'aureon-studio' ),
					the_title_attribute( 'echo=0' )
				)
			)
		);
...
if ( ! function_exists( 'aureon_blog_content_more' ) ) {
	add_filter( 'the_content_more_link', 'aureon_blog_content_more', 15 );
	/**
	 * Prints the read more HTML
	 */
	function aureon_blog_content_more( $more ) {
		$aureon_settings = wp_parse_args(
			get_option( 'aureon_blog_settings', array() ),
			aureon_blog_get_defaults()
		);
 ...
 */
function aureon_disable_post_thing( $data, $thing ) {
	$aureon_blog_settings = wp_parse_args(
		get_option( 'aureon_blog_settings', array() ),
		aureon_blog_get_defaults()
	);

	if ( ! $aureon_blog_settings[$thing] ) {
		return false;
	}
...
if ( ! function_exists( 'aureon_disable_post_date' ) ) {
	add_filter( 'aureon_post_date', 'aureon_disable_post_date' );
	/**
	 * Remove the post date if set
	 *
	 * @since 0.1
	 */
	function aureon_disable_post_date( $date ) {
		if ( is_singular() ) {
			return aureon_disable_post_thing( $date, 'single_date' );
		} else {
			return aureon_disable_post_thing( $date, 'date' );
		}
	}
}

if ( ! function_exists( 'aureon_disable_post_author' ) ) {
	add_filter( 'aureon_post_author', 'aureon_disable_post_author' );
	/**
	 * Set the author if set
	 *
	 * @since 0.1
	 */
	function aureon_disable_post_author( $author ) {
		if ( is_singular() ) {
			return aureon_disable_post_thing( $author, 'single_author' );
		} else {
			return aureon_disable_post_thing( $author, 'author' );
		}
	}
}

if ( ! function_exists( 'aureon_disable_post_categories' ) ) {
	add_filter( 'aureon_show_categories', 'aureon_disable_post_categories' );
	/**
	 * Remove the categories if set
	 *
	 * @since 0.1
	 */
	function aureon_disable_post_categories( $categories ) {
		if ( is_singular() ) {
			return aureon_disable_post_thing( $categories, 'single_categories' );
		} else {
			return aureon_disable_post_thing( $categories, 'categories' );
		}
	}
}

if ( ! function_exists( 'aureon_disable_post_tags' ) ) {
	add_filter( 'aureon_show_tags', 'aureon_disable_post_tags' );
	/**
	 * Remove the tags if set
	 *
	 * @since 0.1
	 */
	function aureon_disable_post_tags( $tags ) {
		if ( is_singular() ) {
			return aureon_disable_post_thing( $tags, 'single_tags' );
		} else {
			return aureon_disable_post_thing( $tags, 'tags' );
		}
	}
}

if ( ! function_exists( 'aureon_disable_post_comments_link' ) ) {
	add_filter( 'aureon_show_comments', 'aureon_disable_post_comments_link' );
	/**
	 * Remove the link to comments if set
	 *
	 * @since 0.1
	 */
	function aureon_disable_post_comments_link( $comments_link ) {
		return aureon_disable_post_thing( $comments_link, 'comments' );
	}
}

add_filter( 'aureon_show_post_navigation', 'aureon_disable_post_navigation' );
/**
 * Remove the single post navigation
 *
 * @since 1.5
 */
function aureon_disable_post_navigation( $navigation ) {
	if ( is_singular() ) {
		return aureon_disable_post_thing( $navigation, 'single_post_navigation' );
	} else {
		return $navigation;
	}
}

add_filter( 'aureon_excerpt_more_output', 'aureon_blog_read_more_button' );
add_filter( 'aureon_content_more_link_output', 'aureon_blog_read_more_button' );
/**
 ...
 */
function aureon_blog_read_more_button( $output ) {
	$settings = wp_parse_args(
		get_option( 'aureon_blog_settings', array() ),
		aureon_blog_get_defaults()
	);
	...
	$aria_label = function_exists( 'aureon_get_read_more_aria_label' )
	...
	return sprintf(
		...
		$aria_label,
		'aureon_excerpt_more_output' === current_filter() ? ' ... ' : ''
	);
}

if ( ! function_exists( 'aureon_blog_load_more' ) ) {
	add_action( 'aureon_after_main_content', 'aureon_blog_load_more', 20 );
	/**
	 * Build our load more button
	 */
	function aureon_blog_load_more() {
		// Get theme options
		$settings = wp_parse_args(
			get_option( 'aureon_blog_settings', array() ),
			aureon_blog_get_defaults()
		...
		if ( function_exists( 'aureon_get_svg_icon' ) ) {
			$icon = aureon_get_svg_icon( 'spinner' );
		}
		...
		printf(
			...
			$icon ? 'has-svg-icon' : '',
			( 'true' == aureon_blog_get_masonry() && aureon_blog_get_columns() ) ? 'are-images-unloaded' : '',
			$icon,
 ...
 */
function aureon_blog_get_singular_template() {
	$template = 'single';
...
add_action( 'aureon_after_footer', 'aureon_blog_do_infinite_scroll_path', 500 );
/**
 * Add a next page of posts link for infinite scroll.
 *
 * @since 2.0.0
 */
function aureon_blog_do_infinite_scroll_path() {
