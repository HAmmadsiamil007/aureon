 ...
 */
class Aureon_Site_Library_Helper {
	...
	public static function get_mapped_term_ids() {
		return (array) get_transient( 'aureon_sites_mapped_term_ids' );
	}
	...
	public static function get_mapped_post_ids( $slug ) {
		return (array) get_option( "aureon_sites_mapped_ids_{$slug}", array() );
	}
	...
	public static function clear_widgets() {
		...
		update_option( '_aureon_site_library_backup', $backup_data );
	...
	public static function do_options_exist() {
		...
		foreach ( $settings as $setting ) {
			if ( get_option( $setting ) ) {
				...
				if ( 'aureon_blog_settings' === $setting && function_exists( 'aureon_blog_get_defaults' ) ) {
	...
	public static function import_xml( $path, $slug ) {
		...
		require_once AUREON_STUDIO_DIR_PATH . 'site-library/libs/wxr-importer/WXRImporter.php';
		require_once AUREON_STUDIO_DIR_PATH . 'site-library/libs/wxr-importer/WPImporterLogger.php';
		require_once AUREON_STUDIO_DIR_PATH . 'site-library/libs/wxr-importer/WXRImportInfo.php';
		require_once AUREON_STUDIO_DIR_PATH . 'site-library/classes/class-content-importer.php';
		...
		$logger   = new Aureon\WPContentImporter2\WPImporterLogger();
		$importer = new Aureon_Sites_Content_Importer( $options );
		...
		update_option( 'aureon_sites_mapped_ids_' . $slug, $all_data, false );
		...
		set_transient( 'aureon_sites_mapped_term_ids', $term_mapping, 0.1 * HOUR_IN_SECONDS );
	...
	public static function set_element_locations( $locations, $slug ) {
		...
		if ( isset( $locations ) && ! empty( $locations ) ) {
			...
			foreach ( (array) $locations as $key => $value ) {
				...
				update_post_meta( $post_ids[ $key ], '_aureon_element_display_conditions', $new_locations );
			}
		}
	}
	...
	public static function set_element_exclusions( $locations, $slug ) {
		...
		if ( isset( $locations ) && ! empty( $locations ) ) {
			...
			foreach ( (array) $locations as $key => $value ) {
				...
				update_post_meta( $post_ids[ $key ], '_aureon_element_exclude_conditions', $new_locations );
			}
		}
	}
	...
	public static function update_page_builder_content() {
		...
		if ( is_plugin_active( 'beaver-builder-lite-version/fl-builder.php' ) || is_plugin_active( 'bb-plugin/fl-builder.php' ) ) {
			require_once AUREON_STUDIO_DIR_PATH . 'site-library/classes/class-beaver-builder-batch-processing.php';
			require_once AUREON_STUDIO_DIR_PATH . 'site-library/classes/class-site-import-image.php';

			$beaver_builder = new Aureon_Sites_Process_Beaver_Builder();
	...
	public static function premium_modules() {
		return array(
			...
			'Font Library' => 'aureon_package_font_library',
		);
	}
	...
	public static function track_post( $post_id ) {
		update_post_meta( $post_id, '_aureon_sites_imported_post', true );
	}
	...
	public static function track_term( $term_id ) {
		...
		update_term_meta( $term_id, '_aureon_sites_imported_term', true );
	}

	/**
	 * Prevent WooCommerce from creating new pages if we're importing them.
	 * aureon_woocommerce_no_create_pages is set during Site Library import.
	 */
	public static function woocommerce_no_new_pages() {
		...
		if ( empty( $woocommerce_version ) ) {
			$no_new_pages = get_option( 'aureon_woocommerce_no_create_pages', false );

			if ( $no_new_pages ) {
				add_filter( 'woocommerce_create_pages', '__return_empty_array' );
				delete_option( 'aureon_woocommerce_no_create_pages' );
			}
		}
	}
}
