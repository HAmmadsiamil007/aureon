 ...
 * @since 1.6
 */
class Aureon_Sites_Image_Importer {
	...
	private function get_saved_image( $attachment ) {
		...
		if ( isset( $this->already_imported_ids[ $attachment['id'] ] ) ) {
			Aureon_Site_Library_Helper::log( 'Successfully replaced: ' . $attachment['url'] );
		...
		$post_id = $wpdb->get_var(
			$wpdb->prepare(
				'SELECT `post_id` FROM `' . $wpdb->postmeta . '`
					WHERE `meta_key` = \'_aureon_sites_image_hash\'
						AND `meta_value` = %s
				;',
	...
	public function import( $attachment ) {
		...
		if ( empty( $file_content ) ) {
			Aureon_Site_Library_Helper::log( 'Failed to replace: ' . $attachment['url'] );
			Aureon_Site_Library_Helper::log( 'Error: Failed wp_remote_retrieve_body().' );
		...
		wp_update_attachment_metadata(
			$post_id,
			wp_aureon_attachment_metadata( $post_id, $upload['file'] )
		);

		update_post_meta( $post_id, '_aureon_sites_image_hash', $this->get_hash_image( $attachment['url'] ) );
