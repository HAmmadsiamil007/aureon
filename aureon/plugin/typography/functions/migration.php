...
if ( ! function_exists( 'aureon_typography_convert_values' ) ) {
	add_action( 'admin_init', 'aureon_typography_convert_values' );
	/**
	...
	function aureon_typography_convert_values() {
		...
		if ( 'true' == get_option( 'aureon_update_premium_typography' ) ) {
			return;
		}
		...
		if ( ! empty( $new_settings ) ) {
			$aureon_new_typography_settings = wp_parse_args( $new_settings, $aureon_settings );
			update_option( 'aureon_settings', $aureon_new_typography_settings );
		}

		// All done, set an option so we don't have to do this again
		update_option( 'aureon_update_premium_typography', 'true' );
	}
}

if ( ! function_exists( 'aureon_typography_convert_secondary_nav_values' ) ) {
	add_action( 'admin_init', 'aureon_typography_convert_secondary_nav_values' );
	/**
	 ...
	 * @since 1.3.0
	 */
	function aureon_typography_convert_secondary_nav_values() {
		...
		if ( 'true' == get_option( 'aureon_update_secondary_typography' ) ) {
			return;
		}
		...
		$value = $aureon_secondary_nav_settings[ 'font_secondary_navigation' ];
		...
		if ( ! empty( $new_settings ) ) {
			$aureon_new_typography_settings = wp_parse_args( $new_settings, $aureon_secondary_nav_settings );
			update_option( 'aureon_secondary_nav_settings', $aureon_new_typography_settings );
		}

		// All done, set an option so we don't go through this again
		update_option( 'aureon_update_secondary_typography','true' );
	}
}
...
if ( ! function_exists( 'aureon_typography_set_font_data' ) ) {
	...
	function aureon_typography_set_font_data() {
		...
		foreach( $font_settings as $setting ) {
			...
			if ( function_exists( 'aureon_secondary_nav_get_defaults' ) && 'font_secondary_navigation' == $setting ) {
				$aureon_settings = wp_parse_args(
					get_option( 'aureon_secondary_nav_settings', array() ),
					aureon_secondary_nav_get_defaults()
				);
			...
			if ( $defaults[ $setting ] == $aureon_settings[ $setting ] ) {
				continue;
			}

			// Don't need to continue if we're using a system font or our default font
			if ( in_array( $aureon_settings[ $setting ], aureon_typography_default_fonts() ) || 'Open Sans' == $aureon_settings[ $setting ] ) {
				continue;
			}
