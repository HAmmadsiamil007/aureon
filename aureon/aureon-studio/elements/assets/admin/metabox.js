jQuery( function( $ ) {
	if ( $( '.element-settings' ).hasClass( 'header' ) || $( '.element-settings' ).hasClass( 'hook' ) ) {
		$( function() {
			if ( elements.settings ) {
				wp.codeEditor.initialize( 'aureon-element-content', elements.settings );
			}
		} );
	}

	$( '#_aureon_block_type' ).on( 'change', function() {
		var _this = $( this ).val();
	...
	$( '#_aureon_hook' ).on( 'change', function() {
		var _this = $( this );
		...
		if ( 'aureon_header' === _this.val() ) {
			$( '.disable-header-hook' ).show();
		}

		if ( 'aureon_footer' === _this.val() ) {
	...
	$( '.element-metabox-tabs li' ).on( 'click', function() {
		...
		$( '.aureon-elements-settings' ).hide();
		$( '.aureon-elements-settings[data-tab="' + tab + '"]' ).show();

		if ( $( '.element-settings' ).hasClass( 'block' ) && 'hook-settings' === tab ) {
			$( '.aureon-elements-settings[data-tab="display-rules"]' ).show();
		}

		if ( $( '.element-settings' ).hasClass( 'header' ) ) {
			if ( 'hero' !== tab ) {
				$( '#aureon-element-content' ).next( '.CodeMirror' ).removeClass( 'aureon-studio-elements-show-codemirror' );
				$( '#aureon_page_hero_template_tags' ).css( 'display', '' );
			} else {
				$( '#aureon-element-content' ).next( '.CodeMirror' ).addClass( 'aureon-studio-elements-show-codemirror' );
				$( '#aureon_page_hero_template_tags' ).css( 'display', 'block' );
			}
		}
	} );

	var select2Init = function() {
		var selects = $( '.aureon-element-row-content .condition:not(.hidden) select:not(.select2-init)' );
	...
	$( '.add-condition' ).on( 'click', function() {
		...
		var row = _this.closest( '.aureon-element-row-content' ).find( '.condition.hidden.screen-reader-text' ).clone( true );
		row.removeClass( 'hidden screen-reader-text' );
		row.insertBefore( _this.closest( '.aureon-element-row-content' ).find( '.condition:last' ) );
	...
	var getLocationObjects = function( _this, onload = false, data = '' ) {
		...
		if ( '' === location ) {
			parent.removeClass( 'aureon-elements-rule-objects-visible' );
			select.closest( '.aureon-element-row-content' ).find( '.aureon-element-row-loading' ).remove();
		} else {
			...
			if ( 'taxonomy' === locationType || 'post' === locationType ) {
				if ( ! ( '.aureon-element-row-loading' ).length ) {
					select.closest( '.aureon-element-row-content' ).prepend( '<div class="aureon-element-row-loading"></div>' );
				}
				...
				var fillObjects = function( response ) {
					...
					parent.addClass( 'aureon-elements-rule-objects-visible' );
				...
				if ( data && onload ) {
					...
					$.post( ajaxurl, {
						action: 'aureon_elements_get_location_' + actionType,
						id: locationID,
	...
	$( '.aureon-elements-rule-objects-visible' ).each( function() {
		var _this = $( this ),
	...
	if ( postObjects.length > 0 || termObjects.length > 0 ) {
		$.post( ajaxurl, {
			action: 'aureon_elements_get_location_objects',
			posts: postObjects,
			terms: termObjects,
			nonce: elements.nonce,
		}, function( response ) {
			...
			$( '.aureon-elements-rule-objects-visible' ).each( function() {
				...
				$( '<div class="aureon-element-row-loading"></div>' ).insertBefore( _this );
	...
	$( '.aureon-upload-file' ).on( 'click', function() {
		if ( frame ) {
			frame.open();
			return;
		}
		...
		frame.on( 'select', function() {
			...
			if ( _this.data( 'preview' ) ) {
				container.find( '.aureon-media-preview' ).empty().append( '<img src="' + attachment.url + '" width="50" />' ).show();
			}
		} );
	...
	$( '.remove-field' ).on( 'click', function() {
		...
		container.find( '.aureon-media-preview' ).empty();
	} );

	$( '#_aureon_hero_background_image' ).on( 'change', function() {
		var _this = $( this );
	...
	$( '.responsive-controls a' ).on( 'click', function( e ) {
		...
		var _this = $( this ),
			...
			controlArea = _this.closest( '.aureon-element-row-content' );
	...
	$( '#_aureon_site_header_merge' ).on( 'change', function() {
		var _this = $( this );

		if ( '' !== _this.val() ) {
			$( '.requires-header-merge' ).show();

			if ( $( '#_aureon_navigation_colors' ).is( ':checked' ) ) {
				$( '.requires-navigation-colors' ).show();
			}

			if ( $( '#_aureon_hero_full_screen' ).is( ':checked' ) ) {
	...
	$( '#_aureon_navigation_colors' ).on( 'change', function() {
		var _this = $( this );
	...
	$( '#_aureon_hero_full_screen' ).on( 'change', function() {
		var _this = $( this );
	...
	$( '#_aureon_hero_background_parallax' ).on( 'change', function() {
		var _this = $( this );

		if ( _this.is( ':checked' ) ) {
			$( '#_aureon_hero_background_position' ).val( '' ).change();
			$( '#_aureon_hero_background_position option[value="left center"]' ).attr( 'disabled', true );
			$( '#_aureon_hero_background_position option[value="left bottom"]' ).attr( 'disabled', true );
			$( '#_aureon_hero_background_position option[value="right center"]' ).attr( 'disabled', true );
			$( '#_aureon_hero_background_position option[value="right bottom"]' ).attr( 'disabled', true );
			$( '#_aureon_hero_background_position option[value="center center"]' ).attr( 'disabled', true );
			$( '#_aureon_hero_background_position option[value="center bottom"]' ).attr( 'disabled', true );
		} else {
			$( '#_aureon_hero_background_position option[value="left center"]' ).attr( 'disabled', false );
			$( '#_aureon_hero_background_position option[value="left bottom"]' ).attr( 'disabled', false );
			$( '#_aureon_hero_background_position option[value="right center"]' ).attr( 'disabled', false );
			$( '#_aureon_hero_background_position option[value="right bottom"]' ).attr( 'disabled', false );
			$( '#_aureon_hero_background_position option[value="center center"]' ).attr( 'disabled', false );
			$( '#_aureon_hero_background_position option[value="center bottom"]' ).attr( 'disabled', false );
		}
	} );
} );
