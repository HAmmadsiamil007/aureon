...
add_action( 'wp', 'aureon_premium_do_elements' );
add_action( 'current_screen', 'aureon_premium_do_elements' );
/**
 * Execute our Elements.
 *
 * @since 1.7
 */
function aureon_premium_do_elements() {
	$args = array(
	...
	foreach ( $posts as $post_id ) {
		$post_id = apply_filters( 'aureon_element_post_id', $post_id );
		$type = get_post_meta( $post_id, '_aureon_element_type', true );

		if ( 'hook' === $type ) {
			new Aureon_Hook( $post_id );
		}

		if ( 'header' === $type && ! Aureon_Hero::$instances ) {
			new Aureon_Hero( $post_id );
		}

		if ( 'layout' === $type ) {
			new Aureon_Site_Layout( $post_id );
		}

		if ( 'block' === $type ) {
			new Aureon_Block_Element( $post_id );
		}
	}
}

add_filter( 'aureon_dashboard_tabs', 'aureon_elements_dashboard_tab' );
/**
 ...
 */
function aureon_elements_dashboard_tab( $tabs ) {
	$screen = get_current_screen();

	$tabs['Elements'] = array(
		'name' => __( 'Elements', 'aureon-studio' ),
		'url' => admin_url( 'edit.php?post_type=aureon_elements' ),
		'class' => 'edit-aureon_elements' === $screen->id ? 'active' : '',
		'id' => 'aureon-elements-tab',
	);

	return $tabs;
}

add_filter( 'aureon_dashboard_screens', 'aureon_elements_dashboard_screen' );
/**
 ...
 */
function aureon_elements_dashboard_screen( $screens ) {
	$screens[] = 'edit-aureon_elements';

	return $screens;
}

add_filter( 'aureon_element_post_id', 'aureon_elements_ignore_languages' );
/**
 ...
 * @return bool|int
 */
function aureon_elements_ignore_languages( $post_id ) {
	if ( function_exists( 'pll_get_post_language' ) && function_exists( 'pll_current_language' ) ) {
		...
		$disable = get_post_meta( $post_id, '_aureon_element_ignore_languages', true );
...
add_action( 'save_post_wp_block', 'aureon_elements_wp_block_update', 10, 2 );
/**
 ...
 */
function aureon_elements_wp_block_update( $post_id, $post ) {
	...
	if ( isset( $post->post_content ) ) {
		if ( strpos( $post->post_content, 'wp:generateblocks' ) !== false ) {
			...
			foreach ( (array) $posts as $id ) {
				$display_conditions = get_post_meta( $id, '_aureon_element_display_conditions', true );
...
add_filter( 'aureon_do_block_element_content', 'aureon_add_block_element_content_filters' );
/**
 ...
 */
function aureon_add_block_element_content_filters( $content ) {
...
add_action( 'admin_bar_menu', 'aureon_add_elements_admin_bar', 100 );
/**
 * Add the Elementd admin bar item.
 *
 * @since 2.0.0
 */
function aureon_add_elements_admin_bar() {
	...
	$count = ! empty( $aureon_elements ) ? count( $aureon_elements ) : 0;
	...
	$wp_admin_bar->add_menu(
		array(
			'id' => 'aureon_elements-menu',
			'title' => $title,
			'href' => esc_url( admin_url( 'edit.php?post_type=aureon_elements' ) ),
		)
	);

	if ( ! empty( $aureon_elements ) ) {
		// Prevent "Entire Site" Elements from being counted on non-edit pages in the admin.
		if ( is_admin() && function_exists( 'get_current_screen' ) ) {
			$screen = get_current_screen();
		...
		foreach ( (array) $aureon_elements as $key => $data ) {
			$label = Aureon_Elements_Helper::get_element_type_label( $data['type'] );

			$wp_admin_bar->add_menu(
				array(
					'id' => 'element-' . absint( $data['id'] ),
					'parent' => 'aureon_elements-menu',
