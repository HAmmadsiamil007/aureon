...
const PAGES = [
  ...
  { name: 'Collection: Outdoor', url: '/product-category/outdoor-living', type: 'collection' },
...
(async () => {
  ...
  for (const pg of PAGES) {
    ...
    try {
      const response = await page.goto(`${BASE}${pg.url}`, { waitUntil: 'networkidle', timeout: 30000 });
      ...
      for (const img of images) {
        if (img.src && img.src !== '' && !img.src.endsWith('/') && img.complete && (img.naturalWidth === 0 || img.naturalHeight === 0)) {
  ...
  fs.writeFileSync(`${RESULTS_DIR}/audit-v2-results.json`, JSON.stringify(results, null, 2));
