...
class Aureon_Dashboard {
	...
	public function __construct() {
		// Load our old dashboard if we're using an old version of Aureon Studio.
		...
		add_action( 'aureon_admin_dashboard', array( $this, 'start_customizing' ) );
		add_action( 'aureon_admin_dashboard', array( $this, 'go_pro' ), 15 );
	...
	public function add_menu_item() {
		add_theme_page(
			esc_html__( 'Aureon', 'aureon' ),
			esc_html__( 'Aureon', 'aureon' ),
			apply_filters( 'aureon_dashboard_page_capability', 'edit_theme_options' ),
			'aureon-options',
			array( $this, 'page' )
		);
	}
	...
	public static function get_pages() {
		return apply_filters(
			'aureon_dashboard_screens',
			array(
				'appearance_page_aureon-options',
			)
		);
	}
	...
	public static function header() {
		?>
		<div class="aureon-dashboard-header">
			<div class="aureon-dashboard-header__title">
				<h1>
	...
	public static function navigation() {
		...
		$tabs = apply_filters(
			'aureon_dashboard_tabs',
			array(
				'dashboard' => array(
					'name' => __( 'Dashboard', 'aureon' ),
		...
		if ( ! defined( 'AUREON_STUDIO_VERSION' ) ) {
			$tabs['premium'] = array(
				'name' => __( 'Premium', 'aureon' ),
				'url' => 'https://aureonstudio.com/premium',
				'class' => '',
				'external' => true,
			);
		}
		...
		$tabs['support'] = array(
			'name' => __( 'Support', 'aureon' ),
			'url' => 'https://aureonstudio.com/support',
			'class' => '',
			'external' => true,
		);

		$tabs['documentation'] = array(
			'name' => __( 'Documentation', 'aureon' ),
			'url' => 'https://docs.aureonstudio.com',
			'class' => '',
			'external' => true,
		);
		?>
		<div class="aureon-dashboard-header__navigation">
			<?php
			foreach ( $tabs as $tab ) {
				printf(
	...
	public function enqueue_scripts() {
		...
		if ( in_array( $current_screen->id, $dashboard_pages ) ) {
			...
			if ( 'appearance_page_aureon-options' === $current_screen->id ) {
				...
				wp_set_script_translations( 'aureon-dashboard', 'aureon' );

				wp_localize_script(
					'aureon-dashboard',
					'aureonDashboard',
					array(
						'hasPremium' => defined( 'AUREON_STUDIO_VERSION' ),
						'customizeSectionUrls' => array(
							...
							'colorsSection' => add_query_arg( rawurlencode( 'autofocus[section]' ), 'aureon_colors_section', wp_customize_url() ),
							'typographySection' => add_query_arg( rawurlencode( 'autofocus[section]' ), 'aureon_typography_section', wp_customize_url() ),
							'layoutSection' => add_query_arg( rawurlencode( 'autofocus[panel]' ), 'aureon_layout_panel', wp_customize_url() ),
						),
					)
				);
			}
	...
	public function page() {
		?>
		<div class="wrap">
			<div class="aureon-dashboard">
				<?php do_action( 'aureon_admin_dashboard' ); ?>
			</div>
		</div>
		<?php
	}
	...
	public function start_customizing() {
		echo '<div id="aureon-dashboard-app"></div>';
	}
	...
	public function go_pro() {
		echo '<div id="aureon-dashboard-go-pro"></div>';
	}
	...
	public function reset() {
		echo '<div id="aureon-reset"></div>';
	}
}

Aureon_Dashboard::get_instance();
