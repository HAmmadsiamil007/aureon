jQuery(document).ready(function($) {
	$( '#aureon-tabs-container input[type="checkbox"]' ).lc_switch( '', '' );

	$( '.aureon-tabs-menu a' ).on( 'click', function( event ) {
		event.preventDefault();
		$( this ).parent().addClass( 'aureon-current' );
		$( this ).parent().siblings().removeClass( 'aureon-current' );
		var tab = $( this ).attr( 'href' );
		$( '.aureon-tab-content' ).not(tab).css( 'display', 'none' );
		$( tab ).fadeIn( 100 );

		if ( '#aureon-image-tab' == tab || '#aureon-content-tab' == tab ) {
	...
	$( '#_meta-aureon-page-header-content' ).on( 'input change', function() {
...
jQuery(window).on('load', function() {
	if ( jQuery( '#_meta-aureon-page-header-enable-image-crop' ).val() == 'enable' ) {
		jQuery( '#crop-enabled' ).show();
	}

	jQuery( '#_meta-aureon-page-header-enable-image-crop' ).change(function () {
	...
	if ( jQuery( '#_meta-aureon-page-header-image-background' ).is( ':checked' ) ) {
	...
	jQuery('body').delegate('.image-background', 'lcs-statuschange', function() {
		if (jQuery(this).is(":checked")) {
			...
			jQuery('.parallax').hide();
			jQuery('#_meta-aureon-page-header-image-background-fixed').lcs_off();
		}
	});

	if ( jQuery('#_meta-aureon-page-header-full-screen').is(':checked')) {
		jQuery('.vertical-center').show();
	} else {
		jQuery('.vertical-center').hide();
	}

	jQuery('body').delegate('#_meta-aureon-page-header-full-screen', 'lcs-statuschange', function() {
		if (jQuery(this).is(":checked")) {
			jQuery('.vertical-center').show();
		} else {
			jQuery('.vertical-center').hide();
			jQuery('#_meta-aureon-page-header-vertical-center').lcs_off();
		}
	});

	if ( jQuery('#_meta-aureon-page-header-transparent-navigation').is(':checked')) {
		jQuery('.navigation-colors').show();
	} else {
		jQuery('.navigation-colors').hide();
	}

	jQuery('body').delegate('#_meta-aureon-page-header-transparent-navigation', 'lcs-statuschange', function() {
		if (jQuery(this).is(":checked")) {
			jQuery('.navigation-colors').show();
		} else {
			jQuery('.navigation-colors').hide();
	...
	if ( jQuery('#_meta-aureon-page-header-combine').is(':checked')) {
		jQuery('.combination-options').show();
	} else {
		jQuery('.combination-options').hide();
	}

	jQuery('body').delegate('#_meta-aureon-page-header-combine', 'lcs-statuschange', function() {
		if (jQuery(this).is(":checked")) {
			jQuery('.combination-options').show();
		} else {
			jQuery('.combination-options').hide();
	...
	if ( jQuery('#_meta-aureon-page-header-image-background-type').val() == '' ) {
		jQuery('.vertical-center').hide();
		jQuery('.fullscreen').hide();
	}
	jQuery('#_meta-aureon-page-header-image-background-type').change(function () {
		if (jQuery(this).val() === '') {
			jQuery('.vertical-center').hide();
			jQuery('#_meta-aureon-page-header-vertical-center').lcs_off();
			jQuery('.fullscreen').hide();
			jQuery('#_meta-aureon-page-header-full-screen').lcs_off();
		} else {
			//jQuery('.vertical-center').show();
	...
	var $set_button = jQuery('.aureon-upload-file');
	/**
...
jQuery(document).ready(function($) {
	$('#aureon-tabs-container .color-picker').wpColorPicker();

	jQuery( '.aureon-page-header-remove-image' ).on( 'click', function( e ) {
		e.preventDefault();
		...
		if ( '-1' == jQuery( '#_thumbnail_id' ).attr( 'value' ) ) {
			...
			jQuery( '.aureon-page-header-set-featured-image' ).show();
	...
	$('#postimagediv').on( 'click', '#remove-post-thumbnail', function() {
		...
		if ( '' == jQuery( '#_meta-aureon-page-header-image-id' ).attr( 'value' ) ) {
			jQuery( '.page-header-image-settings' ).hide();
			jQuery( '.aureon-page-header-set-featured-image' ).show();
		}
	...
	$( '.aureon-featured-image, .aureon-page-header-set-featured-image a' ).on( 'click', function( event ) {
		event.preventDefault();
