 ...
 *      halt the chain; WP actions bridge to lumina_core:wp_* events
...
use Lumina\Core\Container\CircularDependencyException;
...
echo "== Lumina Core Phase 2 smoke suite ==\n\n";

// Same contract as Phase 1: the suite verifies the DEFAULT bootstrap, so a
// developer's own lumina.env.json would invalidate the state assertions.
...
check( 'Repository: features.lumina_core === true', true === $repo->get( 'features.lumina_core' ) );
...
check( 'App::get("features.lumina_core") === true', true === $app->get( 'features.lumina_core' ) );
...
$dispatcher->dispatch( new \Lumina\Core\Events\GenericEvent( 'lumina_core:smoke_event' ) );
check( 'Dispatcher: listeners run in registration order', array( 'first', 'second' ) === $order, implode( ',', $order ) );

$stopped = 0;
$dispatcher->listen(
	'lumina_core:smoke_stop',
...
$dispatcher->dispatch(
	new class() implements \Lumina\Core\Events\EventInterface, \Lumina\Core\Events\StoppableEventInterface {
		public function name(): string {
			return 'lumina_core:smoke_stop';
		}
...
$bridged = array();
$dispatcher->listen(
	'lumina_core:wp_lumina_test_action',
...
$bridge_dsp->map( 'lumina_test_action' );
$hooks->do_action( 'lumina_test_action', 'payload' );
...
$filters->add_filter( 'lumina_test_filter', static fn( string $v ): string => $v . '-1' );
$filters->add_filter( 'lumina_test_filter', static fn( string $v ): string => $v . '-2' );
check( 'HookManager: apply() runs the filter chain', 'x-1-2' === $filters->apply( 'lumina_test_filter', 'x' ), (string) $filters->apply( 'lumina_test_filter', 'x' ) );

$fires = 0;
$cb    = static function () use ( &$fires ): void {
	++$fires;
};
$filters->add_action( 'lumina_test_action2', $cb );
$filters->add_action( 'lumina_test_action2', $cb ); // duplicate registration.
$filters->do_action( 'lumina_test_action2' );
...
check( 'CacheKey: namespaced + versioned format', 'lumina_tokens_v1_primary' === CacheKey::make( 'tokens', 'primary', 'v1' ) );
check( 'CacheKey: sanitizes segments', 'lumina_menu_header_nav' === CacheKey::make( 'menu', 'Header Nav!' ) );
...
check( 'Phase 1 regression: flags resolvable', $app->make( 'flags' ) instanceof \Lumina\Core\Support\FeatureFlags );
check( 'Phase 1 regression: logger resolvable', $app->make( 'logger' ) instanceof \Lumina\Core\Support\Debug\Loggers );
