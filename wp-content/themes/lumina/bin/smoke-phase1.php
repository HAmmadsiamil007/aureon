 ...
 *   5. lumina.env.json overrides are honored end-to-end (ADR-011)
 ...
 *   10. Namespace autoload resolves Lumina\Core\Core\Version
 *
 * Determinism: the suite asserts the DEFAULT bootstrap state first (fresh
 * install → production, debug off), then drives the lumina.env.json override
 ...
 * lumina.env.json is present, so it never corrupts local config.
 *
...
echo "== Lumina Core Phase 1 smoke suite ==\n\n";

// Refuse to run against a developer's own override file — the suite verifies
// the DEFAULT bootstrap, so a local lumina.env.json would invalidate every
// state assertion. CI checkouts are always clean.
$override_file = dirname( __DIR__ ) . '/lumina.env.json';
...
// Self-cleaning: the override section below writes a temporary lumina.env.json.
...
check( 'Version::VERSION is 1.0.0', Version::VERSION === '1.0.0', Version::VERSION );
...
check( 'FeatureFlags: lumina_core enabled', true === $flags->enabled( 'lumina_core' ) );
...
file_put_contents(
	$override_file,
	'{"environment":{"override":"staging"},"debug":true,"features":{"lumina_core":true,"asset_pipeline":true}}'
);

$loader2 = new \Lumina\Core\Config\ConfigLoader( dirname( __DIR__ ) );
$cfg2    = $loader2->load();

check( 'Env override: staging from lumina.env.json', 'staging' === Env::detect( $cfg2 ), (string) Env::detect( $cfg2 ) );
check( 'Debug override: true from lumina.env.json', true === Env::is_debug( $cfg2 ) );
