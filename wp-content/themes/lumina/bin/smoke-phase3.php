 ...
 *   4. A [data-lumina-theme="dark"] block is emitted with an alternate palette
...
use Lumina\Core\Tokens\Invariant;
use Lumina\Core\Tokens\TokenRepository;
use Lumina\Core\Tokens\UnknownToken;
...
echo "== Lumina Core Phase 3 smoke suite (Design Token Engine) ==\n\n";
...
check( 'css() contains --lumina-color-bg', str_contains( $css, '--lumina-color-bg:' ) );
check( 'css() contains --lumina-space-4', str_contains( $css, '--lumina-space-4: 0.25rem' ) );

// 4. Dark preset block emitted.
check( 'css() emits [data-lumina-theme="dark"]', str_contains( $css, '[data-lumina-theme="dark"] {' ) );

// 5. Preset switch flips the semantic palette.
$dark_block_start = strpos( $css, '[data-lumina-theme="dark"]' );
...
check( 'dark preset flips color.bg', str_contains( $dark_block, '--lumina-color-bg:' ) && ! str_contains( $dark_block, '--lumina-color-bg: #ffffff' ) );
check( 'default :root keeps light bg', str_contains( $default_block, '--lumina-color-bg: #ffffff' ) );
...
// preset AA").
$dark_resolved = ( new \Lumina\Core\Tokens\Resolver() )->resolve_all(
	( new \Lumina\Core\Tokens\Preced() )->collect(
		( new \Lumina\Core\Tokens\TokenSource() )->parse(
			( new \Lumina\Core\Tokens\Loader\DataProvider() )->tokens()
		),
		( new \Lumina\Core\Tokens\TokenSource() )->parse(
			(array) ( ( new \Lumina\Core\Tokens\Loader\DataProvider() )->presets()['dark'] ?? array() )
		)
	)
);
