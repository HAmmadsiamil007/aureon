...
use Lumina\Core\Assets\AssetLoader;
use Lumina\Core\Assets\AssetsServiceProvider;
use Lumina\Core\Assets\BuildFingerprint;
use Lumina\Core\Assets\DevServer;
use Lumina\Core\Assets\Image;
use Lumina\Core\Assets\ManifestReader;
use Lumina\Core\Assets\Markup;
use Lumina\Core\Assets\Pipeline\DepsResolver;
use Lumina\Core\Assets\Pipeline\Entries;
...
function make_manifest_fixture(): string {
	$dir  = sys_get_temp_dir() . '/lumina-manifest-' . getmypid();
	@mkdir( $dir, 0777, true );
...
echo "== Lumina Core Phase 7 smoke suite (Asset Pipeline) ==\n\n";
...
$vite6_dir = sys_get_temp_dir() . '/lumina-vite6-' . getmypid();
...
$cycle_path = sys_get_temp_dir() . '/lumina-cycle-' . getmypid() . '.json';
// phpcs:ignore WordPress.WP.AlternativeFunctions.json_encode_json_encode -- WP-free fixture encode.
// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents -- WP-free fixture.
...
check( 'Phase 6 regression: templates.resolver resolves', $app->make( 'templates.resolver' ) instanceof \Lumina\Core\Templates\TemplateResolver );
