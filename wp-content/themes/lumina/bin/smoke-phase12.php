 ...
 *      (24 slugs; `page-header` added Phase 16 for the Companion module)
...
use Lumina\Core\Templates\Composer;
...
echo "== Lumina Core Phase 12 smoke suite (Frontend Template Library) ==\n\n";
...
$expected = array(
	'header', 'footer', 'page-header', 'home', 'landing', 'shop', 'product',
	'cart', 'checkout', 'thank-you', 'account', 'wishlist', 'compare', 'blog',
	'single-post', 'archive', 'author', 'search', 'not-found', 'contact',
	'about', 'faq-page', 'privacy', 'terms',
);
check( 'all 24 template slugs registered', count( $slugs ) === count( $expected ) && 24 === count( $slugs ) );
...
foreach ( $expected as $slug ) {
	if ( in_array( $slug, array( 'header', 'footer', 'page-header' ), true ) ) {
		continue;
	}
...
foreach ( $expected as $slug ) {
	try {
		...
		if ( ! str_contains( $html, 'lumina-' ) ) {
			$compose_fails[] = $slug . ' (no lumina- class)';
		}
...
check( 'all 24 templates compose component HTML', array() === $compose_fails, implode( '; ', $compose_fails ) );
...
check( 'Phase 6 regression: partial loader resolves', $app->make( 'templates.partials' ) instanceof \Lumina\Core\Templates\PartialLoader );
check( 'Phase 10 regression: animation engine resolves', $app->make( 'animation.engine' ) instanceof \Lumina\Core\Animation\Engine );
check( 'Phase 11 regression: hero component renders', str_contains( $registry->render( 'hero', array( 'title' => 'R' ) ), 'lumina-hero' ) );
