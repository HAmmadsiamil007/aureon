/**
 * Lumina Core — component behavior layer (Phase 11).
 *
 ...
 * `[role=tabpanel]` panels inside a `[data-lumina-tabs]` scope.
 */
 ...
 * Activators are `[data-lumina-modal-open]` / `[data-lumina-popup]`.
 */
function initModal(root: HTMLElement): void {
  const overlay = root.querySelector<HTMLElement>('[data-lumina-modal-overlay]');
  const openers = root.querySelectorAll<HTMLElement>('[data-lumina-modal-open]');
  const closers = root.querySelectorAll<HTMLElement>('[data-lumina-modal-close]');
...
function initPopup(root: HTMLElement): void {
  const popup = root.querySelector<HTMLElement>('[data-lumina-popup]');
  ...
  const closeBtn = popup.querySelector<HTMLElement>('[data-lumina-popup-close]');
  closeBtn?.addEventListener('click', close);

  const delay = Number(popup.getAttribute('data-delay') ?? 5000);
  const stored = window.sessionStorage.getItem('lumina-popup-dismissed');
  ...
  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && !popup.hidden) {
      close();
      window.sessionStorage.setItem('lumina-popup-dismissed', '1');
    }
  });
}
...
function initDismissible(root: HTMLElement): void {
  root
    .querySelectorAll<HTMLElement>('[data-lumina-notification], [data-lumina-alert]')
    .forEach((el) => {
      const closeBtn = el.querySelector<HTMLElement>(
        '[data-lumina-notification-close], [data-lumina-alert-close]'
      );
    ...
    (entries) => {
      entries.forEach((entry) => {
        ...
        const duration = Number(
          el.closest('[data-lumina-counters]')?.getAttribute('data-duration') ?? 1200
        );
        ...
        const suffix = el.querySelector<HTMLElement>('.lumina-counters__suffix');
        const number = document.createElement('span');
        number.className = 'lumina-counters__number';
        el.textContent = '';
        el.append(number);
        if (suffix) {
          el.append(suffix);
...
function initBackToTop(root: HTMLElement): void {
  const button = root.querySelector<HTMLElement>('[data-lumina-back-to-top]');
...
function initStickyAddToCart(root: HTMLElement): void {
  const bar = root.querySelector<HTMLElement>('[data-lumina-sticky-atc]');
  ...
  const target = document.querySelector(
    bar.getAttribute('data-lumina-track') ?? '.lumina-product-card'
  );
...
function initTogglePanel(root: HTMLElement): void {
  const toggles = root.querySelectorAll<HTMLElement>('[data-lumina-toggle-target]');

  toggles.forEach((toggle) => {
    const target = document.querySelector<HTMLElement>(
      toggle.getAttribute('data-lumina-toggle-target') ?? ''
    );
...
/** Generic delegated click-to-trigger behaviors via [data-lumina-action]. */
function initActions(root: HTMLElement): void {
  root.querySelectorAll<HTMLElement>('[data-lumina-action]').forEach((el) => {
    const action = el.getAttribute('data-lumina-action');
...
function boot(): void {
  const roots = document.querySelectorAll<HTMLElement>(
    '[data-lumina-tabs], [data-lumina-modal], [data-lumina-popup], [data-lumina-counters]'
  );
  ...
  document.querySelectorAll<HTMLElement>('[data-lumina-modal]').forEach(initModal);
  document.querySelectorAll<HTMLElement>('[data-lumina-popup]').forEach(initPopup);
