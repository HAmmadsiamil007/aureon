 ...
 *   4. null (caller falls back / defers to WordPress)
 *
 * Phase 16 (Safe Rebranding): the parent-theme fallback tier was removed —
 * Lumina is a fully standalone theme that ships its own WP hierarchy files.
 *
 * It also encodes the documented WordPress template hierarchy (public,
 * stable knowledge) so template *types* resolve to the most specific file the
 * theme ships: `single-{post_type}-{slug}`, `single-{post_type}`, `single`,
 * `singular`, `index`. The resolver never touches WordPress globals.
 *
...
class TemplateResolver {
	 ...
	 * Build the resolver.
	 *
	 * @param string $child_dir Theme templates directory.
	 */
	public function __construct( string $child_dir ) {
	...
	public function path( string $name, array $context = array() ): ?string {
		...
		foreach ( $this->candidates( $name, $context ) as $file ) {
			if ( is_readable( $file ) ) {
				return $file;
			}
		}
