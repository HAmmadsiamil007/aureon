 ...
 * is env-driven (ADR-011): `LUMINA_VITE_ACTIVE=1` opts in, and the port is
 * read from `LUMINA_VITE_PORT` (default 5173). Never guesses based on port
...
class DevServer {
	...
	public static function from_env(): DevServer {
		...
		$port_raw = getenv( 'LUMINA_VITE_PORT' );
		// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals -- getenv() is PHP core.
		$host_raw = getenv( 'LUMINA_VITE_HOST' );
		// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals -- getenv() is PHP core.
		$active_raw = getenv( 'LUMINA_VITE_ACTIVE' );
