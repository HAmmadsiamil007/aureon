 ...
 * logged and raise `lumina_core:boot_error` (once), then the sequence stops —
 * the WP request surface is never crashed by a boot failure (ADR-013).
 *
 * The step list is filterable via `lumina_core:boot_steps` so other
...
final class Sequencer {
	 ...
	 *
	 * Lets the Kernel gate the `lumina_core:ready` event on full success.
	 *
	 * @return bool
	 */
	...
	public function run( array $context = array() ): array {
		...
		if ( function_exists( 'apply_filters' ) ) {
			$filtered = apply_filters( 'lumina_core:boot_steps', $steps );
		...
		foreach ( $steps as $id => $step ) {
			try {
				...
				if ( function_exists( 'do_action' ) ) {
					do_action( 'lumina_core:boot_error', $e, $id );
				}
