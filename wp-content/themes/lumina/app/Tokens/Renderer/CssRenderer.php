 ...
 * set plus a `[data-lumina-theme="<preset>"]` block per non-default preset
 ...
 *
 * @package Lumina\Core\Tokens\Renderer
 * @since 0.3.0
 */

declare( strict_types=1 );

namespace Lumina\Core\Tokens\Renderer;

use Lumina\Core\Tokens\TokenFactory;
...
final class CssRenderer {
	 ...
	 */
	public function render( array $default_map, array $presets = array(), string $scope = 'lumina' ): string {
		...
		foreach ( $presets as $slug => $tokens ) {
			$blocks[] = $this->block(
				sprintf( '[data-lumina-theme="%s"]', $this->sanitize_attr( (string) $slug ) ),
