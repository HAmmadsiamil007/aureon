 ...
 * template hooks (plan §Phase 9 hook table). Lumina never removes a WC hook;
 * where Lumina output replaces a template, the hooks are re-emitted
 * (guarded by `has_action`/`do_action` presence). `audit()` reports which
 * hooks still have registered callbacks (WP-free: empty audit).
 *
 * @package Lumina\Core\Woo\Hooks
 * @since 0.9.0
 */

declare( strict_types=1 );

namespace Lumina\Core\Woo\Hooks;
...
class HookPreservation {
	 ...
	 *
	 * Used where Lumina output replaces a WooCommerce template: the official
