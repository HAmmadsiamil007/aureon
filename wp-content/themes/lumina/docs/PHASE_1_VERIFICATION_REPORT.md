...
- **Project:** Lumina Theme / Lumina Core (GeneratePress child theme)
- **Phase:** 1 — Bootstrap
- **Date:** 2026-08-03
...
Phase 1 delivers the deterministic bootstrap layer that initializes Lumina Core
...
| Configuration loading                       | PASS — `Config\ConfigLoader` defaults + `lumina.env.json` overrides (ADR-011)       |
...
| Framework startup lifecycle                 | PASS — `lumina_core:booting → booted → ready`; `boot_error` on failure              |
...
1. `app/load.php` — single entry; registers the PSR-4 autoloader (if readable)
   ...
   Guarded (`ABSPATH`, `LUMINA_CORE_LOADED`) so it is safe in WP-free CLI runs.
...
3. `app/Boot/Sequencer.php` — ordered, filterable boot steps
   (`lumina_core:boot_steps`); failures logged + `lumina_core:boot_error` once.
...
| `wp-content/themes/lumina/.github/workflows/ci.yml` | **Deleted** (moved to repo root)                                                                     |
...
functions.php ──requires──▶ app/load.php
                             │ (guards: ABSPATH, LUMINA_CORE_LOADED)
                    ...
                    plugins_loaded (priority 5) ──▶ Boot\Kernel::launch()
                          ...
                          Sequencer steps (priority order, shared context):
                            ...
                            50 errorHandler  → ErrorHandler → provide('errorHandler')
                                                     ▼
                                                    run()
                             any step failure → Log::error() + raise('lumina_core:boot_error') once, STOP
                                                     ▼
                             raise('lumina_core:booted') → raise('lumina_core:ready')
```

- The Kernel **never throws** on the WordPress surface; failures are logged and
  converted to the single `lumina_core:boot_error` event (ADR-013).
- Double launch is a no-op (verified by the smoke suite).
- The boot step list is filterable via `lumina_core:boot_steps` for extensions.
...
| ADR                           | Compliance                                                                                                                      |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| ADR-001 (child theme)         | PASS — everything lives under `wp-content/themes/lumina`                                                                        |
| ADR-002 (namespaces/prefixes) | PASS — `Lumina\Core\`, `lumina_*` globals, `lumina_core:*` events                                                               |
...
| ADR-006 (event naming)        | PASS — domain bus uses `lumina_core:*` double-colon; PHPCS hook rule scoped (documented)                                        |
| ADR-009 (PSR-4)               | PASS — `Lumina\Core\` → `app/`, optimized autoload (2239 classes)                                                               |
| ADR-011 (environment)         | PASS — `wp_get_environment_type()` + `lumina.env.json` override                                                                 |
...
- **WPCS (PHPCS):** 0 errors, 0 warnings — `WordPress-Extra` + `WordPress-Docs`,
  `lumina` text domain, PHP 8.2 testVersion, 120-char limit.
...
| Override honored (ADR-011)                   | PASS           | `lumina.env.json` → `staging` + `debug=true` + flag override                                            |
| Suite hygiene                                | PASS           | refuses to run over a dev `lumina.env.json` (SKIP, exit 0); temp file always removed (shutdown handler) |
...
- No secrets committed; `lumina.env.json.example` documents the format.
...
## Acceptance Criteria Checklist (source: `PHASE_5_LUMINA_CORE_IMPLEMENTATION_PLAN.md` §Phase 1)
...
| AC-3 | Config is loaded from disk/env                    | **PASS**                   | `ConfigLoader` defaults + `lumina.env.json` override; both paths asserted in smoke                                          |
| AC-4 | `App::debug(false)` resolvable                    | **PASS** _(renamed)_       | `App::is_debug() === false` asserted; naming deviation documented                                                           |
| AC-5 | `lumina_core:ready` fires                         | **PASS** _(code-level)_    | Raised after sequencer run in `Kernel::boot()`; runtime verification on live WP scheduled (Phase 15)                        |
| VC-1 | Fresh install → `App::get('env')` = `production`  | **PASS**                   | Smoke: `env` service = `production`                                                                                         |
| VC-2 | `WP_ENVIRONMENT_TYPE=staging` → staging           | **PASS** _(path verified)_ | ADR-011 override path proven (staging via `lumina.env.json`); WP env-var flows through WP's own `wp_get_environment_type()` |
| VC-3 | `lumina.env.json` secret never in log             | **PASS**                   | Redaction assertions (absent secret, `[REDACTED]` marker)                                                                   |
...
> `lumina_core:ready` now only fires when every boot step succeeded (no false
