...
- **No `<lumina.card>`-style HTML tag DSL** — shortcode DSL only (see Phase 5).
...
| -------------------------------------------------- | -------- | -------------------------------------------------------------------------------- |
| `single` resolves to `lumina/templates/single.php` | **PASS** | smoke: str_ends_with(…, 'single.php')                                            |
| `content-single` partial loaded                    | **PASS** | renders lumina-entry + escaped title                                             |
| Registered section appended to `loop` emerges      | **PASS** | sections render card + callable in order                                         |
| Missing partial throws-aware fallback to `index`   | **PASS** | fallback renders data-lumina-partial="index"; no-fallback throws RenderException |
