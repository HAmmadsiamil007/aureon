...
never global — and `lumina-animation` is enqueued only when the engine is
active (zero cost when unused). A canonical `reveal` preset
(`[data-lumina-anim="reveal"]` → GSAP on scroll) satisfies the acceptance
...
```
Engine (boot_config → window.luminaAnimation)
   ├─ presets:  AnimationRegistry { reveal: Preset([data-lumina-anim=reveal]) }
...
JS: animation.ts (code-split entry, ~3.5 KB)
   ├─ prefers-reduced-motion? → set data-lumina-anim-reduced, exit (no libs)
   ├─ observer cap check
...
| ----------- | ---------------------------------------------------- | ------ |
| ADR-002     | `Lumina\Core\Animation` namespace; `animation.*` ids | ✅     |
...
| -------------------------------------------------------- | -------- | ----------------------------------------------- |
| `data-lumina-anim="reveal"` lifts into GSAP on load      | **PASS** | canonical reveal preset + TS ScrollTrigger path |
| prefers-reduced-motion gated (no transforms)             | **PASS** | CSS guard + JS early exit (no listeners)        |
| Three entry lazy-loads only when `.lumina-canvas` exists | **PASS** | mount presence check before `import('three')`   |
...
- Entry `lumina-animation` is ~3.5 KB (gzip 1.6 KB); GSAP (`index` 70 KB /
