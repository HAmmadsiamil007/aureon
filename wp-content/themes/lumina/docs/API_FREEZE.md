# API FREEZE — Lumina Theme / Lumina Core

- **Version:** 1.0.0 (Lumina — Safe Rebranding)
- **Date:** 2026-08-04
- **Phase:** 16 (Safe Rebranding) — re-freeze of the v0.14.0 surface
...
All identifiers are locked at `v1.0.0-lumina` (namespace `Lumina\Core\`,
prefix `lumina_`, handles `lumina-*`, tokens `--lumina-*`, events
`lumina_core:*` — ADR-026).
...
PSR-4: `Lumina\Core\` → `app/` (ADR-009). All classes final, documented,
...
## 4. Custom Events (namespaced `lumina_core:*`)

```
lumina_core:booting · lumina_core:booted · lumina_core:ready ·
lumina_core:boot_steps (filter) · lumina_core:boot_error ·
lumina_core:animation:ready · lumina_core:cache_purged
```
...
| `lumina_template_data`           | filter | Template composition data (Phase 12)                 |
| `lumina_template_slug`           | filter | Custom template slug (Phase 12)                      |
| `lumina_render_resolver_context` | filter | Render resolver context (Phase 4)                    |
...
- `[lumina:{slug}]` — component registry DSL (Phase 5).
...
- CSS variables: `--lumina-*` prefix (ADR-002/003); scope
  `:root` + `[data-lumina-theme="…"]`.
- Component classes: `lumina-*`; animation hooks `data-lumina-anim`.

## 8. Configuration Schema (frozen keys)

```
environment.override · debug · features.{lumina_core, render_engine,
...
Environment overrides: `lumina.env.json` (ADR-011).

## 9. JavaScript APIs (frozen)

- Vite entries: `main` (index), `styles`, `components`, `animation`.
- Global: `window.luminaAnimation` (animation boot config, Phase 10).
- DOM protocol: `[data-lumina-*]` hooks, `data-lumina-theme` for theming,
  `data-lumina-anim` for reveal presets.
