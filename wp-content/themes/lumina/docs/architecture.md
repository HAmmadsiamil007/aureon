...
Lumina is a **fully standalone WordPress theme** that delivers a modern,
token-driven frontend framework ("Lumina Core") on top of WordPress. It
follows the master architecture blueprint (`Report/master_architecture_blueprint.md`)
and the Phase 5 implementation plan
(`Report/PHASE_5_LUMINA_CORE_IMPLEMENTATION_PLAN.md`).

## Non-negotiable rules (ADR-001 … ADR-004)

1. **Never modify WordPress Core or any plugin.** Everything visual lives in
   this theme.
2. Only **public, documented WordPress APIs** are used — `wp_nav_menu`,
   widgets, template hierarchy, Customizer, hooks. Internal/core symbols are
   never called.
3. Naming: namespace `Lumina\Core\`, global prefix `lumina_`, option prefix
   `lumina_`, hook prefix `lumina_`, asset handles `lumina-*`.
4. Presentation is **token-driven** (CSS custom properties, ADR-003). No
   hard-coded hex/px in component code.
5. A CI job runs the full WP-free smoke suite (14 suites) + a Lumina
   self-integrity gate (`bin/verify-lumina-integrity.sh`).

## Canonical directory layout (Phase 0)

```
lumina/
├─ style.css            # WP theme header; Version: 1.0.0
├─ functions.php        # thin loader → requires app/load.php (Phase 1)
├─ theme.json           # editor defaults (vanilla)
├─ composer.json        # PSR-4 Lumina\Core\ → app/ (ADR-009)
...
├─ app/                 # Lumina Core — namespace Lumina\Core\
...
wp → plugins_loaded (priority 5)
   ...
   → providers (Phase 2) → do_action('lumina_core:ready')
```
...
`wp_get_environment_type()` is the base detector. `lumina.env.json` (git-ignored;
template: `lumina.env.json.example`) overrides it per environment, holds feature
