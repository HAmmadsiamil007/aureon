...
Every phase ships behind a `lumina_feature_*` flag (ADR-002). Flags default
**off** for unshipped subsystems, **on** for complete ones. Toggling is
per-environment via `lumina.env.json` (ADR-011) — never edited in code.
