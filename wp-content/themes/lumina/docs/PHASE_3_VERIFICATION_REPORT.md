   ...
   └─ TokenSource::parse()          nested groups → flat dot-map
        └─ Preced::collect()         default → preset → (future) override
             └─ Resolver::resolve_all()   walks 'extends' graph (cycle-safe)
                  ...
                  └─ TokenFactory::to_css_map()  → '--lumina-{name}'
                       └─ CssRenderer::render()  → :root + [data-lumina-theme]
```

- **CSS vars:** `color.bg` → `--lumina-color-bg`; dots become hyphens.
- **Presets:** `default` (base) + `dark` (`[data-lumina-theme="dark"]`).
...
| `CssRenderer` emits valid `:root` block for default + dark | **PASS** | `:root {` + `[data-lumina-theme="dark"] {` blocks asserted |
