...
- **No `<lumina.card>` tag syntax** — the plan lists both `<lumina.card>`
  and `[lumina:button]`; only the shortcode DSL was implemented (the
  acceptance criterion exercises `[lumina:button]`). HTML-element style tags
...
- **DSL:** `[lumina:button text="go" variant="outline"]` renders identically
...
| Shortcode `[lumina:button]` renders same as direct call | **PASS** | byte-identical output asserted         |
...
| Shortcode DSL `[lumina:button]`                             | **PASS** | parity + attr coercion asserted     |
...
| Shortcode tag collisions       | `lumina:` prefix (ADR-002 handles); tags map to registry names        | Low   |
