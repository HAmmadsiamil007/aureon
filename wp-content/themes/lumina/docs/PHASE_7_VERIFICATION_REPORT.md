...
CSS (containing every `--lumina-*` token) and one deferred JS; dev gets HMR
via `LUMINA_VITE_ACTIVE` / `LUMINA_VITE_PORT`. `assets/dist` is gitignored;
...
npm run build (Vite, 2 entries)
   ...
   ├─ assets/dist/assets/styles-*.css   ← tokens + base (--lumina-* vars)
...
- **Static tokens:** `php bin/build-tokens.php` renders the token repository
  ...
  shipped CSS (verified: `grep -- '--lumina-' dist CSS` hits).
...
| ADR-011     | Dev server via `LUMINA_VITE_*` env overrides        | ✅     |
...
| Dist CSS tokens         | `grep -- '--lumina-' assets/dist/assets/*.css`                                                                                                                                                                                                                                                                                                            | ✅ PASS           |
...
| Output CSS contains `--lumina-*` tokens             | **PASS** | dist CSS carries `--lumina-color-fg` etc.    |
...
| CSS loads with tokens present                        | **PASS** | dist CSS `--lumina-*` grep                               |
| JS entry interactive                                 | **PASS** | main-*.js emitted; `data-lumina-ready` boot in TS        |
| Dev server used in dev (`localhost:5173`)            | **PASS** | `LUMINA_VITE_ACTIVE` → dev URL                           |
