...
| 2   | Event bus (domain `phantom_core:*`) with ordered dispatch + stop-propagation        | ✅       |
...
- `phantom.env.json.example` — schema corrected to `environment.override` /
  `features`.
...
- Domain events stay on `phantom_core:*` (ADR-006); WP actions are forwarded
...
| ------- | --------------------------------------------------- | ------ |
| ADR-002 | `Phantom\Core` namespace, `phantom_*` hooks/options | ✅     |
| ADR-004 | Public WP/GP APIs only; parent dirs untouched       | ✅     |
| ADR-006 | Domain events `phantom_core:*`                      | ✅     |
| ADR-009 | PSR-4 Composer autoload `Phantom\Core\` → `app/`    | ✅     |
...
3. Flush object cache + transients (`wp cache flush`, or delete
   `phantom_*` transient keys written by the framework).
4. Feature flag `phantom_feature_phantom_core` (Phase 0) remains the global
