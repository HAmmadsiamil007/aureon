# CONTRACT FREEZE — Phantom Theme / Phantom Core

- **Version:** 0.14.0 (RC — Production Freeze)
- **Date:** 2026-08-04
- **Phase:** 15.5 (Production Freeze & Release Candidate)
- **Status:** 🔒 **FROZEN** — every contract below is locked at `v0.14.0`
...
contracts, and the animation contract. Phase 16 rebranding must preserve
functional parity against every contract listed here.
...
- **Slot system:** `hero → button`, `footer → footer-columns, copyright`,
  ...
  edges).
- **DSL:** `[phantom:{slug}]` shortcode with `[phantom:{slug} variant="…" …]`
  attribute surface.
...
- **Data contract:** template data arrives via the `phantom_template_data`
  filter; `custom.php` also reads `phantom_template_slug`.
...
- Cache invalidation fires `phantom_core:cache_purged`.
...
- Skip link: first-focusable, `screen-reader-text phantom-skip-link` →
...
- CSS output: `:root` + `[data-phantom-theme="default|dark"]` blocks using
  `--phantom-*` custom properties.
