...
> **Date:** 2026-08-03
> **Theme root:** `wp-content/themes/phantom/` (GeneratePress child theme)
> **Source of truth:** `Report/PHASE_5_PHANTOM_CORE_IMPLEMENTATION_PLAN.md` §Phase 0 · `Report/MASTER_ROADMAP.md` (ADR-001…012)
...
criterion and verification checklist item.** The Phantom child-theme repository
was stood up with the canonical directory structure, PSR-4 autoloading
(`Phantom\Core\` → `app/`), WordPress Coding Standards tooling (PHPCS + WPCS
...
| ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ | ------ |
| Canonical directory structure (child theme + `assets-src` + `tests` + `e2e` + `docs`) | `phantom/` tree                                                                | ✅     |
...
| Composer config, PSR-4 `Phantom\Core\` → `app/`, dev tools                            | `composer.json` + `composer.lock`                                              | ✅     |
...
| Environment configuration template (ADR-011)                                          | `phantom.env.json.example`                                                     | ✅     |
...
- `style.css`, `functions.php`, `theme.json`, `composer.json`, `composer.lock`,
  ...
  `phantom.env.json.example`, `README.md`

**App (Phantom Core):**

- `app/Core/Version.php` — the only Phase 0 class (`Phantom\Core\Core\Version`),
...
| ADR-002 Namespaces/prefixes                     | `Phantom\Core\`, `phantom_`, `phantom-` in `Version.php`, `.phpcs.xml` prefix rule ✅ |
...
| ADR-009 PSR-4 via Composer                      | `composer.json` → `Phantom\Core\` → `app/`; autoload dump ✅                          |
| ADR-010 Caching (deferred Phase 2)              | N/A — not yet implemented by design                                                   |
| ADR-011 Environment config                      | `phantom.env.json.example` template ✅                                                |
...
- No secrets committed; `.gitignore` excludes `phantom.env.json`, `.env*`,
  `*.log`, caches, vendor/node_modules.
- `phantom.env.json.example` contains placeholders only (ADR-011).
...
- Repository revert: `git reset --hard` / revert to `main` — the theme repo is
  independently versioned; nothing outside `wp-content/themes/phantom/` was
  touched.
- Remove the theme: delete `wp-content/themes/phantom/` — GeneratePress remains
