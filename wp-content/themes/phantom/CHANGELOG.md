...
phantom-skip-link` → `#main`) with WP-first/PHP-fallback escaping;
...
- `Performance\CachePurger` — centralized cache purge firing the namespaced
  `phantom_core:cache_purged` action; `wp_cache_flush()` passthrough.
...
contact,about,faq,privacy,terms,custom}.php` — thin WordPress templates
  delegating entirely to `View::compose()`; data via `phantom_template_data`
  / `phantom_template_slug` filters. No markup, no business logic.
...
- `assets-src/scss/_components.scss` — token-driven component layer: every
  value resolves to a `--phantom-*` token (no hardcoded colors/spacing/
  typography), dark-mode inherits `[data-phantom-theme="dark"]`, reduced
...
- `Animation\Engine` — controller: aggregates registry + gates into one
  serialized boot config; emits `phantom_core:animation:ready` when active.
...
- Integration fixes from review: boot config now delivered to the browser via
  `wp_localize_script` (previously `window.phantomAnimation` was never set, so
  ...
  `phantom-animation` style handle (previously dropped); Lenis RAF loop stop
  via a real `phantom:lenis:stop` event; Three mounts queried by declared
...
- `Assets\DevServer` — `PHANTOM_VITE_ACTIVE`/`PHANTOM_VITE_PORT`-driven
...
- `Components\Registry` — register/get/has/all/versions/provides_slot,
  ...
  `[phantom:{slug}]` shortcode DSL (`render_shortcode`).
