 ...
 *   7. Components are animation-ready (data-phantom-anim / data-* hooks)
...
use Phantom\Core\Components\Registry;
...
echo "== Phantom Core Phase 11 smoke suite (Frontend Component Library) ==\n\n";
...
foreach ( $names as $name ) {
	try {
		...
		if ( ! str_contains( $html, 'phantom-' ) ) {
			$render_fails[] = $name . ' (no phantom- class)';
		}
...
check( 'newsletter labels its input', str_contains( $newsletter_html, 'screen-reader-text' ) && str_contains( $newsletter_html, 'for="phantom-newsletter-email"' ) );
...
check( 'cta opts into the reveal animation', str_contains( $registry->render( 'cta', $fixtures['cta'] ), 'data-phantom-anim="reveal"' ) );
check( 'testimonials opt into reveal', str_contains( $registry->render( 'testimonials', $fixtures['testimonials'] ), 'data-phantom-anim="reveal"' ) );
check( 'counters carry count hooks', str_contains( $registry->render( 'counters', $fixtures['counters'] ), 'data-phantom-counters' ) && str_contains( $registry->render( 'counters', $fixtures['counters'] ), 'data-count-target' ) );
check( 'sticky add to cart carries its hook', str_contains( $registry->render( 'sticky-add-to-cart', $fixtures['sticky-add-to-cart'] ), 'data-phantom-sticky-atc' ) );

// 8. Token-driven stylesheet.
$scss = (string) file_get_contents( dirname( __DIR__ ) . '/assets-src/scss/_components.scss' );
check( 'component layer uses var(--phantom-*) tokens', substr_count( $scss, 'var(--phantom-' ) > 50 );
check( 'component layer uses motion tokens', str_contains( $scss, '--phantom-motion-duration' ) );
check( 'component layer uses radius tokens', str_contains( $scss, '--phantom-radius-' ) );
check( 'component layer uses spacing tokens', str_contains( $scss, '--phantom-space-' ) );
check( 'component layer has no raw hex colors', 0 === preg_match( '/#[0-9a-fA-F]{3,6}\b/', preg_replace( '/\/\/.*$/m', '', $scss ) ) );
check( 'component layer respects reduced motion', str_contains( $scss, 'prefers-reduced-motion' ) || str_contains( $scss, 'phantom-shimmer' ) );

// 9. Conditional asset enqueue (provider wiring).
$provider = new \ReflectionClass( \Phantom\Core\Components\ComponentsServiceProvider::class );
...
check( 'hero actions slot renders its button child', str_contains( $hero_slot, 'Buy' ) && str_contains( $hero_slot, 'phantom-btn' ) );

// 11. Shortcode DSL parity for catalog components.
$direct = $registry->render( 'button', array( 'label' => 'Short' ) );
$short  = $registry->render_shortcode( 'phantom:button', array( 'label' => 'Short' ) );
...
check( 'Phase 7 regression: asset loader resolves', $app->make( 'assets.loader' ) instanceof \Phantom\Core\Assets\AssetLoader );
