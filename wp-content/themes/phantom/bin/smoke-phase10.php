...
use Phantom\Core\Animation\AnimationRegistry;
use Phantom\Core\Animation\Breaking;
use Phantom\Core\Animation\Engine;
use Phantom\Core\Animation\Lenis;
use Phantom\Core\Animation\Preset;
use Phantom\Core\Animation\ReducedMotion;
use Phantom\Core\Animation\Scroll\Trigger;
use Phantom\Core\Animation\Three;
...
check( 'reveal targets data-phantom-anim=reveal', '[data-phantom-anim="reveal"]' === $reveal->target() );
...
$three->with_canvas( '.phantom-hero-canvas', array( 'clear_color' => '#000000' ) );
check( 'three with_canvas registers mount', array() !== $three->mounts() );
check( 'three mount selector recorded', isset( $three->mounts()['.phantom-hero-canvas'] ) );
...
$trigger->on( '[data-phantom-trigger]', array( 'start' => 'top 80%' ) );
check( 'trigger on() registers', array() !== $trigger->all() );
check( 'trigger serialized', isset( $trigger->to_array()['[data-phantom-trigger]'] ) );
