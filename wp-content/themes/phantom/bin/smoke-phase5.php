 ...
 *  12. `[phantom:button]` shortcode renders identically to a direct call
...
use Phantom\Core\Components\ComponentCycleException;
use Phantom\Core\Components\ComponentDefinition;
use Phantom\Core\Components\ComponentException;
use Phantom\Core\Components\ComponentNotFoundException;
use Phantom\Core\Components\ComponentsServiceProvider;
use Phantom\Core\Components\CycleDetector;
use Phantom\Core\Components\DefinitionCompiler;
use Phantom\Core\Components\Loader;
use Phantom\Core\Components\Registry;
use Phantom\Core\Components\Resolver;
...
echo "== Phantom Core Phase 5 smoke suite (Component Registry) ==\n\n";
...
check( 'render("button") produces HTML', str_contains( $button_html, 'phantom-btn' ) && str_contains( $button_html, 'Go' ) );
...
check( 'render("card") produces HTML', str_contains( $card_html, 'phantom-card' ) && str_contains( $card_html, 'Hello Card' ) );
...
check( 'variant preset class applies', str_contains( $ghost, 'phantom-btn--ghost' ) );

$explicit = $registry->render( 'button', array( 'label' => 'G', 'variant' => 'ghost', 'class' => 'custom' ) );
check( 'explicit props win over variant presets', str_contains( $explicit, 'custom' ) && ! str_contains( $explicit, 'phantom-btn--ghost' ) );
...
check( 'slot children render recursively', str_contains( $with_slot, 'Buy' ) && str_contains( $with_slot, 'phantom-card__actions' ) );
...
$short  = $registry->render_shortcode( 'phantom:button', array( 'label' => 'Short', 'variant' => 'outline' ) );
check( 'shortcode [phantom:button] renders identically to a direct call', $direct === $short );
check( 'shortcodes() maps the DSL tag', isset( $registry->shortcodes()['phantom:button'] ) );
$demo_short = $registry->render_shortcode( 'phantom:demo', array( 'label' => 'x', 'size' => 5 ) );
check( 'shortcode attrs coerce numeric sizes', str_contains( $demo_short, 'data-phantom-size="5"' ) );
...
check( 'unknown shortcode renders empty string', '' === $registry->render_shortcode( 'phantom:ghost' ) );
