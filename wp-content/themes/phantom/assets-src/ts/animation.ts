/**
 * Phantom Core — animation runtime (Phase 10).
 *
 * Declarative animation driven by `data-phantom-anim` attributes and the
 * boot config emitted by `Phantom\Core\Animation\Engine`. GSAP, Lenis and
 ...
 *   - Three — only when a declared `.phantom-canvas`-style mount exists.
 *
...
interface PhantomAnimationConfig {
  presets: Record<string, PhantomPreset>;
 ...
 */
function resolveConfig(): PhantomAnimationConfig {
  const globalConfig = (globalThis as { phantomAnimation?: PhantomAnimationConfig })
    .phantomAnimation;
 ...
 */
function prefersReducedMotion(config: PhantomAnimationConfig): boolean {
...
export async function startAnimation(): Promise<void> {
  ...
  if (prefersReducedMotion(config)) {
    document.documentElement?.setAttribute('data-phantom-anim-reduced', '');
    return;
  }
  ...
  if (config.lenis.enabled) {
    ...
    document.documentElement?.addEventListener('phantom:lenis:stop', stop, { once: true });
  }
  ...
  // `data-phantom-canvas` attribute is NOT required). Each declared mount
  ...
  document.documentElement?.setAttribute('data-phantom-anim-ready', '');
}

// Self-boot: the PHP side enqueues this entry only when the Animation Engine
// is active. The engine injects `window.phantomAnimation`; without it the
