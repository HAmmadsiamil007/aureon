 ...
 * Component Registry (variants, props, and the `[phantom:button]` DSL).
 *
...
<?php if ( $view->prop( 'href' ) ) : ?>
	<a
		class="<?php echo $view->attr( $view->prop( 'class', 'phantom-btn phantom-btn--primary' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped by ViewContext::attr(). ?>"
		...
		data-phantom-size="<?php echo $view->attr( $view->prop( 'size', 'md' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped by ViewContext::attr(). ?>"
	>
