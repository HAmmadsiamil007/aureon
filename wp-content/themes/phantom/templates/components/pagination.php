...
?>
<?php if ( $total > 1 ) : ?>
	<nav class="phantom-pagination" aria-label="Pagination">
		<ul class="phantom-pagination__list">
			<?php for ( $page = 1; $page <= $total; $page++ ) : ?>
				<li class="phantom-pagination__item">
					<?php if ( $page === $current ) : ?>
						<span class="phantom-pagination__link is-current" aria-current="page">
							<?php echo (int) $page; ?>
						</span>
					<?php else : ?>
						<?php
						...
						?>
						<a class="phantom-pagination__link" href="<?php echo $view->url( $link_url ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped by ViewContext::url(). ?>">
							<?php echo (int) $page; ?>
						</a>
					<?php endif; ?>
				</li>
