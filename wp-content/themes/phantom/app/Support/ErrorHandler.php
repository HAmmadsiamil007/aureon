...
final class ErrorHandler {
	 ...
	 * single, logged `phantom_core:boot_error` event. Never rethrows.
	 *
	 * @return void
	 */
	...
	public function wrap( \Throwable $e ): \WP_Error {
		return new \WP_Error( 'phantom_error', $e->getMessage() );
	}
	...
	/**
	 * Log the failure and raise phantom_core:boot_error exactly once.
	 *
	 * @param \Throwable $e The exception.
	 * @return void
	 */
	private function report( \Throwable $e ): void {
		...
		Log::error(
			'Phantom Core failure: {class}: {message}',
			array(
				'class'   => $e::class,
		...
		if ( function_exists( 'do_action' ) ) {
			do_action( 'phantom_core:boot_error', $e );
		}
	}
}
