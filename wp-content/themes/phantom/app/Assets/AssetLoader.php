 ...
 * (via the Vite manifest) or dev-server URLs (when `PHANTOM_VITE_ACTIVE=1`),
...
class AssetLoader {
	...
	public function css( string $src ): void {
		...
		wp_enqueue_style( 'phantom-' . sanitize_key( $src ), $url, array(), $this->fingerprint->token() );
	}
	...
	public function js( string $src, array $deps = array(), bool $in_footer = true ): void {
		...
		wp_enqueue_script( 'phantom-' . sanitize_key( $src ), $url, $deps, $this->fingerprint->token(), $in_footer );
	}
	...
	public function font_face( array $font ): void {
		...
		// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals -- WP core function.
		wp_add_inline_style( 'phantom-assets', $css );
	}
}
