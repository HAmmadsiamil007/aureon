# VINETA ROUTE CONTRACT REPORT

**Date:** 2026-09-02
**Status:** PASS
**Scope:** All retained routes mapped and verified
**Phases Covered:** PHASE 18 (Route Contract)

---

## Executive Summary

All 108 retained HTML pages in the Vineta package have been mapped to their intended WordPress/WooCommerce routes. Every route has a defined page family, dynamic data source, bridge mechanism, and static fallback. Zero dead routes exist. The route contract is complete and ready for AUREON bridge implementation.

---

## Route Contract

| Source Route | Page Family | WordPress Route | WooCommerce Route | Dynamic Data | Bridge | Fallback | Status |
|-------------|-------------|-----------------|-------------------|--------------|--------|----------|--------|
| `index.html` | HOME | `/` | — | Products, categories, Customizer | Customizer + WC | Static homepage | ✅ |
| `home-*.html` (29 variants) | HOME ALT | `/` (alternate designs) | — | Products, categories, Customizer | Customizer + WC | Static alternate homepages | ✅ |
| `product-detail.html` | PRODUCT | `/product/{slug}` | `/product/{slug}` | Product data, variations, gallery | WC product | Static product page | ✅ |
| `product-*.html` (33 variants) | PRODUCT ALT | `/product/{slug}` (alternate layouts) | `/product/{slug}` | Product data, variations | WC product | Static product layouts | ✅ |
| `shop-default.html` | SHOP | `/shop` | `/shop` | Products, filters, sorting | WC product loop | Static shop page | ✅ |
| `shop-*.html` (13 variants) | SHOP ALT | `/shop` (alternate layouts) | `/shop` | Products, filters | WC product loop | Static shop layouts | ✅ |
...
| Route Type | Count | WP Integration | Status |
|------------|-------|----------------|--------|
...
| Wishlist/compare | 2 | Bridge required | ⚠️ |
...
```
/                                    → index.html (homepage)
/product/{slug}/                     → product-detail.html + variants
/product-category/{slug}/            → shop-collection-list.html
/product-category/{parent}/{child}/  → shop-sub-collection.html
/shop/                               → shop-default.html + variants
/blog/                               → blog-grid-01.html + variants
...
**Result: ZERO dead routes.**

Every HTML file in the package maps to a documented route. No orphan files exist. Every file has:
1. A defined page family
2. A WordPress permalink target
3. A data source (WC, WP, or static)
4. A fallback mechanism

---

## AUREON Slot Coverage

Key routes have `data-aureon-slot` attributes for bridge integration:

| Route | Slots | Coverage |
|-------|-------|----------|
| `index.html` | `global.*`, `global.hero`, `global.featured_*`, `global.footer`, `global.newsletter` | ✅ Core hooks |
| `product-detail.html` | `product.*` (20+ slots: gallery, title, price, variation, tabs, related) | ✅ Full product |
| `shop-default.html` | `shop.*` (filters, grid, pagination) | ✅ Shop hooks |
| `blog-grid-01.html` | `blog.*` (grid, card, pagination) | ✅ Blog hooks |
| `blog-single.html` | `article.*` (title, content, author, comments, related) | ✅ Article hooks |
| `account-page.html` | `account.*` (navigation, dashboard, welcome, order_count) | ✅ Account hooks |
| `cart-drawer-v2.html` | `cart.*` (count, drawer, items, total, checkout) | ✅ Cart hooks |
| `view-cart.html` | `cart.*` (items, coupon, checkout) | ✅ Cart page |
| `checkout.html` | `checkout.*` (form, billing, shipping, payment, order_review) | ✅ Checkout hooks |
| `thank-you.html` | `thankyou.*` (order_number) | ✅ Order hooks |
| `wish-list.html` | `wishlist.*` (table, items, remove, add_to_cart) | ✅ Wishlist hooks |
| `compare.html` | `compare.*` (table, items, add_to_cart, remove) | ✅ Compare hooks |
| `contact-us.html` | `static.*` (map, contact_info, contact_form) | ✅ Contact hooks |
| `404.html` | `error.*` (content, search) | ✅ Error hooks |

**AUREON Hook Coverage:** 14 of 27 unique routes have explicit `data-aureon-slot` attributes (52%). The remaining routes (shop variants, product variants, blog variants, static pages) inherit hooks from their primary page or use static content that requires no bridge.
...
|---|-------|----------|-------|--------|
| 1 | Wishlist/Compare require third-party WC plugin | Medium | wish-list, compare | ⚠️ Bridge required |
| 2 | Product variants (33 files) share primary route `/product/{slug}` | Low | product-* | ✅ Acceptable (design variants) |
| 3 | Shop variants (13 files) share primary route `/shop` | Low | shop-* | ✅ Acceptable (layout variants) |
...
All 108 retained routes are fully mapped, verified, and ready for AUREON bridge implementation. The route contract covers every page in the Vineta package with:
- Defined WordPress permalinks
- WooCommerce data source mapping
- Bridge mechanism identification
- Static fallback for each route
- Zero dead routes

The only bridge-required capabilities (Wishlist and Compare) are documented and flagged for plugin integration. All other routes use standard WordPress/WooCommerce data sources that AUREON already supports natively.
