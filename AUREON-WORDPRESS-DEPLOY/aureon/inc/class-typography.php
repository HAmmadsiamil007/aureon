...
class Aureon_Typography {
	...
	public function __construct() {
		...
		add_filter( 'aureon_editor_styles', array( $this, 'add_editor_styles' ) );
	...
	public static function get_google_fonts_uri() {
		$fonts = aureon_get_option( 'font_manager' );
		...
		foreach ( $fonts as $font ) {
			...
			$variants = apply_filters( 'aureon_google_font_variants', $variants, $font['fontFamily'] );
		...
		if ( ! empty( $data ) ) {
			$font_args = apply_filters(
				'aureon_google_font_args',
				array(
					'family' => implode( '|', $data ),
					'subset' => null,
					'display' => aureon_get_option( 'google_font_display' ),
				)
			);
	...
	public function enqueue_google_fonts() {
		...
		if ( $google_fonts_uri ) {
			wp_enqueue_style( 'aureon-google-fonts', $google_fonts_uri, array(), AUREON_VERSION );
		}
	}
	...
	public static function get_css_selector( $selector ) {
		...
		return apply_filters( 'aureon_typography_css_selector', $selector );
	}
	...
	public static function get_font_family( $font_family ) {
		...
		if ( ! empty( $font_family_args['googleFont'] ) && ! empty( $font_family_args['googleFontCategory'] ) ) {
			...
			$font_family = aureon_get_system_default_font();
		}
	...
	public function enqueue_editor_scripts() {
		...
		if ( $html_typography ) {
			wp_add_inline_style(
				 ...
				 *
				 * Related: https://github.com/tomusborne/aureon/issues/472
				 */
				'wp-edit-blocks',
				$html_typography
			);
...
Aureon_Typography::get_instance();
