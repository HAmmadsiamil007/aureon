<?php
/**
 * WooCommerce My Account Template (AETHER + Vineta Standalone Auth).
 *
 ...
 *              the other endpoints.
 * Logged out → Routes to standalone premium login or register pages
 *              when ?page=login or ?page=register is set. Falls back
 *              to the combined login/register form otherwise.
 *
 * @package Aureon
 */
...
// Route to standalone auth pages when requested via ?auth=login or ?auth=register.
if ( ! is_user_logged_in() ) {
	$auth_param = isset( $_GET['auth'] ) ? sanitize_text_field( wp_unslash( $_GET['auth'] ) ) : '';
	$myaccount_dir = get_template_directory() . '/myaccount/';

	if ( 'login' === $auth_param && file_exists( $myaccount_dir . 'login.php' ) ) {
		require $myaccount_dir . 'login.php';
		return;
	}
	if ( 'register' === $auth_param && file_exists( $myaccount_dir . 'register.php' ) ) {
		require $myaccount_dir . 'register.php';
		return;
	}
}
...
elseif ( ! $is_logged_in && 'lost-password' === $endpoint ) :
	...
	// Logged out, no ?page param — default login form.
	// Redirect to the standalone login page for premium experience.
	$login_page = get_template_directory() . '/myaccount/login.php';
	if ( file_exists( $login_page ) ) {
		get_footer();
		return;
	}

	// Fallback: simple login/register forms in the AETHER frame.
					...
					<div class="checkout-form-wrap aether-wc">
						...
						<form class="woocommerce-form-login" method="post">
							...
							<div class="form-group">
								<label for="username"><?php esc_html_e( 'Email', 'aureon' ); ?> <span style="color:#ff6f61;">*</span></label>
							...
							<div class="form-group">
								<label for="password"><?php esc_html_e( 'Password', 'aureon' ); ?> <span style="color:#ff6f61;">*</span></label>
							...
							<div class="form-group">
								<label class="checkbox" style="cursor:pointer;">
									...
									<span style="text-transform:none;letter-spacing:0;"><?php esc_html_e( 'Remember me', 'aureon' ); ?></span>
								</label>
							</div>
							...
							<div style="margin-top:16px;font-size:0.85rem;">
								<a href="<?php echo esc_url( wc_lostpassword_url() ); ?>" style="color:#888;text-decoration:none;"><?php esc_html_e( 'Lost your password?', 'aureon' ); ?></a>
							</div>
					...
					<div class="col-lg-6">
						<div class="checkout-form-wrap aether-wc">
							...
							<form method="post" class="woocommerce-form-register">
								...
								<div class="form-group">
									<label for="reg_email"><?php esc_html_e( 'Email', 'aureon' ); ?> <span style="color:#ff6f61;">*</span></label>
								...
								<div class="form-group">
									<label for="reg_password"><?php esc_html_e( 'Password', 'aureon' ); ?> <span style="color:#ff6f61;">*</span></label>
