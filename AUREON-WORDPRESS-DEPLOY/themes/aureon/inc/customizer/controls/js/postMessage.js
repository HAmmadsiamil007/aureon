/* global aureonPostMessageFields */
/* eslint max-depth: off */
var aureonPostMessage = {
	...
	styleTag: {
		...
		add( id ) {
			...
			if ( null === document.getElementById( 'aureon-postmessage-' + id ) || 'undefined' === typeof document.getElementById( 'aureon-postmessage-' + id ) ) {
				jQuery( 'head' ).append( '<style id="aureon-postmessage-' + id + '"></style>' );
			}
		},
		...
		addData( id, styles ) {
			...
			aureonPostMessage.styleTag.add( id );
			jQuery( '#aureon-postmessage-' + id ).text( styles );
		},
	},
	...
	css: {
		...
		fromOutput( output, value, controlType ) {
			...
			// Apply the aureonPostMessageStylesOutput filter.
			styles = wp.hooks.applyFilters( 'aureonPostMessageStylesOutput', styles, value, output, controlType );
				...
				switch ( controlType ) {
					...
					case 'kirki-sortable':
						...
						_.each( value, function( val, key ) {
							...
							processedValue = aureonPostMessage.util.processValue( output, val );
					...
					default:
						if ( 'kirki-image' === controlType ) {
							value = ( ! _.isUndefined( value.url ) ) ? aureonPostMessage.util.backgroundImageValue( value.url ) : aureonPostMessage.util.backgroundImageValue( value );
						}
						if ( _.isObject( value ) ) {
							styles += output.element + '{';
							_.each( value, function( val, key ) {
							...
							processedValue = aureonPostMessage.util.processValue( output, value );
							if ( '' === processedValue ) {
								if ( 'background-color' === output.property ) {
									processedValue = 'unset';
								} else if ( 'background-image' === output.property ) {
	...
	html: {
		...
		fromOutput( output, value ) {
			...
			value = aureonPostMessage.util.processValue( output, value );
...
jQuery( document ).ready( function() {
	var styles;
	_.each( aureonPostMessageFields, function( field ) {
		wp.customize( field.settings, function( value ) {
			value.bind( function( newVal ) {
				styles = '';
				_.each( field.js_vars, function( output ) {
					output.function = ( ! output.function || 'undefined' === typeof aureonPostMessage[ output.function ] ) ? 'css' : output.function;
					field.type = ( field.choices && field.choices.parent_type ) ? field.choices.parent_type : field.type;

					if ( 'css' === output.function ) {
						styles += aureonPostMessage.css.fromOutput( output, newVal, field.type );
					} else {
						aureonPostMessage[ output.function ].fromOutput( output, newVal, field.type );
					}
				} );
				aureonPostMessage.styleTag.addData( field.settings, styles );
			} );
		} );
	} );
} );
