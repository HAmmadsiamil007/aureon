wp.customize.controlConstructor[ 'aureon-pro-range-slider' ] = wp.customize.Control.extend( {
	ready() {
		'use strict';

		var control = this,
			value,
			controlClass = '.customize-control-aureon-pro-range-slider',
			footerActions = jQuery( '#customize-footer-actions' );

		// Set up the sliders
		jQuery( '.aureon-slider' ).each( function() {
		...
		jQuery( controlClass + ' .aureon_range_value input[type=number]' ).on( 'input', function() {
			...
			jQuery( this ).closest( 'label' ).find( '.aureon-slider' ).slider( 'value', parseFloat( value ) ).change();
		} );

		// Handle the reset button
		jQuery( controlClass + ' .aureon-reset' ).on( 'click', function() {
			var icon = jQuery( this ),
				visibleArea = icon.closest( '.aureon-range-title-area' ).next( '.aureon-range-slider-areas' ).children( 'label:visible' ),
				input = visibleArea.find( 'input[type=number]' ),
				sliderValue = visibleArea.find( '.aureon-slider' ),
				visualValue = visibleArea.find( '.aureon_range_value' ),
		...
		jQuery( controlClass + ' .aureon-range-slider-control' ).each( function() {
			var _this = jQuery( this );
		...
		jQuery( controlClass + ' .aureon-device-controls > span' ).on( 'click', function( event ) {
			...
			jQuery( controlClass + ' .aureon-range-slider-areas label' ).each( function() {
				var _this = jQuery( this );
