 ...
 */
class Aureon_External_CSS_File {
	/**
	 * Instance.
	 *
	 * @access private
	...
	public function __construct() {
		...
		add_filter( 'aureon_option_defaults', array( $this, 'add_option_default' ) );
		add_filter( 'aureon_dynamic_css_print_method', array( $this, 'set_print_method' ) );

		if ( ! empty( $_POST ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Just checking, false positive.
			add_action( 'wp_ajax_aureon_regenerate_css_file', array( $this, 'regenerate_css_file' ) );
		}
	}
	...
	public function add_customizer_field( $wp_customize ) {
		...
		$wp_customize->add_setting(
			'aureon_settings[css_print_method]',
			array(
		...
		$wp_customize->add_control(
			...
			array(
				'type' => 'select',
				'label' => __( 'Dynamic CSS Print Method', 'aureon-studio' ),
				'description' => __( 'Generating your dynamic CSS in an external file offers significant performance advantages.', 'aureon-studio' ),
				'section' => 'aureon_general_section',
				'choices' => array(
					'inline' => __( 'Inline Embedding', 'aureon-studio' ),
					'file' => __( 'External File', 'aureon-studio' ),
				),
				'settings' => 'aureon_settings[css_print_method]',
			)
		);
		...
		$wp_customize->add_control(
			new Aureon_Action_Button_Control(
				$wp_customize,
				'aureon_regenerate_external_css_file',
				array(
					'section' => 'aureon_general_section',
					'data_type' => 'regenerate_external_css',
					'nonce' => esc_html( wp_create_nonce( 'aureon_regenerate_css_file' ) ),
					'label' => __( 'Regenerate CSS File', 'aureon-studio' ),
					'settings' => ( isset( $wp_customize->selective_refresh ) ) ? array() : 'blogname',
					'active_callback' => 'aureon_is_using_external_css_file_callback',
				)
			)
		);
	}
	...
	public function set_print_method( $method ) {
		...
		return aureon_get_option( 'css_print_method' );
	}
	...
	public function mode() {
		$mode = aureon_get_css_print_method();

		if ( 'file' === $mode && $this->needs_update() ) {
			$data = get_option( 'aureon_dynamic_css_data', array() );
			...
			if ( ! isset( $data['updated_time'] ) ) {
				...
				update_option( 'aureon_dynamic_css_data', $data );
			}
	...
	public function enqueue_dynamic_css() {
		if ( 'file' === $this->mode() ) {
			wp_enqueue_style( 'aureon-dynamic', esc_url( $this->file( 'uri' ) ), array( 'aureon-style' ), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion

			// Move the child theme after our dynamic stylesheet.
			if ( is_child_theme() && wp_style_is( 'aureon-child', 'enqueued' ) ) {
				wp_dequeue_style( 'aureon-child' );
			...
			if ( ! function_exists( 'aureon_get_dynamic_css' ) && function_exists( 'aureon_no_cache_dynamic_css' ) ) {
				...
				if ( function_exists( 'aureon_do_icon_css' ) ) {
					$nocache_css .= aureon_do_icon_css();
				}

				wp_add_inline_style( 'aureon-style', wp_strip_all_tags( $nocache_css ) );
			}
		}
	}
	...
	public function make_css() {
		...
		if ( function_exists( 'aureon_get_dynamic_css' ) ) {
			$content = aureon_get_dynamic_css();
		} elseif ( function_exists( 'aureon_base_css' ) && function_exists( 'aureon_font_css' ) && function_exists( 'aureon_advanced_css' ) && function_exists( 'aureon_spacing_css' ) ) {
			$content = aureon_base_css() . aureon_font_css() . aureon_advanced_css() . aureon_spacing_css();
		}

		$content = apply_filters( 'aureon_external_dynamic_css_output', $content );
	...
	public function can_write() {
		...
		$folder_path = $upload_dir['basedir'] . DIRECTORY_SEPARATOR . 'aureon';
	...
	public function file( $target = 'path' ) {
		...
		$css_uri = trailingslashit( $css_uri_folder ) . 'aureon/' . $file_name;
	...
	public function update_versions() {
		...
		$data['theme_version'] = AUREON_VERSION;
		$data['plugin_version'] = AUREON_STUDIO_VERSION;
	...
	public function needs_update() {
		...
		if ( (string) AUREON_VERSION !== (string) $data['theme_version'] || (string) AUREON_STUDIO_VERSION !== (string) $data['plugin_version'] ) {
			$update = true;
			$this->update_versions();
		}
	...
	public function regenerate_css_file() {
		check_ajax_referer( 'aureon_regenerate_css_file', '_nonce' );
...
Aureon_External_CSS_File::get_instance();
