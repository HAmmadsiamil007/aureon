 ...
 */
function aureon_premium_get_media_query( $name ) {
	if ( function_exists( 'aureon_get_media_query' ) ) {
		return aureon_get_media_query( $name );
	}
	...
	$tablet = apply_filters( 'aureon_tablet_media_query', '(min-width: 769px) and (max-width: 1024px)' );
 ...
 * @since 1.11.0
 */
function aureon_get_css_print_method() {
	$mode = apply_filters( 'aureon_dynamic_css_print_method', 'inline' );
 ...
 */
function aureon_has_active_element( $element, $block_element ) {
...
/**
 * Check our Aureon version.
 *
 * @since 2.1.0
 */
function aureon_premium_get_theme_version() {
	return defined( 'AUREON_VERSION' ) ? AUREON_VERSION : false;
}
 ...
 * @since 2.1.0
 */
function aureon_premium_remove_featured_image_class( $classes, $remove_class ) {
 ...
 */
function aureon_premium_get_wp_filesystem() {
 ...
 */
function aureon_premium_get_enqueue_assets(
	$filename = '',
	...
	$assets_file = AUREON_STUDIO_DIR_PATH . 'dist/' . $filename . '.asset.php';
