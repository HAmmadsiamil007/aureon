...
if ( ! function_exists( 'aureon_sections_page_template' ) ) {
	add_filter( 'template_include', 'aureon_sections_page_template' );
	/**
	 * Use our custom template if sections are enabled
	 */
	function aureon_sections_page_template( $template ) {
...
if ( ! function_exists( 'aureon_sections_show_excerpt' ) ) {
	add_filter( 'aureon_show_excerpt', 'aureon_sections_show_excerpt' );
	/**
	 * If Sections is enabled on a post, make sure we use the excerpt field on the blog page
	 */
	function aureon_sections_show_excerpt( $show_excerpt ) {
		global $post;
		$use_sections = ( isset( $post ) ) ? get_post_meta( $post->ID, '_aureon_use_sections', TRUE) : '';
...
if ( ! function_exists( 'aureon_sections_styles' ) ) {
	add_action( 'wp_enqueue_scripts', 'aureon_sections_styles' );
	/**
	 * Enqueue necessary scripts if sections are enabled
	 */
	function aureon_sections_styles() {
		...
		if ( isset( $use_sections['use_sections'] ) && 'true' == $use_sections['use_sections'] ) {
			wp_enqueue_style( 'aureon-sections-styles', plugin_dir_url( __FILE__ ) . 'css/style.min.css' );
			wp_enqueue_script( 'aureon-sections-parallax', plugin_dir_url( __FILE__ ) . 'js/parallax.min.js', array(), AUREON_SECTIONS_VERSION, true );
		}
	}
}

if ( ! function_exists( 'aureon_sections_body_classes' ) ) {
	add_filter( 'body_class', 'aureon_sections_body_classes' );
	/**
	 * Add classes to our <body> element when sections are enabled
	 */
	function aureon_sections_body_classes( $classes ) {
		global $post;
		$use_sections = ( isset( $post ) ) ? get_post_meta( $post->ID, '_aureon_use_sections', TRUE) : '';
...
if ( ! function_exists( 'aureon_sections_add_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'aureon_sections_add_css', 500 );
	/**
	 * Create the CSS for our sections
	 */
	function aureon_sections_add_css() {
		global $post;
		$use_sections = ( isset( $post ) ) ? get_post_meta( $post->ID, '_aureon_use_sections', TRUE) : '';
		...
		if ( $sections && '' !== $sections ) {
			...
			$css = '.aureon-sections-inside-container {padding-left:' . $left_padding . 'px;padding-right:' . $right_padding . 'px;}';
			...
			foreach ( $sections['sections'] as $section ) :
				...
				$padding_type = apply_filters( 'aureon_sections_padding_type','px' );
				...
				// Default padding top
				$padding_top = apply_filters( 'aureon_sections_default_padding_top','40' );

				// Default padding bottom
				$padding_bottom = apply_filters( 'aureon_sections_default_padding_bottom','40' );
				...
				if ( '' !== $background_color || '' !== $background_image ) {
					$css .= '#' . $custom_id . '.aureon-sections-container{' . $background_color . $background_image . '}';
				}

				// Inner container
				if ( '' !== $top_padding || '' !== $bottom_padding || '' !== $text_color ) {
					$css .= '#' . $custom_id . ' .aureon-sections-inside-container{' . $top_padding . $bottom_padding . $text_color . '}';
				}

				// Link color
				if ( '' !== $link_color ) {
					$css .= '#' . $custom_id . ' a,#aureon-section-' . $i . ' a:visited{' . $link_color . '}';
				}
				...
				$css .= '@media ' . esc_attr( $mobile ) . ' {.aureon-sections-inside-container {padding-left: ' . $mobile_padding_left . 'px;padding-right: ' . $mobile_padding_right . 'px;}}';
			endforeach;
...
if ( ! function_exists( 'aureon_sections_filter_admin_init' ) ) {
	add_action( 'admin_init', 'aureon_sections_filter_admin_init' );
	/*
	 * Recreate the default filters on the_content
	 * this will make it much easier to output the meta content with proper/expected formatting
	*/
	function aureon_sections_filter_admin_init() {
		if ( user_can_richedit() ) {
			add_filter( 'aureon_section_content', 'convert_smilies'    );
			add_filter( 'aureon_section_content', 'convert_chars'      );
			add_filter( 'aureon_section_content', 'wpautop'            );
			add_filter( 'aureon_section_content', 'shortcode_unautop'  );
			add_filter( 'aureon_section_content', 'prepend_attachment' );
		}
	}
}

if ( ! function_exists( 'aureon_sections_filter' ) ) {
	add_action( 'init', 'aureon_sections_filter' );
	/*
	 * Recreate the default filters on the_content
	 * this will make it much easier to output the meta content with proper/expected formatting
	*/
	function aureon_sections_filter() {
		if ( is_admin() ) {
			return;
		}
		...
		add_filter( 'aureon_section_content', 'shortcode_unautop'  );
		add_filter( 'aureon_section_content', 'prepend_attachment' );
		add_filter( 'aureon_section_content', 'do_shortcode');

		add_filter( 'aureon_the_section_content', array($GLOBALS['wp_embed'], 'autoembed'), 9 );
	}
}

if ( ! function_exists( 'aureon_sections_save_content' ) ) {
	add_action( 'save_post', 'aureon_sections_save_content', 99, 4 );
	/*
	...
	*/
	function aureon_sections_save_content( $post_id, $post ) {
		...
		$use_sections = get_post_meta( $post_id, '_aureon_use_sections', true);
		...
		// Get our sections
		$sections = get_post_meta( $post_id, '_aureon_sections', true );
	    ...
	    remove_action( 'save_post', 'aureon_sections_save_content', 99, 4 );
