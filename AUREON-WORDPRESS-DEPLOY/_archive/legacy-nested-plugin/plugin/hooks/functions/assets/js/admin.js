jQuery(document).ready(function($) {
	...
	jQuery('#hook-dropdown').on('change', function() {
		...
		jQuery('#aureon_hooks_settings .form-table tr').hide();
		jQuery('#aureon_hooks_settings .form-table tr').eq(id).show();
		Cookies.set('remember_hook', $('#hook-dropdown option:selected').attr('id'), { expires: 90, path: '/'});
		
		if ( jQuery('#hook-dropdown').val() == 'all' ) {
			$('#aureon_hooks_settings .form-table tr').show();
	...
	{	
		...
		$('#aureon_hooks_settings .form-table tr').hide();
		$('#aureon_hooks_settings .form-table tr').eq(Cookies.get('remember_hook')).show();
	}
