 ...
 * Font library class.
 */
class Aureon_Pro_Font_Library extends Aureon_Pro_Singleton {
	const FONT_LIBRARY_CPT = 'aureon_font';
	const FONTS_MAX_QUERY  = 100;
	const CSS_VAR_PREFIX   = '--aureon-font--';
	const SETTINGS_OPTION  = 'aureon_font_library_settings';
	...
	public function add_dashboard_tab( $tabs ) {
		...
		$tabs['Fonts'] = array(
			'name' => __( 'Font Library', 'aureon-studio' ),
			'url' => self::get_font_library_uri(),
			'class' => 'appearance_page_aureon-font-library' === $screen->id ? 'active' : '',
			'id' => 'aureon-font-library-tab',
		);
	...
	public function library_page() {
		echo '<div id="aureon-font-library" class="aureon-font-library aureon-studio"></div>';
	}
	...
	public function add_head_tags() {
		...
		if ( 'appearance_page_aureon-font-library' !== $screen->id || ! $google_gdpr ) {
			return;
		}

		echo '
			<link href="https://fonts.gstatic.com" rel="preconnect" crossorigin="anonymous" id="aureon-preconnect-gstatic">
			<link href="https://fonts.googleapis.com" rel="preconnect" id="aureon-preconnect-google-api">
			<link href="https://s.w.org" rel="preconnect" id="aureon-preconnect-wp-cdn">
		';
	}
	...
	public function enqueue_scripts() {
		...
		if ( 'appearance_page_aureon-font-library' === $screen->id ) {
			$assets     = aureon_premium_get_enqueue_assets( 'font-library' );
			$upload_dir = wp_get_upload_dir();

			wp_enqueue_script(
				'aureon-pro-font-library',
				AUREON_STUDIO_DIR_URL . 'dist/font-library.js',
				$assets['dependencies'],
				$assets['version'],
				true
			);
			...
			if ( function_exists( 'wp_set_script_translations' ) ) {
				wp_set_script_translations( 'aureon-pro-font-library', 'aureon-studio', AUREON_STUDIO_DIR_PATH . 'langs' );
			}
			...
			wp_enqueue_style(
				'aureon-pro-font-library',
				AUREON_STUDIO_DIR_URL . 'dist/font-library.css',
				array( 'wp-components' ),
	...
	public function add_dashboard_screen( $screens ) {
		$screens[] = 'appearance_page_aureon-font-library';
	...
	public static function get_fonts( $name = null ) {
		$args = array(
			...
			'post_status'            => 'any',
			'numberposts'            => Aureon_Pro_Font_Library::FONTS_MAX_QUERY, // phpcs:ignore
			'fields'                 => 'ids',
			'no_found_rows'          => true,
		...
		if ( is_array( $all_fonts ) ) {
			foreach ( $all_fonts as $font_post ) {
				...
				$alias           = get_post_meta( $font_post, 'aureon_font_family_alias', true );
				$slug            = get_post_field( 'post_name', $font_post );
				$status          = get_post_status( $font_post );
				$fallback        = get_post_meta( $font_post, 'aureon_font_fallback', true );
				$preview         = get_post_meta( $font_post, 'aureon_font_preview', true );
				...
				$response[] = array(
					...
					'slug'        => get_post_field( 'post_name', $font_post ),
					'alias'       => get_post_meta( $font_post, 'aureon_font_family_alias', true ),
					'variants'    => get_post_meta( $font_post, 'aureon_font_variants', true ),
					'source'      => get_post_meta( $font_post, 'aureon_font_source', true ),
					'fallback'    => $fallback,
					'fontDisplay' => get_post_meta( $font_post, 'aureon_font_display', true ),
					'preview'     => empty( $preview ) ? '' : $preview,
					'cssVariable' => get_post_meta( $font_post, 'aureon_font_variable', true ),
				);
			}
	...
	public static function get_font_library_uri() {
		return admin_url( 'themes.php?page=aureon-font-library' );
	}
	...
	public static function get_variant_name( $variant ) {
		...
		return str_replace( ' ', '-', $variant['fontWeight'] ) . ' ' . __( '(Variable)', 'aureon-studio' );
	}
	...
	public static function get_font_css_file( $type ) {
		$upload_dir = wp_get_upload_dir();
		$file_path  = 'aureon/fonts/fonts.css';
		$base       = '';
	...
	public function enqueue_font_css() {
		...
		if ( $font_file_url ) {
			$version = filemtime( self::get_font_css_file( 'path' ) ) ?? AUREON_STUDIO_VERSION;

			wp_enqueue_style( 'aureon-fonts', $font_file_url, array(), $version );
		}
	}
	...
	public static function handle_font_file_upload( $variant, $slug, $file ) {
		...
		if ( is_array( $file ) ) {
			...
			$set_upload_dir = function ( $font_dir ) use ( $base_path, $slug, $upload_dir ) {
				...
				$font_dir['url']  = untrailingslashit(
					trailingslashit( $upload_dir['baseurl'] ) . 'aureon/fonts/' . $slug
				);
				$font_dir['subdir'] = '';
				return $font_dir;
			};
		...
		if ( empty( $file_type['ext'] ) || empty( $file_type['type'] ) ) {
			return new WP_Error(
				...
				__( 'Sorry, you are not allowed to upload this file type.', 'aureon-studio' ),
				array( 'status' => 400 )
			);
		}
	...
	public function update_post_meta( $post_id, $key, $value ) {
		...
		if ( get_post_type( $post_id ) !== self::FONT_LIBRARY_CPT || 'aureon_font_variants' !== $key ) {
			return;
		}
		...
		update_post_meta( $post_id, 'aureon_font_variants', $variants );
	}
	...
	public static function build_css_file() {
		$generated_css = self::aureon_font_css();
		$upload_dir    = wp_get_upload_dir();

		// Save the generated font CSS to a file.
		$base_path_dir = trailingslashit( $upload_dir['basedir'] ) . 'aureon/fonts/';
		$file_path     = $base_path_dir . 'fonts.css';
		$filesystem    = aureon_premium_get_wp_filesystem();

		if ( ! $filesystem ) {
			return new WP_Error( 500, __( 'Error setting up the file system object.', 'aureon-studio' ) );
		}
		...
		if ( empty( $generated_css ) ) {
			...
			if ( is_writable( $file_path ) || is_writable( dirname( $file_path ) ) ) {
				if ( ! $filesystem->put_contents( $file_path, $generated_css, $chmod_file ) ) {
					return new WP_Error( 500, __( 'Failed to write Google font CSS to file.', 'aureon-studio' ) );
				}
			}
		}
	 ...
	 * @return mixed
	 */
	public static function aureon_font_css() {
		$fonts     = self::get_fonts();
		...
		return apply_filters( 'aureon_font_css', $css, $fonts );
	}
...
Aureon_Pro_Font_Library::get_instance()->init();
