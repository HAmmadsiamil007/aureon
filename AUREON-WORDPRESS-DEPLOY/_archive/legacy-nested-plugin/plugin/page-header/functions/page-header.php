...
if ( ! function_exists( 'aureon_get_blog_page_header' ) ) {
	/**
	 ...
	 */
	function aureon_get_blog_page_header() {
		$page_header = ( is_home() ) ? true : false;
		return apply_filters( 'aureon_get_blog_page_header', $page_header );
	}
}

if ( ! function_exists( 'aureon_page_header_area_start_container' ) ) {
	/**
	 ...
	 */
	function aureon_page_header_area_start_container( $image_class, $content_class ) {
		...
		$parallax_speed = apply_filters( 'aureon_page_header_parallax_speed', 2 );
		...
		if ( $video_enabled && '' !== $options[ 'content' ] ) {
			...
			$video_options[ 'className' ] = 'className:aureon-page-header-video';

			if ( apply_filters( 'aureon_page_header_video_loop', true ) ) {
				$video_options[ 'loop' ] = 'loop:true';
			} else {
				$video_options[ 'loop' ] = 'loop:false';
			}

			if ( apply_filters( 'aureon_page_header_video_muted', true ) ) {
		...
		$combined_content = ( '' !== $options[ 'merge' ] ) ? ' aureon-combined-page-header' : '';
		...
		if ( '' !== $options[ 'content' ] && false !== $options[ 'content' ] ) {
			printf(
				...
				$content_class . $parallax . $full_screen . $vertical_center_container . $container_type . $combined_content . ' aureon-page-header aureon-content-header',
				( 'fluid' !== $options[ 'container_type' ] ) ? $video : null,
				$vertical_center,
				( '' !== $options[ 'merge' ] ) ? 'aureon-merged-header' : '',
				( ! empty( $parallax ) ) ? 'data-parallax-speed="' . esc_attr( $parallax_speed ) . '"' : '',
				$options[ 'page_header_id' ]
			);
		}

		do_action( 'aureon_inside_merged_page_header' );
	}
}

if ( ! function_exists( 'aureon_page_header_area' ) ) {
	/**
	 ...
	 */
	function aureon_page_header_area( $image_class, $content_class ) {
		// Get our options
		$options = aureon_page_header_get_options();

		// Get out of here if we don't have content or an image
		if ( '' == $options[ 'content' ] && ! aureon_page_header_get_image( 'ALL' ) ) {
			return;
		}

		$inner_container = ( '' == $options[ 'inner_container' ] ) ? ' grid-container grid-parent' : '';

		do_action( 'aureon_before_page_header' );
		...
		if ( '' == $options[ 'content' ] && aureon_page_header_get_image( 'ALL' ) ) {
			...
			printf(
				...
				esc_attr( $image_class ) . $inner_container . ' aureon-page-header',
				( ! empty( $options[ 'image_link' ] ) ) ? '<a href="' . esc_url( $options[ 'image_link' ] ) . '"' . apply_filters( 'aureon_page_header_link_target','' ) . '>' : null,
				( ! empty( $options[ 'image_link' ] ) ) ? '</a>' : null,
				aureon_page_header_get_image_output()
			);
		...
		if ( '' !== $options[ 'content' ] && false !== $options[ 'content' ] ) {
			...
			// If we were merging, the container would be added in the aureon_before_header hook
			if ( '' == $options[ 'merge' ] ) {
				aureon_page_header_area_start_container( 'page-header-image', 'page-header-content' );
			}

			// Replace any found template tags
			$options[ 'content' ] = aureon_page_header_template_tags( $options[ 'content' ] );

			// Print the rest of our page header HTML
			// The starting elements are inside aureon_page_header_area_start_container()
			printf ( '<div class="page-header-content-wrapper %1$s %2$s">
				...
				( '' !== $options[ 'merge' ] ) ? 'aureon-combined-content' : '',
				$inner_container,
				( '' !== $options[ 'merge' ] ) ? 'aureon-inside-combined-content' : 'aureon-inside-page-header-content',
		...
		do_action( 'aureon_after_page_header' );
	}
}
