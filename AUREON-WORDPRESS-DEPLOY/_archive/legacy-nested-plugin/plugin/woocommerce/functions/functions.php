 ...
 * @since 1.3
 */
function aureon_wc_defaults() {
	return apply_filters( 'aureon_woocommerce_defaults', array(
		'cart_menu_item' => true,
...
add_filter( 'aureon_color_option_defaults', 'aureon_wc_color_defaults' );
/**
 * Set the WC color option defaults.
 *
 * @since 1.3
 */
function aureon_wc_color_defaults( $defaults ) {
...
add_filter( 'aureon_font_option_defaults', 'aureon_wc_typography_defaults' );
/**
 * Set the WC typography option defaults.
 *
 * @since 1.3
 */
function aureon_wc_typography_defaults( $defaults ) {
...
add_filter( 'aureon_navigation_class', 'aureon_wc_navigation_class' );
/**
 * Add navigation class when the menu icon is enabled.
 *
 * @since 1.3
 */
function aureon_wc_navigation_class( $classes ) {
	$classes[] = ( aureon_wc_get_setting( 'cart_menu_item' ) ) ? 'wc-menu-cart-activated' : '';
	return $classes;
}

add_filter( 'post_class', 'aureon_wc_post_class' );
add_filter( 'product_cat_class', 'aureon_wc_post_class' );
/**
 ...
 * @return array
 */
function aureon_wc_post_class( $classes ) {
	if ( 'product' == get_post_type() ) {
		$classes[] = ( aureon_wc_get_setting( 'product_archive_sale_flash_overlay' ) && aureon_wc_get_setting( 'product_archive_image' ) ) ? 'sales-flash-overlay' : '';
		$classes[] = 'woocommerce-text-align-' . aureon_wc_get_setting( 'product_archive_alignment' );

		if ( is_single() ) {
			$classes[] = 'wc-related-upsell-columns-' . aureon_wc_get_setting( 'related_upsell_columns' );
			$classes[] = 'wc-related-upsell-tablet-columns-' . aureon_wc_get_setting( 'tablet_related_upsell_columns' );
			$classes[] = 'wc-related-upsell-mobile-columns-' . aureon_wc_get_setting( 'mobile_related_upsell_columns' );
		} else {
			$classes[] = 'woocommerce-image-align-' . aureon_wc_get_setting( 'product_archive_image_alignment' );
		}
	}

	if ( 'product' === get_post_type() || is_cart() ) {
		if ( aureon_wc_get_setting( 'quantity_buttons' ) ) {
...
add_action( 'woocommerce_before_shop_loop', 'aureon_wc_before_shop_loop' );
/**
 * Add opening element inside shop page.
 *
 * @since 1.3
 */
function aureon_wc_before_shop_loop() {
	$classes = apply_filters( 'aureon_woocommerce_container_classes', array(
		'wc-columns-container',
		'wc-columns-' . aureon_wc_get_setting( 'columns' ),
		'wc-tablet-columns-' . aureon_wc_get_setting( 'tablet_columns' ),
		'wc-mobile-columns-' . aureon_wc_get_setting( 'mobile_columns' ),
	) );
...
add_action( 'woocommerce_after_shop_loop', 'aureon_wc_after_shop_loop' );
/**
 * Add closing element inside shop page.
 *
 * @since 1.3
 */
function aureon_wc_after_shop_loop() {
	echo '</div>';
}

add_action( 'wp_enqueue_scripts', 'aureon_wc_scripts', 100 );
/**
 * Add scripts and styles.
 *
 * @since 1.3
 */
function aureon_wc_scripts() {
	...
	if ( ! is_singular() && aureon_wc_get_setting( 'off_canvas_panel_on_add_to_cart' ) ) {
	...
	wp_localize_script(
		...
		array(
			...
			'addToCartPanel' => apply_filters( 'aureon_woocommerce_show_add_to_cart_panel', $show_add_to_cart_panel ),
		)
	);

	if ( aureon_wc_get_setting( 'distraction_free' ) && is_checkout() ) {
		wp_dequeue_script( 'aureon-advanced-sticky' );
		wp_dequeue_script( 'aureon-sticky' );
	}
 ...
 */
function aureon_wc_get_setting( $setting ) {
	$settings = wp_parse_args(
		get_option( 'aureon_woocommerce_settings', array() ),
		aureon_wc_defaults()
	);
...
add_filter( 'aureon_sidebar_layout', 'aureon_wc_sidebar_layout' );
/**
 * Set the WC sidebars.
 *
 * @since 1.3
 ...
 */
function aureon_wc_sidebar_layout( $layout ) {
	if ( function_exists( 'is_woocommerce' ) && is_woocommerce() ) {
		$layout = aureon_wc_get_setting( 'sidebar_layout' );

		if ( is_single() ) {
			if ( 'inherit' !== aureon_wc_get_setting( 'single_sidebar_layout' ) ) {
				$layout = aureon_wc_get_setting( 'single_sidebar_layout' );
			}

			if ( get_post_meta( get_the_ID(), '_aureon-sidebar-layout-meta', true ) ) {
				$layout = get_post_meta( get_the_ID(), '_aureon-sidebar-layout-meta', true );
			}
		}
	}
...
add_filter( 'loop_shop_columns', 'aureon_wc_product_columns', 999 );
/**
 * Set the WC column number.
 *
 * @since 1.3
 */
function aureon_wc_product_columns() {
	return aureon_wc_get_setting( 'columns' );
}

add_filter( 'loop_shop_per_page', 'aureon_wc_products_per_page', 20 );
/**
 * Set the WC products per page.
 *
 * @since 1.3
 */
function aureon_wc_products_per_page() {
	return aureon_wc_get_setting( 'products_per_page' );
}

add_action( 'wp', 'aureon_wc_setup' );
/**
 * Set up WC.
 *
 * @since 1.3
 */
function aureon_wc_setup() {
	...
	remove_action( 'wp_enqueue_scripts', 'aureon_woocommerce_css', 100 );

	remove_action( 'woocommerce_before_shop_loop', 'woocommerce_catalog_ordering', 30 );
	add_action( 'woocommerce_before_shop_loop', 'woocommerce_catalog_ordering', 10 );

	add_action( 'woocommerce_before_shop_loop_item_title', 'aureon_wc_image_wrapper_open', 8 );
	add_action( 'woocommerce_before_subcategory_title', 'aureon_wc_image_wrapper_open', 8 );
	add_action( 'woocommerce_shop_loop_item_title', 'aureon_wc_image_wrapper_close', 8 );
	add_action( 'woocommerce_before_subcategory_title', 'aureon_wc_image_wrapper_close', 20 );

	$archive_results_count       = aureon_wc_get_setting( 'product_results_count' );
	$archive_sorting             = aureon_wc_get_setting( 'product_sorting' );
	$archive_image               = aureon_wc_get_setting( 'product_archive_image' );
	$archive_sale_flash          = aureon_wc_get_setting( 'product_archive_sale_flash' );
	$archive_sale_flash_overlay  = aureon_wc_get_setting( 'product_archive_sale_flash_overlay' );
	$archive_rating              = aureon_wc_get_setting( 'product_archive_rating' );
	$archive_price               = aureon_wc_get_setting( 'product_archive_price' );
	$archive_add_to_cart         = aureon_wc_get_setting( 'product_archive_add_to_cart' );
	$archive_title               = aureon_wc_get_setting( 'product_archive_title' );
	$single_product_sale_flash   = aureon_wc_get_setting( 'single_product_sale_flash' );
	$product_tabs                = aureon_wc_get_setting( 'product_tabs' );
	$product_related             = aureon_wc_get_setting( 'product_related' );
	$product_upsells             = aureon_wc_get_setting( 'product_upsells' );
	$product_meta                = aureon_wc_get_setting( 'product_meta' );
	$product_description         = aureon_wc_get_setting( 'product_description' );
	$breadcrumbs                 = aureon_wc_get_setting( 'breadcrumbs' );
	$page_title                  = aureon_wc_get_setting( 'shop_page_title' );
	$distraction_free            = aureon_wc_get_setting( 'distraction_free' );
	$archive_description         = aureon_wc_get_setting( 'product_archive_description' );
	...
	if ( true === $distraction_free ) {
		add_filter( 'aureon_sidebar_layout', 'aureon_wc_checkout_sidebar_layout' );
		add_filter( 'aureon_footer_widgets', 'aureon_wc_checkout_footer_widgets' );
	}

	if ( true === $archive_description && ! is_single() && ! is_cart() ) {
		add_action( 'woocommerce_after_shop_loop_item_title', 'woocommerce_template_single_excerpt', 5 );
		add_action( 'woocommerce_after_subcategory_title', 'aureon_wc_category_description', 12 );
	}
}
 ...
 */
function aureon_wc_checkout_sidebar_layout( $layout ) {
	if ( ! function_exists( 'is_checkout' ) ) {
		return $layout;
	}
 ...
 */
function aureon_wc_checkout_footer_widgets( $widgets ) {
	if ( ! function_exists( 'is_checkout' ) ) {
		return $widgets;
	}
...
add_filter( 'wp_nav_menu_items', 'aureon_wc_menu_cart', 10, 2 );
/**
 ...
 */
function aureon_wc_menu_cart( $nav, $args ) {
	if ( function_exists( 'aureon_is_using_flexbox' ) && aureon_is_using_flexbox() ) {
		return $nav;
	}

	// If our primary menu is set, add the search icon.
	if ( apply_filters( 'aureon_woocommerce_menu_item_location', 'primary' ) === $args->theme_location && aureon_wc_get_setting( 'cart_menu_item' ) ) {
...
add_action( 'wp', 'aureon_wc_add_menu_bar_items' );
/**
 * Add to the menu bar items.
 *
 * @since 1.11.0
 */
function aureon_wc_add_menu_bar_items() {
	if ( function_exists( 'aureon_is_using_flexbox' ) && aureon_is_using_flexbox() ) {
		if ( 'secondary' === apply_filters( 'aureon_woocommerce_menu_item_location', 'primary' ) && aureon_wc_get_setting( 'cart_menu_item' ) ) {
			add_action( 'aureon_secondary_menu_bar_items', 'aureon_wc_do_cart_secondary_menu_item', 5 );
		}

		if ( 'primary' === apply_filters( 'aureon_woocommerce_menu_item_location', 'primary' ) && aureon_wc_get_setting( 'cart_menu_item' ) ) {
			add_action( 'aureon_menu_bar_items', 'aureon_wc_do_cart_menu_item', 5 );
		}
	}
}
 ...
 */
function aureon_wc_do_cart_secondary_menu_item() {
	if ( function_exists( 'aureon_is_using_flexbox' ) && aureon_is_using_flexbox() ) {
		if ( 'secondary' === apply_filters( 'aureon_woocommerce_menu_item_location', 'primary' ) && aureon_wc_get_setting( 'cart_menu_item' ) ) {
			if ( ! class_exists( 'WooCommerce' ) ) {
				return;
 ...
 * @since 1.11.0
 */
function aureon_wc_do_cart_menu_item() {
	if ( function_exists( 'aureon_is_using_flexbox' ) && aureon_is_using_flexbox() ) {
		if ( 'primary' === apply_filters( 'aureon_woocommerce_menu_item_location', 'primary' ) && aureon_wc_get_setting( 'cart_menu_item' ) ) {
			if ( ! class_exists( 'WooCommerce' ) ) {
				return;
 ...
 * @since 1.3
 */
function aureon_wc_cart_link() {
	// Kept for backward compatibility.
	$legacy_icon = apply_filters( 'aureon_woocommerce_menu_cart_icon', '' );

	// Get the icon type.
	$icon_type = aureon_wc_get_setting( 'cart_menu_item_icon' );

	$icon = '';

	if ( function_exists( 'aureon_get_svg_icon' ) ) {
		$icon = aureon_get_svg_icon( $icon_type );
	}
	...
	printf(
		...
		esc_attr( $icon_type ),
		$icon ? 'has-svg-icon' : '',
		esc_attr__( 'View your shopping cart', 'aureon-studio' ),
		$icon,
		sprintf(
	...
	if ( aureon_wc_get_setting( 'menu_mini_cart' ) && ! is_cart() ) : ?>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'aureon_wc_cart_link_fragment' );
/**
 ...
 * @return array
 */
function aureon_wc_cart_link_fragment( $fragments ) {
	global $woocommerce;
 ...
 * @since 1.3
 */
function aureon_wc_mobile_cart_link() {
	if ( function_exists( 'aureon_is_using_flexbox' ) && aureon_is_using_flexbox() ) {
		return;
	}

	if ( ! aureon_wc_get_setting( 'cart_menu_item' ) || 'primary' !== apply_filters( 'aureon_woocommerce_menu_item_location', 'primary' ) ) {
		return;
	}
	...
	<div class="mobile-bar-items wc-mobile-cart-items<?php echo $has_items; ?>">
		<?php do_action( 'aureon_mobile_cart_items' ); ?>
		<?php echo aureon_wc_cart_link(); ?>
	</div>
	<?php
}

add_action( 'wp', 'aureon_woocommerce_do_mobile_cart_link' );
/**
 * Add the mobile cart link to the menus.
 */
function aureon_woocommerce_do_mobile_cart_link() {
	if ( function_exists( 'aureon_is_using_flexbox' ) && aureon_is_using_flexbox() ) {
		add_action( 'aureon_after_mobile_menu_button', 'aureon_wc_mobile_cart_link' );
		add_action( 'aureon_after_mobile_header_menu_button', 'aureon_wc_mobile_cart_link' );
	} else {
		add_action( 'aureon_inside_navigation', 'aureon_wc_mobile_cart_link' );
		add_action( 'aureon_inside_mobile_header', 'aureon_wc_mobile_cart_link' );
	}
}

add_filter( 'woocommerce_output_related_products_args', 'aureon_wc_related_products_count' );
/**
 ...
 * @return array
 */
function aureon_wc_related_products_count( $args ) {
	$args['posts_per_page'] = aureon_wc_get_setting( 'related_upsell_columns' );
	$args['columns'] = aureon_wc_get_setting( 'related_upsell_columns' );
	return $args;
}
...
function aureon_wc_css() {
	...
	if ( $using_dynamic_typography && class_exists( 'Aureon_Typography' ) ) {
		...
		foreach ( (array) $typography as $key => $data ) {
			if ( 'buttons' === $data['selector'] ) {
				...
				if ( ! empty( $data['fontSize'] ) ) {
					...
					// @see https://github.com/tomusborne/aureon/pull/548.
				...
				if ( ! empty( $data['fontSizeTablet'] ) ) {
					$css->start_media_query( aureon_premium_get_media_query( 'tablet' ) );
	...
	$css->add_property( 'width', absint( aureon_wc_get_setting( 'single_product_image_width' ) ), false, '%' );

	if ( ! $using_dynamic_typography && function_exists( 'aureon_get_font_family_css' ) ) {
		$buttons_family = aureon_get_font_family_css( 'font_buttons', 'aureon_settings', aureon_get_default_fonts() );
	...
	$transparent_border_color = aureon_premium_check_text_color( $transparent_border_color );
	...
	if ( 'light' === $transparent_border_color ) {
		...
		if ( aureon_wc_get_setting( 'sticky_add_to_cart_panel' ) ) {
	...
	if ( 'number' === aureon_wc_get_setting( 'cart_menu_item_content' ) ) {
		$nav_text_color = aureon_premium_check_text_color( $settings['navigation_text_color'] );
	...
	if ( '' !== aureon_wc_get_setting( 'columns_gap' ) ) {
		$css->add_property( 'grid-gap', aureon_wc_get_setting( 'columns_gap' ), false, 'px' );
	}

	$css->start_media_query( aureon_premium_get_media_query( 'tablet' ) );
		$css->set_selector( '.wc-columns-container .products, .woocommerce .related ul.products, .woocommerce .up-sells ul.products' );

		if ( '' !== aureon_wc_get_setting( 'tablet_columns_gap' ) ) {
			$css->add_property( 'grid-gap', aureon_wc_get_setting( 'tablet_columns_gap' ), false, 'px' );
		}

		if ( 3 === aureon_wc_get_setting( 'tablet_columns' ) ) {
		...
		if ( 2 === aureon_wc_get_setting( 'tablet_columns' ) ) {
		...
		if ( 1 === aureon_wc_get_setting( 'tablet_columns' ) ) {
		...
		if ( 3 === aureon_wc_get_setting( 'tablet_related_upsell_columns' ) ) {
		...
		if ( 2 === aureon_wc_get_setting( 'tablet_related_upsell_columns' ) ) {
		...
		if ( 1 === aureon_wc_get_setting( 'tablet_related_upsell_columns' ) ) {
	...
	$css->start_media_query( aureon_premium_get_media_query( 'mobile' ) );
		...
		if ( '' !== aureon_wc_get_setting( 'mobile_columns_gap' ) ) {
			$css->add_property( 'grid-gap', aureon_wc_get_setting( 'mobile_columns_gap' ), false, 'px' );
		}
...
add_action( 'wp_enqueue_scripts', 'aureon_wc_enqueue_css', 100 );
/**
 * Enqueue our dynamic CSS.
 *
 * @since 1.3
 */
function aureon_wc_enqueue_css() {
	wp_add_inline_style( 'aureon-woocommerce', aureon_wc_css() );

	if ( class_exists( 'Aureon_Typography' ) ) {
		wp_add_inline_style( 'aureon-woocommerce', Aureon_Typography::get_css( 'woocommerce' ) );
	}
}
 ...
 * @since 1.3
 */
function aureon_wc_image_wrapper_open() {
	if ( aureon_wc_get_setting( 'product_archive_image' ) ) {
 ...
 * @since 1.3
 */
function aureon_wc_image_wrapper_close() {
	if ( aureon_wc_get_setting( 'product_archive_image' ) ) {
		echo '</div></div>';
	}
}

add_filter( 'post_class', 'aureon_wc_product_has_gallery' );
add_filter( 'product_cat_class', 'aureon_wc_product_has_gallery' );
/**
 ...
 */
function aureon_wc_product_has_gallery( $classes ) {
	...
	if ( 'product' === $post_type && method_exists( 'WC_Product', 'get_gallery_image_ids' ) ) {
		...
		if ( $attachment_ids && aureon_wc_get_setting( 'product_secondary_image' ) && aureon_wc_get_setting( 'product_archive_image' ) && has_post_thumbnail() ) {
...
add_action( 'woocommerce_before_shop_loop_item_title', 'aureon_wc_secondary_product_image' );
/**
 * Add secondary product image.
 *
 * @since 1.3
 */
function aureon_wc_secondary_product_image() {
	$post_type = get_post_type( get_the_ID() );
...
add_filter( 'woocommerce_product_get_rating_html', 'aureon_wc_rating_html', 10, 2 );
/**
 ...
 * @return string
 */
function aureon_wc_rating_html( $rating_html, $rating ) {
	if ( $rating > 0 ) {
		$title = sprintf( __( 'Rated %s out of 5', 'aureon-studio' ), $rating );
	} else {
		$title = __( 'Not yet rated', 'aureon-studio' );
		$rating = 0;
	}

	$rating_html  = '<div class="star-rating" title="' . esc_attr( $title ) . '">';
	$rating_html .= '<span style="width:' . ( ( $rating / 5 ) * 100 ) . '%"><strong class="rating">' . $rating . '</strong> ' . __( 'out of 5', 'aureon-studio' ) . '</span>';
	$rating_html .= '</div>';
 ...
 * @return string
 */
function aureon_wc_category_description( $category ) {
...
add_action( 'aureon_after_footer', 'aureon_wc_add_to_cart_helper' );
/**
 * Adds a sticky/slide down navigation with add to cart details.
 *
 * @since 1.8
 */
function aureon_wc_add_to_cart_helper() {
	if ( ! aureon_wc_get_setting( 'off_canvas_panel_on_add_to_cart' ) && ! aureon_wc_get_setting( 'sticky_add_to_cart_panel' ) ) {
		return;
	}
	...
	if ( function_exists( 'aureon_get_option' ) ) {
		...
		if ( 'contained' === aureon_get_option( 'nav_inner_width' ) ) {
	...
	?>
		<div id="wc-sticky-cart-panel" class="<?php echo implode( ' ', $outer_classes ); ?>">
			<div class="<?php echo implode( ' ', $inner_classes ); ?>">
				...
				<?php
				if ( aureon_wc_get_setting( 'off_canvas_panel_on_add_to_cart' ) && ! is_singular( 'product' ) ) :
					$svg_icon = '';
					...
					?>
						<div class="continue-shopping <?php echo $svg_icon ? 'has-svg-icon' : ''; ?>">
							<?php echo $svg_icon; ?>
							<a href="#" class="continue-shopping-link"><span class="continue-shopping-text"><?php _e( 'Continue Shopping', 'aureon-studio' ); ?> &rarr;</span></a>
						</div>

						<div class="cart-info">
							<div class="item-added">
								<?php _e( 'Item added to cart.', 'aureon-studio' ); ?>
							</div>

							<div class="cart-data">
								<?php
								if ( isset( WC()->cart ) ) {
									echo sprintf( _n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'aureon-studio' ), WC()->cart->get_cart_contents_count() ); ?> - <?php echo WC()->cart->get_cart_total();
								}
								?>
							</div>
						</div>
						...
						<?php
							...
							echo apply_filters(
								'aureon_wc_cart_panel_checkout_button_output',
								sprintf(
									'<div class="checkout">
										<a href="%s" class="button">%s</a>
									</div>',
									esc_url( wc_get_checkout_url() ),
									esc_html__( 'Checkout', 'aureon-studio' )
								)
							);
						?>
					<?php
				...
				if ( aureon_wc_show_sticky_add_to_cart() ) :
					$product = wc_get_product( get_the_ID() );
					$quantity_buttons = '';
					...
					?>
						...
						echo apply_filters( 'aureon_wc_sticky_add_to_cart_action', $action, $product ); // phpcs:ignore -- No escaping needed.
				endif;
				?>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'aureon_add_to_cart_panel_fragments', 10, 1 );
/**
 * Update cart totals in sticky add to cart panel.
 *
 * @since 1.8
 */
function aureon_add_to_cart_panel_fragments( $fragments ) {
	if ( isset( WC()->cart ) ) {
		$fragments['.add-to-cart-panel .cart-data'] = '<div class="cart-data">' . sprintf( _n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'aureon-studio' ), WC()->cart->get_cart_contents_count() ) . ' - ' .  WC()->cart->get_cart_total() . '</div>';
	}
 ...
 * @since 1.8
 */
function aureon_wc_show_sticky_add_to_cart() {
	...
	if ( ! $product || ! aureon_wc_get_setting( 'sticky_add_to_cart_panel' ) || ! is_singular( 'product' ) ) {
		return false;
	}
	...
	return apply_filters( 'aureon_wc_show_sticky_add_to_cart', $show );
}
 ...
 */
function aureon_premium_check_text_color( $color ) {
	// Get the hex value if we're using variables.
	if ( function_exists( 'aureon_get_option' ) && strpos( $color, 'var(' ) !== false ) {
		$global_colors = aureon_get_option( 'global_colors' );
		$found_color = false;
...
add_filter( 'aureon_typography_css_selector', 'aureon_woocommerce_typography_selectors' );
/**
 ...
 */
function aureon_woocommerce_typography_selectors( $selector ) {
	switch ( $selector ) {
