...
class Aureon_Site_Layout {
	...
	public function __construct( $post_id ) {
		...
		if ( get_post_meta( $post_id, '_aureon_sidebar_layout', true ) ) {
			$this->sidebar_layout = get_post_meta( $post_id, '_aureon_sidebar_layout', true );
		}

		if ( get_post_meta( $post_id, '_aureon_footer_widgets', true ) ) {
			$this->footer_widgets = get_post_meta( $post_id, '_aureon_footer_widgets', true );
		}

		if ( get_post_meta( $post_id, '_aureon_disable_site_header', true ) ) {
			$this->disable_site_header = get_post_meta( $post_id, '_aureon_disable_site_header', true );
		}

		if ( get_post_meta( $post_id, '_aureon_disable_mobile_header', true ) ) {
			$this->disable_mobile_header = get_post_meta( $post_id, '_aureon_disable_mobile_header', true );
		}

		if ( get_post_meta( $post_id, '_aureon_disable_top_bar', true ) ) {
			$this->disable_top_bar = get_post_meta( $post_id, '_aureon_disable_top_bar', true );
		}

		if ( get_post_meta( $post_id, '_aureon_disable_primary_navigation', true ) ) {
			$this->disable_primary_navigation = get_post_meta( $post_id, '_aureon_disable_primary_navigation', true );
		}

		if ( get_post_meta( $post_id, '_aureon_disable_secondary_navigation', true ) ) {
			$this->disable_secondary_navigation = get_post_meta( $post_id, '_aureon_disable_secondary_navigation', true );
		}

		if ( get_post_meta( $post_id, '_aureon_disable_featured_image', true ) ) {
			$this->disable_featured_image = get_post_meta( $post_id, '_aureon_disable_featured_image', true );
		}

		if ( get_post_meta( $post_id, '_aureon_disable_content_title', true ) ) {
			$this->disable_content_title = get_post_meta( $post_id, '_aureon_disable_content_title', true );
		}

		if ( get_post_meta( $post_id, '_aureon_disable_footer', true ) ) {
			$this->disable_footer = get_post_meta( $post_id, '_aureon_disable_footer', true );
		}

		if ( get_post_meta( $post_id, '_aureon_content_area', true ) ) {
			$this->content_area = get_post_meta( $post_id, '_aureon_content_area', true );
		}

		if ( get_post_meta( $post_id, '_aureon_content_width', true ) ) {
			$this->content_width = get_post_meta( $post_id, '_aureon_content_width', true );
		}

		$display = apply_filters( 'aureon_layout_element_display', Aureon_Conditions::show_data( $this->conditional, $this->exclude, $this->users ), $post_id );
	...
	public function after_setup() {
		if ( $this->sidebar_layout && ! self::post_meta_exists( '_aureon-sidebar-layout-meta' ) ) {
			add_filter( 'aureon_sidebar_layout', array( $this, 'filter_options' ) );
		}

		if ( $this->footer_widgets && ! self::post_meta_exists( '_aureon-footer-widget-meta' ) ) {
			add_filter( 'aureon_footer_widgets', array( $this, 'filter_options' ) );
		}
		...
		if ( $this->disable_footer ) {
			...
			add_filter( 'aureon_footer_widgets', '__return_null' );
		}
		...
		if ( is_admin() ) {
			if ( $this->sidebar_layout && ! self::admin_post_meta_exists( '_aureon-sidebar-layout-meta' ) ) {
				add_filter( 'aureon_block_editor_sidebar_layout', array( $this, 'filter_options' ) );
			}

			if ( $this->disable_content_title ) {
				add_filter( 'aureon_block_editor_show_content_title', '__return_false' );
			}

			if ( $this->content_area ) {
				add_filter( 'aureon_block_editor_content_area_type', array( $this, 'set_editor_content_area' ) );
			}

			if ( $this->content_width ) {
				add_filter( 'aureon_block_editor_container_width', array( $this, 'set_editor_content_width' ) );
			}
		}
	}
	...
	public function build_css() {
		if ( $this->content_width ) {
			wp_add_inline_style( 'aureon-style', '#content {max-width: ' . absint( $this->content_width ) . 'px;margin-left: auto;margin-right: auto;}' );
		}

		if ( is_admin() ) {
			$admin_css = '';

			if ( version_compare( aureon_premium_get_theme_version(), '3.2.0-alpha.1', '<' ) ) {
			...
			if ( $this->content_area ) {
				$admin_css .= '#aureon-layout-page-builder-container {opacity: 0.5;pointer-events: none;}';
			}
	...
	public static function post_meta_exists( $meta ) {
		...
		if ( '_aureon-footer-widget-meta' === $meta && '0' === $value ) {
			$value = true;
		}
	...
	public function filter_options() {
		...
		if ( 'aureon_sidebar_layout' === $filter || 'aureon_block_editor_sidebar_layout' === $filter ) {
			return $this->sidebar_layout;
		}

		if ( 'aureon_footer_widgets' === $filter ) {
	...
	public function remove_featured_image_class( $classes ) {
		if ( is_singular() ) {
			$classes = aureon_premium_remove_featured_image_class( $classes, $this->disable_featured_image );
		}
