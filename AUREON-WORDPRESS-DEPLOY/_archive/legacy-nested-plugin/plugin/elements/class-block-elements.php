...
class Aureon_Block_Elements {
	...
	public function __construct() {
		...
		add_action( 'generateblocks_block_css_data', array( $this, 'aureon_css' ), 10, 7 );
	...
	public function enqueue_assets() {
		...
		wp_enqueue_script(
			'aureon-studio-block-elements',
			AUREON_STUDIO_DIR_URL . 'dist/block-elements.js',
			array( 'wp-blocks', 'wp-i18n', 'wp-element', 'wp-editor' ),
			filemtime( AUREON_STUDIO_DIR_PATH . 'dist/block-elements.js' ),
			true
		);

		wp_set_script_translations( 'aureon-studio-block-elements', 'aureon-studio', AUREON_STUDIO_DIR_PATH . 'langs' );

		$taxonomies = get_taxonomies(
			apply_filters(
				'aureon_get_block_element_taxonomies_args',
				array(
					'public' => true,
				)
			)
		...
		$containerWidth = function_exists( 'aureon_get_option' ) ? aureon_get_option( 'container_width' ) : 1100;
		...
		wp_localize_script(
			...
			array(
				...
				'hooks' => Aureon_Elements_Helper::get_available_hooks(),
		...
		wp_enqueue_style(
			'aureon-studio-block-elements',
			AUREON_STUDIO_DIR_URL . 'dist/block-elements.css',
			array( 'wp-edit-blocks' ),
			filemtime( AUREON_STUDIO_DIR_PATH . 'dist/block-elements.css' )
		);
	}
	...
	public function do_excerpt_block( $attributes ) {
			...
			if ( isset( $attributes['useThemeMoreLink'] ) && ! $attributes['useThemeMoreLink'] ) {
				$filter_more_text = function() use ( $attributes ) {
					...
					return apply_filters(
						'aureon_excerpt_block_more_output',
						sprintf(
		...
		if ( 'aureon_elements' === get_post_type() || is_admin() ) {
			$post = get_posts(
				array(
					'post_type' => 'post',
					'numberposts' => 1,
			...
			if ( ! empty( $post[0] ) ) {
				...
				return sprintf(
					...
					__( 'This is a placeholder for your content.', 'aureon-studio' )
				);
			}
		}
	...
	public function do_dynamic_image_block( $attributes ) {
			...
			if ( $image_field_name ) {
				...
				if ( ctype_digit( $image ) ) {
					...
					$image_output = apply_filters(
						'aureon_dynamic_custom_field_image',
						sprintf(
		...
		if ( 'author-avatar' === $attributes['imageType'] ) {
			...
			$image_alt = apply_filters( 'aureon_dynamic_author_image_alt', __( 'Photo of author', 'aureon-studio' ) );
	...
	public function get_dynamic_url( $link_type, $source, $block ) {
		...
		return apply_filters( 'aureon_dynamic_element_url', $url, $link_type, $source, $block );
	}
		...
		if ( 'generateblocks/headline' === $block['blockName'] || 'generateblocks/button' === $block['blockName'] ) {
			if ( ! empty( $block['attrs']['gpDynamicTextType'] ) && ! empty( $block['attrs']['gpDynamicTextReplace'] ) ) {
				...
				if ( 'title' === $text_type ) {
					...
					$post_title = apply_filters( 'aureon_dynamic_element_text', $post_title, $block );
					...
					$post_title = apply_filters( 'aureon_dynamic_element_text_output', $post_title, $block );
				...
				if ( 'post-date' === $text_type ) {
					...
					$post_date = apply_filters( 'aureon_dynamic_element_text', $post_date, $block );
					...
					$post_date = apply_filters( 'aureon_dynamic_element_text_output', $post_date, $block );
				...
				if ( 'post-author' === $text_type ) {
					...
					$post_author = apply_filters( 'aureon_dynamic_element_text', $post_author, $block );
					...
					$post_author = apply_filters( 'aureon_dynamic_element_text_output', $post_author, $block );
				...
				if ( 'terms' === $text_type && 'generateblocks/headline' === $block['blockName'] ) {
					if ( ! empty( $block['attrs']['gpDynamicTextTaxonomy'] ) ) {
						...
						$term_output = apply_filters( 'aureon_dynamic_element_text_output', $term_output, $block );
				...
				if ( 'comments-number' === $text_type ) {
					if ( ! post_password_required( $id ) && ( comments_open( $id ) || get_comments_number( $id ) ) ) {
						if ( ! isset( $block['attrs']['gpDynamicNoCommentsText'] ) ) {
							$block['attrs']['gpDynamicNoCommentsText'] = __( 'No Comments', 'aureon-studio' );
						}
						...
						$comments_text = get_comments_number_text(
							...
							! empty( $block['attrs']['gpDynamicSingleCommentText'] ) ? $block['attrs']['gpDynamicSingleCommentText'] : __( '1 Comment', 'aureon-studio' ),
							! empty( $block['attrs']['gpDynamicMultipleCommentsText'] ) ? $block['attrs']['gpDynamicMultipleCommentsText'] : __( '% Comments', 'aureon-studio' )
						);

						$comments_text = apply_filters( 'aureon_dynamic_element_text', $comments_text, $block );
						...
						$comments_text = apply_filters( 'aureon_dynamic_element_text_output', $comments_text, $block );
				...
				if ( 'post-meta' === $text_type || 'term-meta' === $text_type || 'user-meta' === $text_type ) {
					if ( ! empty( $block['attrs']['gpDynamicTextCustomField'] ) ) {
						...
						$custom_field = apply_filters( 'aureon_dynamic_element_text', $custom_field, $block );
						...
						if ( $custom_field ) {
							...
							$custom_field = apply_filters( 'aureon_dynamic_element_text_output', $custom_field, $block );
	 ...
	 */
	public function aureon_css( $name, $settings, $css, $desktop_css, $tablet_css, $tablet_only_css, $mobile_css ) {
		if ( 'container' === $name ) {
	...
	public function get_source_id( $source, $attributes = array() ) {
		...
		return apply_filters( 'aureon_dynamic_element_source_id', $id, $source, $attributes );
	}
	...
	public function register_meta() {
		register_meta(
			'post',
			'_aureon_block_element_editor_width',
			array(
				'object_subtype' => 'aureon_elements',
				'type' => 'string',
				'show_in_rest' => true,
		...
		register_meta(
			'post',
			'_aureon_block_element_editor_width_unit',
			array(
				'object_subtype' => 'aureon_elements',
				'type' => 'string',
				'show_in_rest' => true,
		...
		register_meta(
			'post',
			'_aureon_post_loop_item_tagname',
			array(
				'object_subtype' => 'aureon_elements',
				'type' => 'string',
				'show_in_rest' => true,
		...
		register_meta(
			'post',
			'_aureon_disable_primary_post_meta',
			array(
				'object_subtype' => 'aureon_elements',
				'type' => 'boolean',
				'show_in_rest' => true,
		...
		register_meta(
			'post',
			'_aureon_disable_secondary_post_meta',
			array(
				'object_subtype' => 'aureon_elements',
				'type' => 'boolean',
				'show_in_rest' => true,
		...
		register_meta(
			'post',
			'_aureon_disable_featured_image',
			array(
				'object_subtype' => 'aureon_elements',
				'type' => 'boolean',
				'show_in_rest' => true,
		...
		register_meta(
			'post',
			'_aureon_use_theme_post_container',
			array(
				'object_subtype' => 'aureon_elements',
				'type' => 'boolean',
				'show_in_rest' => true,
		...
		register_meta(
			'post',
			'_aureon_use_archive_navigation_container',
			array(
				'object_subtype' => 'aureon_elements',
				'type' => 'boolean',
				'show_in_rest' => true,
		...
		register_meta(
			'post',
			'_aureon_disable_post_navigation',
			array(
				'object_subtype' => 'aureon_elements',
				'type' => 'boolean',
				'show_in_rest' => true,
		...
		register_meta(
			'post',
			'_aureon_disable_archive_navigation',
			array(
				'object_subtype' => 'aureon_elements',
				'type' => 'boolean',
				'show_in_rest' => true,
		...
		register_meta(
			'post',
			'_aureon_post_loop_item_display',
			array(
				'object_subtype' => 'aureon_elements',
				'type' => 'string',
				'show_in_rest' => true,
		...
		register_meta(
			'post',
			'_aureon_post_loop_item_display_tax',
			array(
				'object_subtype' => 'aureon_elements',
				'type' => 'string',
				'show_in_rest' => true,
		...
		register_meta(
			'post',
			'_aureon_post_loop_item_display_term',
			array(
				'object_subtype' => 'aureon_elements',
				'type' => 'string',
				'show_in_rest' => true,
		...
		register_meta(
			'post',
			'_aureon_post_loop_item_display_post_meta',
			array(
				'object_subtype' => 'aureon_elements',
				'type' => 'string',
				'show_in_rest' => true,
...
Aureon_Block_Elements::get_instance();
