...
class Aureon_Conditions {
	...
	public static function get_conditions() {
		$types = array(
			'general' => array(
				'label' => esc_attr__( 'General', 'aureon-studio' ),
				'locations' => array(
					'general:site'       => esc_attr__( 'Entire Site', 'aureon-studio' ),
					'general:front_page' => esc_attr__( 'Front Page', 'aureon-studio' ),
					'general:blog'       => esc_attr__( 'Blog', 'aureon-studio' ),
					'general:singular'   => esc_attr__( 'All Singular', 'aureon-studio' ),
					'general:archive'    => esc_attr__( 'All Archives', 'aureon-studio' ),
					'general:author'     => esc_attr__( 'Author Archives', 'aureon-studio' ),
					'general:date'       => esc_attr__( 'Date Archives', 'aureon-studio' ),
					'general:search'     => esc_attr__( 'Search Results', 'aureon-studio' ),
					'general:no_results' => esc_attr__( 'No Search Results', 'aureon-studio' ),
					'general:404'        => esc_attr__( '404 Template', 'aureon-studio' ),
					'general:is_paged'   => esc_attr__( 'Paginated Results', 'aureon-studio' ),
				),
			),
		);
		...
		foreach ( $post_types as $post_type_slug => $post_type ) {
			...
			$types[ $post_type_slug . '_archive' ] = array(
				...
				'label' => sprintf( esc_html_x( '%s Archives', '%s is a singular post type name', 'aureon-studio' ), $post_type->labels->singular_name ),
				'locations' => array(
					/* translators: post type name */
					'archive:' . $post_type_slug => sprintf( esc_html_x( '%s Archive', '%s is a singular post type name', 'aureon-studio' ), $post_type->labels->singular_name ),
				),
			);
			...
			foreach ( $taxonomies as $taxonomy_slug => $taxonomy ) {
				...
				if ( 'post_format' === $taxonomy_slug ) {
					continue;
				} elseif ( ! apply_filters( 'aureon_elements_show_taxonomy', $public, $taxonomy ) ) {
					continue;
				}
				...
				if ( isset( $types[ $post_type_slug . '_archive' ]['locations'] ) ) {
					...
					$types[ $post_type_slug . '_archive' ]['locations'][ 'taxonomy:' . $taxonomy_slug ] = sprintf( esc_html_x( '%1$s %2$s Archive', '%1$s is post type label. %2$s is taxonomy label.', 'aureon-studio' ), $post_type->labels->singular_name, $label );
				}
	...
	public static function get_user_conditions() {
		$rules = array(
			'general' => array(
				'label' => esc_attr__( 'General', 'aureon-studio' ),
				'rules' => array(
					'general:all'        => esc_attr__( 'All Users', 'aureon-studio' ),
					'general:logged_in'  => esc_attr__( 'Logged In', 'aureon-studio' ),
					'general:logged_out' => esc_attr__( 'Logged Out', 'aureon-studio' ),
				),
			),
			'role' => array(
				'label' => esc_attr__( 'Roles', 'aureon-studio' ),
				'rules' => array(),
			),
		);
...
Aureon_Conditions::get_instance();
