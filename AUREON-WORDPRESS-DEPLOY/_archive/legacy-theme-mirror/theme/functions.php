...
define( 'AUREON_VERSION', '3.6.1' );

if ( ! function_exists( 'aureon_setup' ) ) {
	add_action( 'after_setup_theme', 'aureon_setup' );
	/**
	 * Sets up theme defaults and registers support for various WordPress features.
	 *
	 * @since 0.1
	 */
	function aureon_setup() {
		...
		$color_palette = aureon_get_editor_color_palette();
		...
		register_nav_menus(
			array(
				'primary' => __( 'Primary Menu', 'aureon' ),
			)
		);

		/**
		 * Set the content width to something large
		 * We set a more accurate width in aureon_smart_content_width()
		 */
