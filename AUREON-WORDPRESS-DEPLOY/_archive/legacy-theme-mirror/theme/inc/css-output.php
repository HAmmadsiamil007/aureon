...
if ( ! function_exists( 'aureon_base_css' ) ) {
	/**
	 * Generate the CSS in the <head> section using the Theme Customizer.
	 *
	 * @since 0.1
	 */
	function aureon_base_css() {
		...
		$underline_links = aureon_get_option( 'underline_links' );
		...
		if ( apply_filters( 'aureon_do_group_inner_container_style', true ) ) {
		...
		$nav_drop_point = aureon_get_option( 'nav_drop_point' );
		$nav_location = aureon_get_navigation_location();

		if ( ( 'nav-float-right' === $nav_location || 'nav-float-left' === $nav_location ) && $nav_drop_point ) {
			$media_query = sprintf(
				'(max-width: %1$s) and %2$s',
				absint( $nav_drop_point ) . 'px',
				apply_filters( 'aureon_not_mobile_menu_media_query', '(min-width: 769px)' )
			);
			...
			$css->start_media_query( $media_query );
				...
				if ( 'nav-float-left' === aureon_get_option( 'nav_position_setting' ) ) {
		...
		if ( aureon_get_option( 'logo_width' ) ) {
			$css->set_selector( '.site-header .header-image' );
			$css->add_property( 'width', absint( aureon_get_option( 'logo_width' ) ), false, 'px' );
		}

		if ( aureon_get_option( 'back_to_top' ) ) {
			$css->set_selector( '.aureon-back-to-top' );
			...
			$css->set_selector( '.aureon-back-to-top__show' );
		...
		if ( 'click' === aureon_get_option( 'nav_dropdown_type' ) || 'click-arrow' === aureon_get_option( 'nav_dropdown_type' ) ) {
			...
			$css->set_selector( '.dropdown-click .sfHover > a > .dropdown-menu-toggle > .aureon-icon svg' );
		...
		do_action( 'aureon_base_css', $css );

		return apply_filters( 'aureon_base_css_output', $css->css_output() );
	}
}

if ( ! function_exists( 'aureon_advanced_css' ) ) {
	/**
	 * Generate the CSS in the <head> section using the Theme Customizer.
	 *
	 * @since 0.1
	 */
	function aureon_advanced_css() {
		$settings = wp_parse_args(
			get_option( 'aureon_settings', array() ),
		...
		$css->set_selector( 'a.aureon-back-to-top' );
		...
		$css->set_selector( 'a.aureon-back-to-top:hover,a.aureon-back-to-top:focus' );
		...
		$css->set_selector( ':root' );
		$css->add_property( '--aureon-search-modal-bg-color', $settings['search_modal_bg_color'] );
		$css->add_property( '--aureon-search-modal-text-color', $settings['search_modal_text_color'] );
		$css->add_property( '--aureon-search-modal-overlay-bg-color', $settings['search_modal_overlay_bg_color'] );

		$css->start_media_query( aureon_get_media_query( 'mobile-menu' ) );
		...
		do_action( 'aureon_colors_css', $css );

		return apply_filters( 'aureon_colors_css_output', $css->css_output() );
	}
}

if ( ! function_exists( 'aureon_font_css' ) ) {
	/**
	 * Generate the CSS in the <head> section using the Theme Customizer.
	 *
	 * @since 0.1
	 */
	function aureon_font_css() {
		...
		$defaults = aureon_get_default_fonts( false );

		$css = new Aureon_CSS();

		$body_family = aureon_get_font_family_css( 'font_body', 'aureon_settings', aureon_get_default_fonts() );
		$top_bar_family = aureon_get_font_family_css( 'font_top_bar', 'aureon_settings', aureon_get_default_fonts() );
		$site_title_family = aureon_get_font_family_css( 'font_site_title', 'aureon_settings', aureon_get_default_fonts() );
		$site_tagline_family = aureon_get_font_family_css( 'font_site_tagline', 'aureon_settings', aureon_get_default_fonts() );
		$navigation_family = aureon_get_font_family_css( 'font_navigation', 'aureon_settings', aureon_get_default_fonts() );
		$widget_family = aureon_get_font_family_css( 'font_widget_title', 'aureon_settings', aureon_get_default_fonts() );
		$h1_family = aureon_get_font_family_css( 'font_heading_1', 'aureon_settings', aureon_get_default_fonts() );
		$h2_family = aureon_get_font_family_css( 'font_heading_2', 'aureon_settings', aureon_get_default_fonts() );
		$h3_family = aureon_get_font_family_css( 'font_heading_3', 'aureon_settings', aureon_get_default_fonts() );
		$h4_family = aureon_get_font_family_css( 'font_heading_4', 'aureon_settings', aureon_get_default_fonts() );
		$h5_family = aureon_get_font_family_css( 'font_heading_5', 'aureon_settings', aureon_get_default_fonts() );
		$h6_family = aureon_get_font_family_css( 'font_heading_6', 'aureon_settings', aureon_get_default_fonts() );
		$footer_family = aureon_get_font_family_css( 'font_footer', 'aureon_settings', aureon_get_default_fonts() );
		...
		if ( apply_filters( 'aureon_do_wp_block_margin_bottom', true ) ) {
		...
		$css->start_media_query( aureon_get_media_query( 'mobile' ) );
		$css->set_selector( '.main-title' );
		...
		do_action( 'aureon_typography_css', $css );

		return apply_filters( 'aureon_typography_css_output', $css->css_output() );
	}
}

if ( ! function_exists( 'aureon_spacing_css' ) ) {
	/**
	 * Write our dynamic CSS.
	 *
	 * @since 0.1
	 */
	function aureon_spacing_css() {
		$settings = wp_parse_args(
			get_option( 'aureon_spacing_settings', array() ),
			aureon_spacing_get_defaults()
		);

		$defaults = aureon_spacing_get_defaults( false );
		$sidebar_layout = aureon_get_layout();

		$css = new Aureon_CSS();

		$css->set_selector( '.inside-top-bar' );
		$css->add_property( 'padding', aureon_padding_css( $settings['top_bar_top'], $settings['top_bar_right'], $settings['top_bar_bottom'], $settings['top_bar_left'] ), aureon_padding_css( $defaults['top_bar_top'], $defaults['top_bar_right'], $defaults['top_bar_bottom'], $defaults['top_bar_left'] ) );

		if ( aureon_is_using_flexbox() && 'boxes' === aureon_get_option( 'container_alignment' ) ) {
			$top_bar_padding = absint( $settings['top_bar_right'] ) + absint( $settings['top_bar_left'] );

			$css->set_selector( '.inside-top-bar.grid-container' );
			$css->add_property( 'max-width', aureon_get_option( 'container_width' ) + $top_bar_padding, false, 'px' );
		}

		$css->set_selector( '.inside-header' );
		$css->add_property( 'padding', aureon_padding_css( $settings['header_top'], $settings['header_right'], $settings['header_bottom'], $settings['header_left'] ), aureon_padding_css( $defaults['header_top'], $defaults['header_right'], $defaults['header_bottom'], $defaults['header_left'] ) );

		if ( aureon_is_using_flexbox() ) {
			if ( 'boxes' === aureon_get_option( 'container_alignment' ) ) {
				$header_padding = absint( $settings['header_right'] ) + absint( $settings['header_left'] );

				$css->set_selector( '.inside-header.grid-container' );
				$css->add_property( 'max-width', aureon_get_option( 'container_width' ) + $header_padding, false, 'px' );
			}
		...
		$css->add_property( 'padding', aureon_padding_css( $settings['content_top'], $settings['content_right'], $settings['content_bottom'], $settings['content_left'] ), aureon_padding_css( $defaults['content_top'], $defaults['content_right'], $defaults['content_bottom'], $defaults['content_left'] ) );

		if ( apply_filters( 'aureon_do_group_inner_container_style', true ) ) {
			$css->set_selector( '.site-main .wp-block-group__inner-container' );
			$css->add_property( 'padding', aureon_padding_css( $settings['content_top'], $settings['content_right'], $settings['content_bottom'], $settings['content_left'] ) );
		}
		...
		if ( ! aureon_is_using_flexbox() && 'text' === aureon_get_option( 'container_alignment' ) ) {
			$css->set_selector( '.container.grid-container' );
			$css->add_property( 'max-width', aureon_get_option( 'container_width' ) + $content_padding, false, 'px' );
		}
		...
		$css->add_property( 'margin', aureon_padding_css( '0', $settings['content_right'], '0', $settings['content_left'] ), aureon_padding_css( '0', $defaults['content_right'], '0', $defaults['content_left'] ) );
		...
		$css->set_selector( '.main-navigation .main-nav ul ul li a' );
		$css->add_property( 'padding', aureon_padding_css( $settings['sub_menu_item_height'], $settings['menu_item'], $settings['sub_menu_item_height'], $settings['menu_item'] ), aureon_padding_css( $defaults['sub_menu_item_height'], $defaults['menu_item'], $defaults['sub_menu_item_height'], $defaults['menu_item'] ) );
		...
		$css->add_property( 'padding', aureon_padding_css( $settings['widget_top'], $settings['widget_right'], $settings['widget_bottom'], $settings['widget_left'] ), aureon_padding_css( $defaults['widget_top'], $defaults['widget_right'], $defaults['widget_bottom'], $defaults['widget_left'] ) );

		if ( aureon_is_using_flexbox() ) {
			$css->set_selector( '.footer-widgets-container' );
			$css->add_property( 'padding', aureon_padding_css( $settings['footer_widget_container_top'], $settings['footer_widget_container_right'], $settings['footer_widget_container_bottom'], $settings['footer_widget_container_left'] ), aureon_padding_css( $defaults['footer_widget_container_top'], $defaults['footer_widget_container_right'], $defaults['footer_widget_container_bottom'], $defaults['footer_widget_container_left'] ) );
			...
			if ( 'boxes' === aureon_get_option( 'container_alignment' ) ) {
				...
				$css->add_property( 'max-width', aureon_get_option( 'container_width' ) + $footer_widgets_padding, false, 'px' );
			}
		} else {
			$css->set_selector( '.footer-widgets' );
			$css->add_property( 'padding', aureon_padding_css( $settings['footer_widget_container_top'], $settings['footer_widget_container_right'], $settings['footer_widget_container_bottom'], $settings['footer_widget_container_left'] ), aureon_padding_css( $defaults['footer_widget_container_top'], $defaults['footer_widget_container_right'], $defaults['footer_widget_container_bottom'], $defaults['footer_widget_container_left'] ) );
		...
		$css->add_property( 'padding', aureon_padding_css( '0', '0', '0', $settings['footer_widget_separator'] ), aureon_padding_css( '0', '0', '0', $defaults['footer_widget_separator'] ) );
		...
		if ( aureon_is_using_flexbox() ) {
			...
			$css->add_property( 'padding', aureon_padding_css( $settings['footer_top'], $settings['footer_right'], $settings['footer_bottom'], $settings['footer_left'] ), aureon_padding_css( $defaults['footer_top'], $defaults['footer_right'], $defaults['footer_bottom'], $defaults['footer_left'] ) );
			...
			if ( 'boxes' === aureon_get_option( 'container_alignment' ) ) {
				...
				$css->add_property( 'max-width', aureon_get_option( 'container_width' ) + $site_info_padding, false, 'px' );
			}
		} else {
			$css->set_selector( '.site-info' );
			$css->add_property( 'padding', aureon_padding_css( $settings['footer_top'], $settings['footer_right'], $settings['footer_bottom'], $settings['footer_left'] ), aureon_padding_css( $defaults['footer_top'], $defaults['footer_right'], $defaults['footer_bottom'], $defaults['footer_left'] ) );
		...
		$css->start_media_query( aureon_get_media_query( 'mobile' ) );
		$css->set_selector( '.separate-containers .inside-article, .separate-containers .comments-area, .separate-containers .page-header, .separate-containers .paging-navigation, .one-container .site-content, .inside-page-header' );
		$css->add_property( 'padding', aureon_padding_css( $settings['mobile_content_top'], $settings['mobile_content_right'], $settings['mobile_content_bottom'], $settings['mobile_content_left'] ) );
		...
		if ( ! aureon_is_using_flexbox() ) {
			...
			$sidebar = strtoupper( $aureon_settings['sidebar_widget_background_color'] );
			$content = strtoupper( $aureon_settings['content_background_color'] );
		...
		do_action( 'aureon_spacing_css', $css );

		return apply_filters( 'aureon_spacing_css_output', $css->css_output() . $output );
	}
}
 ...
 * @since 2.0
 */
function aureon_no_cache_dynamic_css() {
	$css = new Aureon_CSS();

	if ( aureon_is_using_flexbox() ) {
		$right_sidebar_width = apply_filters( 'aureon_right_sidebar_width', '30' );
		$left_sidebar_width = apply_filters( 'aureon_left_sidebar_width', '30' );
 ...
 * @since 3.0.0
 */
function aureon_get_dynamic_css() {
	if ( aureon_is_using_dynamic_typography() ) {
		$typography_css = Aureon_Typography::get_css();
	} else {
		$typography_css = aureon_font_css();
	}

	$css = aureon_base_css() . $typography_css . aureon_advanced_css() . aureon_spacing_css();

	return apply_filters( 'aureon_dynamic_css', $css );
}

add_action( 'wp_enqueue_scripts', 'aureon_enqueue_dynamic_css', 50 );
/**
 * Enqueue our dynamic CSS.
 *
 * @since 2.0
 */
function aureon_enqueue_dynamic_css() {
	if ( apply_filters( 'aureon_using_dynamic_css_external_file', false ) ) {
		$css = '';
	} elseif ( ! get_option( 'aureon_dynamic_css_output', false ) || is_customize_preview() || apply_filters( 'aureon_dynamic_css_skip_cache', false ) ) {
		$css = aureon_get_dynamic_css();
	} else {
		$css = get_option( 'aureon_dynamic_css_output' ) . '/* End cached CSS */';
	}

	$css = $css . aureon_no_cache_dynamic_css();

	wp_add_inline_style( 'aureon-style', wp_strip_all_tags( $css ) );
}

add_action( 'init', 'aureon_set_dynamic_css_cache' );
/**
 ...
 * @since 2.0
 */
function aureon_set_dynamic_css_cache() {
	if ( apply_filters( 'aureon_dynamic_css_skip_cache', false ) ) {
		return;
	}

	$cached_css = get_option( 'aureon_dynamic_css_output', false );
	$cached_version = get_option( 'aureon_dynamic_css_cached_version', '' );

	if ( ! $cached_css || AUREON_VERSION !== $cached_version ) {
		$css = aureon_get_dynamic_css();

		update_option( 'aureon_dynamic_css_output', wp_strip_all_tags( $css ) );
		update_option( 'aureon_dynamic_css_cached_version', esc_html( AUREON_VERSION ) );
	}
}

add_action( 'customize_save_after', 'aureon_update_dynamic_css_cache' );
/**
 * Update our CSS cache when done saving Customizer options.
 *
 * @since 2.0
 */
function aureon_update_dynamic_css_cache() {
	if ( apply_filters( 'aureon_dynamic_css_skip_cache', false ) ) {
		return;
	}
...
add_action( 'aureon_base_css', 'aureon_do_modal_css' );
/**
 * Do the modal CSS.
 *
 * @param Object $css The existing CSS object.
 */
function aureon_do_modal_css( $css ) {
	if ( ! apply_filters( 'aureon_enable_modal_script', false ) ) {
		return;
	}

	$css->set_selector( '.aureon-modal:not(.aureon-modal--open):not(.aureon-modal--transition)' );
	$css->add_property( 'display', 'none' );

	$css->set_selector( '.aureon-modal--transition:not(.aureon-modal--open)' );
	$css->add_property( 'pointer-events', 'none' );

	$css->set_selector( '.aureon-modal-overlay:not(.aureon-modal-overlay--open):not(.aureon-modal--transition)' );
	$css->add_property( 'display', 'none' );

	$css->set_selector( '.aureon-modal__overlay' );
	$css->add_property( 'display', 'none' );
	$css->add_property( 'position', 'fixed' );
	$css->add_property( 'top', '0' );
	$css->add_property( 'left', '0' );
	...
	$css->set_selector( '.aureon-modal--open:not(.aureon-modal--transition) .aureon-modal__overlay' );
	$css->add_property( 'opacity', 1 );

	$css->set_selector( '.aureon-modal__container' );
	...
	$css->set_selector( '.aureon-modal--open:not(.aureon-modal--transition) .aureon-modal__container' );
