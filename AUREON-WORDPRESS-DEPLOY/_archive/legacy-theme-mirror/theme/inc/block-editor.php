<?php
/**
 * Integrate Aureon with the WordPress block editor.
 *
 * @package Aureon
 */
 ...
 * We need this function as the post meta in aureon_get_layout() only runs
 * on is_singular()
 *
 * @since 2.2
 *
 ...
 */
function aureon_get_block_editor_sidebar_layout( $meta = true ) {
	...
	$layout = apply_filters( 'aureon_sidebar_layout', $layout );
	...
	return apply_filters( 'aureon_block_editor_sidebar_layout', $layout );
}

/**
 * Check whether we're disabling the content title or not.
 * We need this function as the post meta in aureon_show_title() only runs
 * on is_singular()
 *
 * @since 2.2
 */
function aureon_get_block_editor_show_content_title() {
	$title = aureon_show_title();
	...
	return apply_filters( 'aureon_block_editor_show_content_title', $title );
}
 ...
 * @since 2.2
 */
function aureon_get_block_editor_content_width() {
	...
	$layout = aureon_get_block_editor_sidebar_layout();
	...
	return apply_filters( 'aureon_block_editor_content_width', $content_width );
}

add_filter( 'block_editor_settings_all', 'aureon_add_inline_block_editor_styles' );
/**
 * Add dynamic inline styles to the block editor content.
 *
 * @param array $editor_settings The existing editor settings.
 */
function aureon_add_inline_block_editor_styles( $editor_settings ) {
	$show_editor_styles = apply_filters( 'aureon_show_block_editor_styles', true );

	if ( $show_editor_styles ) {
		if ( aureon_is_using_dynamic_typography() ) {
			$google_fonts_uri = Aureon_Typography::get_google_fonts_uri();
		...
		$editor_settings['styles'][] = array( 'css' => wp_strip_all_tags( aureon_do_inline_block_editor_css() ) );

		if ( aureon_is_using_dynamic_typography() ) {
			$editor_settings['styles'][] = array( 'css' => wp_strip_all_tags( Aureon_Typography::get_css( 'core' ) ) );
		}
	}
...
add_action( 'enqueue_block_editor_assets', 'aureon_enqueue_google_fonts' );
add_action( 'enqueue_block_editor_assets', 'aureon_enqueue_backend_block_editor_assets' );
/**
 * Add CSS to the admin side of the block editor.
 *
 * @since 2.2
 */
function aureon_enqueue_backend_block_editor_assets() {
	wp_enqueue_script(
		'aureon-block-editor',
	...
	$text_color = aureon_get_option( 'text_color' );

	if ( $color_settings['content_text_color'] ) {
		$text_color = $color_settings['content_text_color'];
	}

	$sidebar_layout = get_post_meta( get_the_ID(), '_aureon_sidebar_layout', true );
	$content_area_type = get_post_meta( get_the_ID(), '_aureon-full-width-content', true );

	wp_localize_script(
		'aureon-block-editor',
		'aureonBlockEditor',
		array(
			'sidebarLayout' => $sidebar_layout ? $sidebar_layout : aureon_get_block_editor_sidebar_layout( false ),
			'containerWidth' => aureon_get_option( 'container_width' ),
			'contentPaddingRight' => absint( $spacing_settings['content_right'] ) . 'px',
			'contentPaddingLeft' => absint( $spacing_settings['content_left'] ) . 'px',
			'rightSidebarWidth' => apply_filters( 'aureon_right_sidebar_width', '25' ),
			'leftSidebarWidth' => apply_filters( 'aureon_left_sidebar_width', '25' ),
			'text_color' => $text_color,
			'show_editor_styles' => apply_filters( 'aureon_show_block_editor_styles', true ),
			'contentAreaType' => $content_area_type ? $content_area_type : apply_filters( 'aureon_block_editor_content_area_type', '' ),
			'customContentWidth' => apply_filters( 'aureon_block_editor_container_width', '' ),
		)
	);

	wp_register_style( 'aureon-block-editor', false, array(), true, true );
	wp_add_inline_style( 'aureon-block-editor', aureon_do_inline_block_editor_css( 'block-editor' ) );
	wp_enqueue_style( 'aureon-block-editor' );
}
 ...
 */
function aureon_do_inline_block_editor_css( $for = 'block-content' ) {
	$css = new Aureon_CSS();
	...
	$content_width = aureon_get_block_editor_content_width();
	...
	$css->add_property(
		'--content-width',
		'true' === get_post_meta( get_the_ID(), '_aureon-full-width-content', true )
			? '100%'
			: $content_width_calc
	);
	...
	if ( apply_filters( 'aureon_do_group_inner_container_style', true ) ) {
		...
		$css->add_property( 'padding', aureon_padding_css( $spacing_settings['content_top'], $spacing_settings['content_right'], $spacing_settings['content_bottom'], $spacing_settings['content_left'] ) );
	}
	...
	if ( $color_settings['content_text_color'] ) {
		...
		$css->add_property( 'color', aureon_get_option( 'text_color' ) );
	}
	...
	if ( version_compare( $GLOBALS['wp_version'], '5.7-alpha.1', '>' ) ) {
		...
		$css->add_property( 'background-color', aureon_get_option( 'background_color' ) );
		...
		if ( $color_settings['content_background_color'] ) {
			$body_background = aureon_get_option( 'background_color' );
	...
	if ( $color_settings['content_link_color'] ) {
		...
		$css->add_property( 'color', aureon_get_option( 'link_color' ) );
	}
	...
	if ( $color_settings['content_link_hover_color'] ) {
		...
		$css->add_property( 'color', aureon_get_option( 'link_color_hover' ) );
	}
