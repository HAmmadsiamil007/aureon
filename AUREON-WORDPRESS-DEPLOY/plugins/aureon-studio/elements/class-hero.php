...
class Aureon_Hero {
	...
	public function __construct( $post_id ) {
		...
		if ( get_post_meta( $post_id, '_aureon_element_display_conditions', true ) ) {
			$this->conditional = get_post_meta( $post_id, '_aureon_element_display_conditions', true );
		}

		if ( get_post_meta( $post_id, '_aureon_element_exclude_conditions', true ) ) {
			$this->exclude = get_post_meta( $post_id, '_aureon_element_exclude_conditions', true );
		}

		if ( get_post_meta( $post_id, '_aureon_element_user_conditions', true ) ) {
			$this->users = get_post_meta( $post_id, '_aureon_element_user_conditions', true );
		}

		$display = apply_filters( 'aureon_header_element_display', Aureon_Conditions::show_data( $this->conditional, $this->exclude, $this->users ), $post_id );
		...
		if ( $display ) {
			...
			$location = apply_filters( 'aureon_page_hero_location', 'aureon_after_header', $post_id );
	...
	public function enqueue() {
		...
		wp_add_inline_style( 'aureon-style', self::build_css() );

		if ( $options['parallax'] ) {
			wp_enqueue_script( 'aureon-hero-parallax', plugin_dir_url( __FILE__ ) . 'assets/js/parallax.min.js', array(), AUREON_STUDIO_VERSION, true );
			wp_localize_script(
				'aureon-hero-parallax',
				'hero',
				array(
					'parallax' => apply_filters( 'aureon_hero_parallax_speed', 2 ),
				)
			);
		}
	}
	...
	public static function build_css() {
		...
		$image_url = apply_filters( 'aureon_page_hero_background_image_url', $image_url, $options );
		...
		if ( $padding_inside && function_exists( 'aureon_get_option' ) ) {
		...
		if ( '' !== $options['site_header_merge'] ) {
			if ( 'merge-desktop' === $options['site_header_merge'] ) {
				$css->start_media_query( apply_filters( 'aureon_not_mobile_media_query', '(min-width: 769px)' ) );
			}
			...
			if ( ! Aureon_Elements_Helper::does_option_exist( 'navigation-as-header' ) ) {
			...
			if ( $options['site_logo'] && Aureon_Elements_Helper::does_option_exist( 'navigation-as-header' ) ) {
				...
				if ( ! Aureon_Elements_Helper::does_option_exist( 'sticky-navigation-logo' ) ) {
			...
			if ( $options['navigation_logo'] && Aureon_Elements_Helper::does_option_exist( 'sticky-navigation' ) ) {
			...
			if ( $options['mobile_logo'] && Aureon_Elements_Helper::does_option_exist( 'mobile-logo' ) ) {
			...
			if ( $options['site_logo'] && Aureon_Elements_Helper::does_option_exist( 'site-logo' ) ) {
		...
		if ( Aureon_Elements_Helper::does_option_exist( 'site-logo' ) && 'merge-desktop' === $options['site_header_merge'] ) {
		...
		return apply_filters( 'aureon_page_hero_css_output', $css->css_output(), $options );
	}
	...
	public static function get_options() {
		...
		return apply_filters(
			...
			array(
				'element_id'                          => $post_id,
				'content'                             => get_post_meta( $post_id, '_aureon_element_content', true ),
				'classes'                             => get_post_meta( $post_id, '_aureon_hero_custom_classes', true ),
				'container'                           => get_post_meta( $post_id, '_aureon_hero_container', true ),
				'inner_container'                     => get_post_meta( $post_id, '_aureon_hero_inner_container', true ),
				'horizontal_alignment'                => get_post_meta( $post_id, '_aureon_hero_horizontal_alignment', true ),
				'full_screen'                         => get_post_meta( $post_id, '_aureon_hero_full_screen', true ),
				'vertical_alignment'                  => get_post_meta( $post_id, '_aureon_hero_vertical_alignment', true ),
				'padding_top'                         => get_post_meta( $post_id, '_aureon_hero_padding_top', true ),
				'padding_top_unit'                    => get_post_meta( $post_id, '_aureon_hero_padding_top_unit', true ),
				'padding_right'                       => get_post_meta( $post_id, '_aureon_hero_padding_right', true ),
				'padding_right_unit'                  => get_post_meta( $post_id, '_aureon_hero_padding_right_unit', true ),
				'padding_bottom'                      => get_post_meta( $post_id, '_aureon_hero_padding_bottom', true ),
				'padding_bottom_unit'                 => get_post_meta( $post_id, '_aureon_hero_padding_bottom_unit', true ),
				'padding_left'                        => get_post_meta( $post_id, '_aureon_hero_padding_left', true ),
				'padding_left_unit'                   => get_post_meta( $post_id, '_aureon_hero_padding_left_unit', true ),
				'padding_top_mobile'                  => get_post_meta( $post_id, '_aureon_hero_padding_top_mobile', true ),
				'padding_top_unit_mobile'             => get_post_meta( $post_id, '_aureon_hero_padding_top_unit_mobile', true ),
				'padding_right_mobile'                => get_post_meta( $post_id, '_aureon_hero_padding_right_mobile', true ),
				'padding_right_unit_mobile'           => get_post_meta( $post_id, '_aureon_hero_padding_right_unit_mobile', true ),
				'padding_bottom_mobile'               => get_post_meta( $post_id, '_aureon_hero_padding_bottom_mobile', true ),
				'padding_bottom_unit_mobile'          => get_post_meta( $post_id, '_aureon_hero_padding_bottom_unit_mobile', true ),
				'padding_left_mobile'                 => get_post_meta( $post_id, '_aureon_hero_padding_left_mobile', true ),
				'padding_left_unit_mobile'            => get_post_meta( $post_id, '_aureon_hero_padding_left_unit_mobile', true ),
				'background_image'                    => get_post_meta( $post_id, '_aureon_hero_background_image', true ),
				'disable_featured_image'              => get_post_meta( $post_id, '_aureon_hero_disable_featured_image', true ),
				'background_overlay'                  => get_post_meta( $post_id, '_aureon_hero_background_overlay', true ),
				'background_position'                 => get_post_meta( $post_id, '_aureon_hero_background_position', true ),
				'parallax'                            => get_post_meta( $post_id, '_aureon_hero_background_parallax', true ),
				'background_color'                    => get_post_meta( $post_id, '_aureon_hero_background_color', true ),
				'text_color'                          => get_post_meta( $post_id, '_aureon_hero_text_color', true ),
				'link_color'                          => get_post_meta( $post_id, '_aureon_hero_link_color', true ),
				'link_color_hover'                    => get_post_meta( $post_id, '_aureon_hero_background_link_color_hover', true ),
				'site_header_merge'                   => get_post_meta( $post_id, '_aureon_site_header_merge', true ),
				'site_header_height'                  => get_post_meta( $post_id, '_aureon_site_header_height', true ),
				'site_header_height_mobile'           => get_post_meta( $post_id, '_aureon_site_header_height_mobile', true ),
				'site_logo'                           => get_post_meta( $post_id, '_aureon_site_logo', true ),
				'retina_logo'                         => get_post_meta( $post_id, '_aureon_retina_logo', true ),
				'navigation_logo'                     => get_post_meta( $post_id, '_aureon_navigation_logo', true ),
				'mobile_logo'                         => get_post_meta( $post_id, '_aureon_mobile_logo', true ),
				'navigation_location'                 => get_post_meta( $post_id, '_aureon_navigation_location', true ),
				'header_background_color'             => get_post_meta( $post_id, '_aureon_site_header_background_color', true ),
				'header_title_color'                  => get_post_meta( $post_id, '_aureon_site_header_title_color', true ),
				'header_tagline_color'                => get_post_meta( $post_id, '_aureon_site_header_tagline_color', true ),
				'navigation_colors'                   => get_post_meta( $post_id, '_aureon_navigation_colors', true ),
				'navigation_background_color'         => get_post_meta( $post_id, '_aureon_navigation_background_color', true ),
				'navigation_text_color'               => get_post_meta( $post_id, '_aureon_navigation_text_color', true ),
				'navigation_background_color_hover'   => get_post_meta( $post_id, '_aureon_navigation_background_color_hover', true ),
				'navigation_text_color_hover'         => get_post_meta( $post_id, '_aureon_navigation_text_color_hover', true ),
				'navigation_background_color_current' => get_post_meta( $post_id, '_aureon_navigation_background_color_current', true ),
				'navigation_text_color_current'       => get_post_meta( $post_id, '_aureon_navigation_text_color_current', true ),
			)
		);
	}
	...
	public function after_setup() {
		...
		if ( $options['site_logo'] && Aureon_Elements_Helper::does_option_exist( 'site-logo' ) ) {
			if ( '' !== $options['site_header_merge'] ) {
				add_action( 'aureon_after_logo', array( $this, 'add_site_logo' ) );
			} else {
				add_filter( 'theme_mod_custom_logo', array( $this, 'replace_logo' ) );

				if ( $options['retina_logo'] && Aureon_Elements_Helper::does_option_exist( 'retina-logo' ) ) {
					add_filter( 'aureon_retina_logo', array( $this, 'replace_logo' ) );
				}
			}
		}

		if ( $options['navigation_logo'] && Aureon_Elements_Helper::does_option_exist( 'navigation-logo' ) ) {
			if ( $options['site_header_merge'] && Aureon_Elements_Helper::does_option_exist( 'sticky-navigation' ) ) {
				add_action( 'aureon_inside_navigation', array( $this, 'add_navigation_logo' ) );
			} else {
				add_filter( 'aureon_navigation_logo', array( $this, 'replace_logo' ) );
			}
		}

		if ( $options['mobile_logo'] && Aureon_Elements_Helper::does_option_exist( 'mobile-logo' ) ) {
			if ( 'merge' === $options['site_header_merge'] ) {
				add_action( 'aureon_inside_mobile_header', array( $this, 'add_mobile_header_logo' ) );
			} else {
				add_filter( 'aureon_mobile_header_logo', array( $this, 'replace_logo' ) );
			}
		}

		if ( $options['navigation_location'] ) {
			add_filter( 'aureon_navigation_location', array( $this, 'navigation_location' ) );
		}

		if ( '' !== $options['site_header_merge'] ) {
			add_action( 'aureon_before_header', array( $this, 'merged_header_start' ), 1 );
			add_action( 'aureon_after_header', array( $this, 'merged_header_end' ), 8 );

			if ( 'contained' === $options['container'] ) {
				add_filter( 'aureon_header_class', array( $this, 'site_header_classes' ) );
			}
		}
	...
	public static function add_site_logo() {
		...
		$attr = apply_filters(
			'aureon_page_hero_logo_attributes',
			array(
	...
	public static function add_mobile_header_logo() {
		...
		if ( 'title' === Aureon_Elements_Helper::does_option_exist( 'mobile-header-branding' ) ) {
			return;
		}
	...
	public static function template_tags( $content ) {
		...
		if ( is_singular() ) {
			...
			$post_title = __( 'Blog', 'aureon-studio' );
		}

		$replace[] = apply_filters( 'aureon_page_hero_post_title', $post_title );
		...
		if ( is_singular() ) {
			...
			$search[] = '{{post_date}}';
			$replace[] = apply_filters( 'aureon_page_hero_post_date', $time_string );
			...
			$search[] = '{{post_author}}';
			$replace[] = apply_filters( 'aureon_page_hero_post_author', $author );
			...
			if ( strpos( $content, '{{post_terms' ) !== false ) {
				...
				foreach ( $matches[1] as $match ) {
					...
					$terms = get_the_term_list( get_the_ID(), $match, apply_filters( 'aureon_page_hero_terms_before', '' ), apply_filters( 'aureon_page_hero_terms_separator', ', ' ), apply_filters( 'aureon_page_hero_terms_after', '' ) );
			...
			if ( strpos( $content, '{{custom_field' ) !== false ) {
				...
				foreach ( $matches[1] as $match ) {
					if ( null !== get_post_meta( get_the_ID(), $match, true ) && '_thumbnail_id' !== $match ) {
						...
						add_filter( 'wp_kses_allowed_html', [ 'Aureon_Elements_Helper', 'expand_allowed_html' ], 10, 2 );
						$value = wp_kses_post( $value );
						remove_filter( 'wp_kses_allowed_html', [ 'Aureon_Elements_Helper', 'expand_allowed_html' ], 10, 2 );
						$replace[] = $value;
					}
				}
				...
				if ( null !== $thumbnail_id ) {
					...
					$replace[] = wp_get_attachment_image( $thumbnail_id, apply_filters( 'aureon_hero_thumbnail_id_size', 'medium' ) );
				}
			}
		}
