( function( api ) {
	api.controlConstructor[ 'aureon-pro-customizer-typography' ] = api.Control.extend( {
		ready() {
			var control = this;

			control.container.on( 'change', '.aureon-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the aureon_get_all_google_fonts_ajax function
						var response = jQuery.getJSON( {
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'aureon_get_all_google_fonts_ajax',
								aureon_customize_nonce: aureon_customize.nonce,
							},
							async: false,
							dataType: 'json',
						} );
						...
						if ( id in fonts ) {
							...
							jQuery( '.aureon-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.aureon-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.aureon-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.aureon-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.aureon-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.aureon-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.aureon-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.aureon-font-variant' );
			...
			control.container.on( 'change', '.aureon-font-category input',
				function() {
					control.settings.category.set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.aureon-font-weight select',
				function() {
					control.settings.weight.set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.aureon-font-transform select',
				function() {
...
jQuery( document ).ready( function( $ ) {
	jQuery( '.aureon-font-family select' ).selectWoo();
	jQuery( '.aureon-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( '.aureon-font-family' ).each( function( key, value ) {
		var _this = $( this );

		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.aureon-font-variant' ).hide();
		}
	} );
} );
