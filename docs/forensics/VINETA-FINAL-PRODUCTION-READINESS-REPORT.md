...
1. Page-level duplicate libs on 3 templates (product: drift+photoswipe; shop: shop.js+nouislider; home/cookies: infinityslide). Fix later via manifest page-gating restructure (entries already support `"page"` keys) or stripping the raw template tags for those libs where the manifest is the sole provider.
2. Static info pages expose their title as `<h4>` in `.tf-page-title` with no `<h1>`; homepage hero uses per-slide `<h1>` (2 in DOM, 1 visible). Template-wide heading-hierarchy pass recommended (client-pack templates/CSS).
